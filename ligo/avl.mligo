...
[@inline]
let mem_new (value, mem : node * mem) : mem * ptr =
...
[@inline]
let max (i, j : int * int) : int = if i>j then i else j
...
let update_matching_child
    (ptr, from_ptr, to_ptr, mem : ptr * ptr * ptr * mem): mem =
...
let ref_rotate_left (curr_ptr, mem : ptr * mem): mem * ptr =
  let curr =
...
let ref_rotate_right (curr_ptr, mem : ptr * mem): mem * ptr =
  let curr =
    match mem_get (curr_ptr, mem) with
...
let ref_balance (curr_ptr, mem : ptr * mem): mem * ptr =
  match mem_get (curr_ptr, mem) with
  | Branch branch ->
...
let avl_push_back (root_ptr, value, tez, mem : avl_ptr * int * tez * mem) : mem * leaf_ptr =
    ...
    then (* total_tez <= limit *)
      ...
      then (* limit < left_tez < total_tez *)
        ref_split
          ...
          , branch.left
          , limit
          , (fun (left, rightM, mem : ptr option * ptr option * mem) ->
        ...
        ref_split
          ...
          , branch.right
          , limit - node_tez left
          , (fun (leftM, right, mem : ptr option * ptr option * mem) ->
              match leftM with
               | Some left ->
...
let avl_take (root_ptr, limit, root_data, mem : avl_ptr * tez * int * mem)
  : mem * avl_ptr =
  let root_ptr = match root_ptr with AvlPtr p -> p in
  match mem_get (root_ptr, mem) with
  | Root p -> (
    ...
    | Some r ->
        let (mem, l, r) = ref_split (mem, r, limit
                                    , (fun (a, b, c : ptr option * ptr option * mem) -> (c, a, b))) in
