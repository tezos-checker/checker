        ...
        LiquidationAuctionPrimitiveTypes.
          {
            ...
            min_kit_for_unwarranted =
              Some
                (kit_of_mukit (Ligo.nat_from_literal (string_of_int kit ^ "n")));
          })
...
let suite =
  "SliceListTests"
  >::: [
         ( "slice_list_from_auction - creates empty list when auction has no \
            corresponding burrow_slices"
         >:: fun _ ->
           let burrow_slices =
             slice_list_from_auction_state liquidation_auction_empty burrow_id_1
           in
           assert_equal
             (slice_list_empty burrow_id_1)
             burrow_slices ~printer:show_slice_list );
         ( qcheck_to_ounit
         @@ QCheck.Test.make
         ...
         let _ =
           List.fold_left
             (fun (auctions, burrow_slices) slice_contents ->
               let auctions, burrow_slices, element =
                 slice_list_append burrow_slices auctions auctions.queued_slices
                   QueueFront slice_contents
               in
               let youngest =
                 match slice_list_youngest burrow_slices auctions with
                 | Some expected_element -> expected_element
                 | None ->
                     failwith
                       "slice list should have bounds after appending but has \
                        none."
               in
               assert_equal youngest element ~printer:show_slice_list_element
                 ~msg:"new element is youngest one";
             ...
             (fun (auctions, burrow_slices) slice_contents ->
               ...
               let _ =
                 ...
                 | SliceListElement (ptr, slice) -> (
                     let avl_value =
                       Avl.avl_peek_front auctions.avl_storage
                         auctions.queued_slices
                     in
                     match avl_value with
                     | None ->
                         failwith "AVL was empty after appending slice to list"
                     | Some (avl_ptr, avl_slice) ->
                         assert_equal slice avl_slice.value
                           ~printer:show_liquidation_slice;
               ...
               let auctions, burrow_slices, element =
                 slice_list_append burrow_slices auctions auctions.queued_slices
                   QueueBack slice_contents
               in
               let _ =
                 match element with
                 | SliceListElement (_, slice) ->
                     (* Note: didn't know how to get the last item in the queue without doing this. *)
                     let avl_value =
                       List.hd
                         (List.rev
                            (TestAvl.avl_to_list auctions.avl_storage
                               auctions.queued_slices))
                     in
                     assert_equal slice avl_value
                       ~printer:show_liquidation_slice
               in
         ...
         @@ QCheck.Test.make
              ~name:
                "slice_list_remove - removed element is also removed from AVL \
                 storage"
              ~count:property_test_count
              (QCheck.triple
                 (QCheck.make (gen_liquidation_slice_contents_list 5))
                 (QCheck.make gen_liquidation_slice_contents_single_burrow_id)
                 (QCheck.make (gen_liquidation_slice_contents_list 5)))
         ...
         let auctions, burrow_slices =
           List.fold_left
             (fun (auctions, burrow_slices) slice_contents ->
               let auctions, burrow_slices, _ =
                 slice_list_append burrow_slices auctions auctions.queued_slices
                   queue_end slice_contents
               in
               (auctions, burrow_slices))
             (auctions, slice_list_empty burrow_id_1)
             first_section
         in
         (* Add the element which we will later remove *)
         let auctions, burrow_slices, to_remove =
           slice_list_append burrow_slices auctions auctions.queued_slices
             queue_end slice_to_remove
         in
         ...
         let auctions, burrow_slices =
           List.fold_left
             (fun (auctions, burrow_slices) slice_contents ->
               ...
               in
               (auctions, burrow_slices))
             (auctions, burrow_slices) second_section
         in

         (* Remove an element from the list *)
         let removed_ptr =
           match to_remove with SliceListElement (ptr, _) -> ptr
         in
         let auctions, _, _, _ =
           slice_list_remove burrow_slices auctions to_remove
         in
