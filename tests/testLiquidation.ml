...
let arbitrary_burrow (params : parameters) =
    ...
    QCheck.map
      (fun (t, k, factor) ->
        let tez =
          ...
          in
          fraction_to_tez_floor x.num x.den
        in
        let kit =
        ...
        in
        (tez, kit))
      (QCheck.triple positive_int positive_int positive_int)
  in
  ...
  let arb_smart_tez_kit =
    QCheck.map
      ...
      (QCheck.triple arb_smart_tez_kit_1 arb_smart_tez_kit_2 QCheck.int)
  in
  QCheck.map
    (fun (tez, kit) ->
      make_burrow_for_test ~address:burrow_addr ~delegate:None ~active:true
        ~collateral:tez ~outstanding_kit:kit ~excess_kit:kit_zero
...
let params : parameters =
  {
    q =
      fixedpoint_of_ratio_floor
        (make_ratio
           (Ligo.int_from_literal "1015")
           (Ligo.int_from_literal "1000"));
    index = Ligo.nat_from_literal "320_000n";
    protected_index = Ligo.nat_from_literal "360_000n";
    target =
      fixedpoint_of_ratio_floor
        (make_ratio (Ligo.int_from_literal "108") (Ligo.int_from_literal "100"));
...
let liquidatable_implies_overburrowed =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"liquidatable_implies_overburrowed"
       ~count:property_test_count (arbitrary_burrow params)
  @@ fun burrow ->
...
let optimistically_overburrowed_implies_overburrowed =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"optimistically_overburrowed_implies_overburrowed"
       ~count:property_test_count (arbitrary_burrow params)
  @@ fun burrow ->
  QCheck.(
...
let assert_properties_of_partial_liquidation params burrow_in details =
  ...
  assert_bool "partial liquidation means overburrowed input burrow"
    (burrow_is_overburrowed params burrow_in);
  assert_bool "partial liquidation means liquidatable input burrow"
    (burrow_is_liquidatable params burrow_in);
  assert_bool "partial liquidation means non-liquidatable output burrow"
    (not (burrow_is_liquidatable params burrow_out));
  assert_bool "partial liquidation means overburrowed output burrow"
  ...
  assert_tez_equal
    ...
    ~real:
      (Ligo.add_tez_tez
         (Ligo.add_tez_tez
            (burrow_collateral burrow_out)
            details.tez_to_auction)
         details.liquidation_reward);
  assert_tez_equal
    ~expected:(burrow_collateral_at_auction burrow_in)
    ~real:
      (Ligo.sub_tez_tez
         (burrow_collateral_at_auction burrow_out)
         details.tez_to_auction);
  assert_bool "partial liquidation does not deactivate burrows"
...
let assert_properties_of_complete_liquidation params burrow_in details =
  let burrow_out = details.burrow_state in
  assert_bool "complete liquidation means liquidatable input burrow"
    (burrow_is_liquidatable params burrow_in);
  assert_bool "complete liquidation means overburrowed input burrow"
    (burrow_is_overburrowed params burrow_in);
  assert_bool "complete liquidation means liquidatable output burrow"
    (burrow_is_liquidatable params burrow_out);
  assert_bool "complete liquidation means overburrowed output burrow"
    (burrow_is_overburrowed params burrow_out);
  assert_bool
  ...
  assert_bool "complete liquidation means no collateral in the output burrow"
  ...
  assert_tez_equal
    ...
    ~real:
      (Ligo.sub_tez_tez
         (burrow_collateral_at_auction burrow_out)
         details.tez_to_auction);
  assert_bool "complete liquidation does not deactivate burrows"
    (burrow_active burrow_out)
...
let assert_properties_of_close_liquidation params burrow_in details =
  let burrow_out = details.burrow_state in
  assert_bool "close liquidation means overburrowed input burrow"
    (burrow_is_overburrowed params burrow_in);
  assert_bool "close liquidation means liquidatable input burrow"
    (burrow_is_liquidatable params burrow_in);
  assert_bool "close liquidation means overburrowed output burrow"
    (burrow_is_overburrowed params burrow_out);
  assert_bool "close liquidation means non-liquidatable output burrow"
    (not (burrow_is_liquidatable params burrow_out));
  assert_bool "close liquidation means no collateral in the output burrow"
    (burrow_collateral burrow_out = Ligo.tez_from_literal "0mutez");
  assert_bool "close liquidation means inactive output burrow"
...
let test_general_liquidation_properties =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_general_liquidation_properties"
       ~count:property_test_count (arbitrary_burrow params)
  @@ fun burrow ->
  ...
  | None ->
      assert_bool "unnecessary liquidation means non-liquidatable input burrow"
...
let barely_not_overburrowed_test =
  ...
  assert_bool "is not optimistically overburrowed"
    (not (burrow_is_optimistically_overburrowed params burrow));
  ...
  assert_liquidation_result_equal ~expected:expected_liquidation_result
...
let barely_overburrowed_test =
  ...
  assert_bool "is optimistically overburrowed"
    (burrow_is_optimistically_overburrowed params burrow);
  assert_bool "is not liquidatable" (not (burrow_is_liquidatable params burrow));
...
let barely_liquidatable_test =
      ...
      ( Partial,
        {
          ...
          burrow_state =
            make_burrow_for_test ~active:true ~address:burrow_addr
              ~delegate:None
              ~collateral:(Ligo.tez_from_literal "2_346_632mutez")
              ~outstanding_kit:
                (kit_of_mukit (Ligo.nat_from_literal "10_000_000n"))
              ~excess_kit:kit_zero ~adjustment_index:fixedpoint_one
  ...
  let details =
    ...
    | Some (Partial, details) -> details
  in

  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "8_677_329n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
  ...
  assert_ratio_equal ~expected:expected_expected_kit ~real:expected_kit;
...
let barely_non_complete_liquidatable_test =
  ...
  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "15_229_815n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
...
let barely_complete_liquidatable_test =
  ...
  let details =
    ...
    | Some (Complete, details) -> details
  in

  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "15_229_814n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
...
let barely_close_liquidatable_test =
      ...
      ( Close,
        {
          ...
          burrow_state =
            make_burrow_for_test ~active:false ~address:burrow_addr
              ~delegate:None
              ~collateral:(Ligo.tez_from_literal "0mutez")
              ~outstanding_kit:
                (kit_of_mukit (Ligo.nat_from_literal "10_000_000n"))
  ...
  let details =
    ...
    | Some (Close, details) -> details
  in

  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "18_981_000n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
...
let unwarranted_liquidation_unit_test =
  ...
  let liquidation_result = burrow_request_liquidation params burrow in
  assert_liquidation_result_equal ~expected:None ~real:liquidation_result
...
let partial_liquidation_unit_test =
      ...
      ( Partial,
        {
          liquidation_reward =
            Ligo.add_tez_tez Constants.creation_deposit
              (Ligo.tez_from_literal "10_000mutez");
          tez_to_auction = Ligo.tez_from_literal "7_142_472mutez";
          burrow_state =
            make_burrow_for_test ~address:burrow_addr ~delegate:None
              ~active:true
  ...
  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "27_141_394n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
...
let complete_liquidation_unit_test =
      ...
      ( Complete,
        {
          ...
          burrow_state =
            make_burrow_for_test ~address:burrow_addr ~delegate:None
              ...
              ~collateral:(Ligo.tez_from_literal "0mutez")
              ~outstanding_kit:
                (kit_of_mukit (Ligo.nat_from_literal "100_000_000n"))
              ~excess_kit:kit_zero
              ~adjustment_index:(compute_adjustment_index params)
  ...
  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "170_810_000n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
  ...
  assert_bool "input burrow is optimistically overburrowed"
    (burrow_is_optimistically_overburrowed params burrow);
  assert_properties_of_complete_liquidation params burrow details
...
let complete_and_close_liquidation_test =
      ...
      ( Close,
        {
          liquidation_reward =
            Ligo.add_tez_tez Constants.creation_deposit
              (Ligo.tez_from_literal "1_000mutez");
  ...
  let expected_min_kit_for_unwarranted =
    kit_of_mukit (Ligo.nat_from_literal "189_810_000n")
  in
  assert_kit_option_equal ~expected:(Some expected_min_kit_for_unwarranted)
    ~real:(compute_min_kit_for_unwarranted params burrow details.tez_to_auction);
  ...
  assert_bool "input burrow is optimistically overburrowed"
    (burrow_is_optimistically_overburrowed params burrow);
  assert_bool "output burrow is optimistically overburrowed"
...
let test_burrow_request_liquidation_invariant_close =
  ...
  let kit_to_allow_liquidation =
    kit_of_mukit (Ligo.nat_from_literal "526_843n")
  in
  let arb_tez =
    QCheck.map
      (fun x -> Ligo.tez_from_literal (string_of_int x ^ "mutez"))
      QCheck.(0 -- upper_collat_bound_for_test)
  in

  qcheck_to_ounit
  @@ QCheck.Test.make
       ~name:
         "burrow_request_liquidation - burrow returned in case 2a (close \
          burrow) obeys burrow invariants"
       ~count:property_test_count arb_tez
  @@ fun collateral ->
  let burrow0 =
    make_burrow_for_test ~address:burrow_addr ~delegate:None ~active:true
      ~collateral ~outstanding_kit:kit_to_allow_liquidation ~excess_kit:kit_zero
      ~adjustment_index:(compute_adjustment_index params)
      ~collateral_at_auction:(Ligo.tez_from_literal "0mutez")
      ~last_touched:(Ligo.timestamp_from_seconds_literal 0)
  in

  let liquidation_details =
    match Burrow.burrow_request_liquidation initial_parameters burrow0 with
    | Some (Burrow.Close, liquidation_details) -> liquidation_details
    | None ->
        failwith
          "liquidation_result returned by burrow_request_liquidation was None \
           but the test expects a value."
    | Some (liquidation_type, _) ->
        failwith
          (Format.sprintf
             "liquidation_type returned by burrow_request_liquidation was %s \
              but Close was expected"
             (Burrow.show_liquidation_type liquidation_type))
  in

  Burrow.assert_burrow_invariants liquidation_details.burrow_state;
  assert_properties_of_close_liquidation initial_parameters burrow0
    liquidation_details;
  true
...
let test_burrow_request_liquidation_invariant_complete =
  ...
  let arb_tez =
    QCheck.map
      (fun x -> Ligo.tez_from_literal (string_of_int x ^ "mutez"))
      QCheck.(lower_collat_bound_for_test -- max_int)
  in

  qcheck_to_ounit
  @@ QCheck.Test.make
       ~name:
         "burrow_request_liquidation - burrow returned in case 2b (liquidate \
          all collateral) obeys burrow invariants"
  ...
  let min_kit_to_trigger_case =
    Ligo.sub_int_int
      (Common.cdiv_int_int
         (Ligo.mul_int_int
            (Ligo.int_from_literal "8_991")
            (Common.tez_to_mutez collateral))
         (Ligo.int_from_literal "10_000"))
      (Ligo.int_from_literal "899_999")
  in
  let outstanding_kit =
    match Ligo.is_nat min_kit_to_trigger_case with
    | Some n -> kit_add (kit_of_mukit n) extra_kit
    | None ->
        failwith
          "The calculated outstanding_kit for the test case was not a nat"
  in
  let burrow0 =
    make_burrow_for_test ~address:burrow_addr ~delegate:None ~active:true
      ~collateral ~outstanding_kit ~excess_kit:kit_zero
      ~adjustment_index:(compute_adjustment_index params)
      ~collateral_at_auction:(Ligo.tez_from_literal "0mutez")
      ~last_touched:(Ligo.timestamp_from_seconds_literal 0)
  in
  ...
  let liquidation_details =
    ...
    | Some (liquidation_type, _) ->
        failwith
          (Format.sprintf
             "liquidation_type returned by burrow_request_liquidation was %s \
              but Complete was expected"
             (Burrow.show_liquidation_type liquidation_type))
  in
  Burrow.assert_burrow_invariants liquidation_details.burrow_state;
  assert_properties_of_complete_liquidation initial_parameters burrow0
    liquidation_details;
  true
...
let test_burrow_request_liquidation_invariant_partial =
  ...
  let arb_kit =
    QCheck.map
      ...
      QCheck.(min_kit_for_case -- max_kit_for_case)
  in

  qcheck_to_ounit
  @@ QCheck.Test.make
       ~name:
         "burrow_request_liquidation - burrow returned in case 2c (partially \
          liquidate collateral) obeys burrow invariants"
       ~count:property_test_count arb_kit
  @@ fun outstanding_kit ->
  let burrow0 =
    make_burrow_for_test ~address:burrow_addr ~delegate:None ~active:true
      ~collateral:(Ligo.tez_from_literal (string_of_int collateral ^ "mutez"))
      ~outstanding_kit ~excess_kit:kit_zero
      ~adjustment_index:(compute_adjustment_index params)
      ~collateral_at_auction:(Ligo.tez_from_literal "0mutez")
      ~last_touched:(Ligo.timestamp_from_seconds_literal 0)
  in
  ...
  let liquidation_details =
    ...
    | Some (liquidation_type, _) ->
        failwith
          (Format.sprintf
             "liquidation_type returned by burrow_request_liquidation was %s \
              but Partial was expected"
             (Burrow.show_liquidation_type liquidation_type))
  in
  Burrow.assert_burrow_invariants liquidation_details.burrow_state;
  assert_properties_of_partial_liquidation initial_parameters burrow0
    liquidation_details;
  true
...
let test_burrow_request_liquidation_preserves_tez =
  ...
  let _ =
    ...
    | Some (_, liquidation_details) ->
        ...
        let tez_out =
          Ligo.add_tez_tez liquidation_details.liquidation_reward
            (burrow_total_associated_tez liquidation_details.burrow_state)
        in
        ...
        assert (
          Ligo.eq_tez_tez
            ...
            (Ligo.sub_tez_tez
               (Burrow.burrow_collateral_at_auction
                  liquidation_details.burrow_state)
...
let regression_test_72 =
  ...
  let burrow0 =
    Burrow.make_burrow_for_test
      ...
      ~outstanding_kit:
        (kit_of_mukit (Ligo.nat_from_literal "3928478924648448718n"))
      ~collateral_at_auction:(Ligo.tez_from_literal "0mutez")
      ~excess_kit:kit_zero ~active:true ~address:burrow_addr ~delegate:None
  ...
  let liquidation_details =
    match
      Burrow.burrow_request_liquidation Parameters.initial_parameters burrow0
    with
    | Some (Burrow.Complete, liquidation_details) -> liquidation_details
    | None ->
        failwith
    ...
    | Some (liquidation_type, _) ->
        failwith
          (Format.sprintf
             ...
             (Burrow.show_liquidation_type liquidation_type))
  in
  assert_properties_of_complete_liquidation Parameters.initial_parameters
    burrow0 liquidation_details
...
let regression_test_93 =
  ...
  let liquidation_details =
    match
      Burrow.burrow_request_liquidation Parameters.initial_parameters burrow_in
    with
    | Some (Burrow.Complete, liquidation_details) -> liquidation_details
    | liquidation_result ->
        assert_failure
          ( "Unexpected liquidation result: "
          ^ Burrow.show_liquidation_result liquidation_result )
  in
  assert_properties_of_complete_liquidation Parameters.initial_parameters
    burrow_in liquidation_details;
  (* The following line must succeed. *)
  let _ =
    compute_min_kit_for_unwarranted Parameters.initial_parameters burrow_in
      liquidation_details.tez_to_auction
  in
  ...
  let liquidation_details =
    ...
    | liquidation_result ->
        assert_failure
          ...
          ^ Burrow.show_liquidation_result liquidation_result )
  in
  assert_properties_of_close_liquidation Parameters.initial_parameters burrow_in
    liquidation_details;
  assert_bool "For this test to be potent the collateral should have been zero"
    (Ligo.eq_tez_tez
       (burrow_collateral burrow_in)
