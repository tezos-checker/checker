    ...
    (* NOTE: this could still give us tough numbers I think. The liquidity created can be zero for example. *)
      ...
      let max_kit_deposited =
        let { num = x_num; den = x_den } =
          mul_ratio (kit_to_ratio kit)
            (make_ratio (ctez_to_muctez_int amount) (ctez_to_muctez_int ctez))
        in
        kit_of_fraction_ceil x_num x_den
      in
      let min_lqt_minted =
        let { num = x_num; den = x_den } =
          mul_ratio (lqt_to_ratio lqt)
            (make_ratio (ctez_to_muctez_int amount) (ctez_to_muctez_int ctez))
        in
        lqt_of_fraction_floor x_num x_den
      in
      let deadline =
        Ligo.add_timestamp_int !Ligo.Tezos.now (Ligo.int_from_literal "1")
      in
      (* always one second later *)
      (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline))
    (QCheck.pair
       (arbitrary_non_empty_cfmm one_ratio !Ligo.Tezos.level)
       TestArbitrary.arb_positive_ctez)
  ...
  QCheck.map
    (* NOTE: this could still give us tough numbers I think. *)
      ...
      let min_ctez_withdrawn =
        let { num = x_num; den = x_den } =
          div_ratio
            (mul_ratio (ratio_of_ctez ctez) (lqt_to_ratio lqt_to_burn))
            (lqt_to_ratio lqt)
        in
      ...
      let min_kit_withdrawn =
        let { num = x_num; den = x_den } =
          div_ratio
            (mul_ratio (kit_to_ratio kit) (lqt_to_ratio lqt_to_burn))
            (lqt_to_ratio lqt)
        in
      ...
      let lqt_burned, min_ctez_withdrawn, min_kit_withdrawn =
        if
          lqt_to_burn = lqt_zero
          || min_ctez_withdrawn = ctez_zero
          || min_kit_withdrawn = kit_zero
        then
          let lqt_to_burn =
            let least_kit_percentage =
              div_ratio
                (kit_to_ratio (kit_of_mukit (Ligo.nat_from_literal "1n")))
                (kit_to_ratio kit)
            in
            let least_ctez_percentage =
              make_ratio
                (ctez_to_muctez_int
                   (ctez_of_muctez (Ligo.nat_from_literal "1n")))
                (ctez_to_muctez_int ctez)
            in
            let as_q =
              mul_ratio (lqt_to_ratio lqt)
                (max_ratio least_kit_percentage least_ctez_percentage)
            in
            lqt_of_fraction_ceil as_q.num as_q.den
          in
          let lqt_burned = lqt_to_burn in
          let min_ctez_withdrawn =
            let { num = x_num; den = x_den } =
        ...
        else (lqt_burned, min_ctez_withdrawn, min_kit_withdrawn)
      in
...
let test_buy_kit_increases_price =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_buy_kit_increases_price"
  ...
  let _bought_kit, new_cfmm =
    cfmm_buy_kit cfmm amount min_kit_expected deadline
  in
...
let test_buy_kit_increases_product =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_buy_kit_increases_product"
       ~count:property_test_count make_inputs_for_buy_kit_to_succeed
  @@ fun (cfmm, amount, min_kit_expected, deadline) ->
  let _bought_kit, new_cfmm =
    cfmm_buy_kit cfmm amount min_kit_expected deadline
...
let test_buy_kit_does_not_affect_liquidity =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_buy_kit_does_not_affect_liquidity"
       ~count:property_test_count make_inputs_for_buy_kit_to_succeed
  @@ fun (cfmm, amount, min_kit_expected, deadline) ->
  let _bought_kit, new_cfmm =
    cfmm_buy_kit cfmm amount min_kit_expected deadline
...
let test_buy_kit_preserves_ctez =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_buy_kit_preserves_ctez"
       ~count:property_test_count make_inputs_for_buy_kit_to_succeed
  @@ fun (cfmm, amount, min_kit_expected, deadline) ->
  let _bought_kit, new_cfmm =
    cfmm_buy_kit cfmm amount min_kit_expected deadline
...
let test_sell_kit_decreases_price =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_sell_kit_decreases_price"
  ...
  let _bought_ctez, new_cfmm =
    cfmm_sell_kit cfmm kit_amount min_ctez_expected deadline
  in
...
let test_sell_kit_increases_product =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_sell_kit_increases_product"
       ~count:property_test_count make_inputs_for_sell_kit_to_succeed
  @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
  let _bought_ctez, new_cfmm =
    cfmm_sell_kit cfmm kit_amount min_ctez_expected deadline
...
let test_sell_kit_does_not_affect_liquidity =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_sell_kit_does_not_affect_liquidity"
       ~count:property_test_count make_inputs_for_sell_kit_to_succeed
  @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
  let _bought_ctez, new_cfmm =
    cfmm_sell_kit cfmm kit_amount min_ctez_expected deadline
...
let test_sell_kit_respects_min_ctez_expected =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_sell_kit_respects_min_ctez_expected"
       ~count:property_test_count make_inputs_for_sell_kit_to_succeed
  @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
...
let test_sell_kit_preserves_ctez =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_sell_kit_preserves_ctez"
       ~count:property_test_count make_inputs_for_sell_kit_to_succeed
  @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
...
let sell_kit_unit_test =
  ...
  assert_raises
    ...
    (fun () ->
      cfmm_sell_kit cfmm kit_one ctez_zero
        (Ligo.timestamp_from_seconds_literal 10))
...
let test_add_liquidity_might_decrease_price =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_liquidity_might_decrease_price"
       ~count:property_test_count make_inputs_for_add_liquidity_to_succeed
  @@ fun (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline) ->
  let _bought_liquidity, _bought_kit, new_cfmm =
    cfmm_add_liquidity cfmm amount max_kit_deposited min_lqt_minted deadline
  in
...
let test_add_liquidity_increases_product =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_liquidity_increases_product"
       ~count:property_test_count make_inputs_for_add_liquidity_to_succeed
  @@ fun (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline) ->
  let _bought_liquidity, _bought_kit, new_cfmm =
    cfmm_add_liquidity cfmm amount max_kit_deposited min_lqt_minted deadline
...
let test_add_liquidity_increases_liquidity =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_liquidity_increases_liquidity"
       ~count:property_test_count make_inputs_for_add_liquidity_to_succeed
  @@ fun (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline) ->
  let _bought_liquidity, _bought_kit, new_cfmm =
    cfmm_add_liquidity cfmm amount max_kit_deposited min_lqt_minted deadline
...
let test_add_liquidity_respects_min_lqt_minted =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_liquidity_respects_min_lqt_minted"
       ~count:property_test_count make_inputs_for_add_liquidity_to_succeed
  @@ fun (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline) ->
...
let test_add_liquidity_respects_max_kit_deposited =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_liquidity_respects_max_kit_deposited"
       ~count:property_test_count make_inputs_for_add_liquidity_to_succeed
  @@ fun (cfmm, amount, max_kit_deposited, min_lqt_minted, deadline) ->
...
let add_liquidity_unit_test =
  ...
  let cfmm : cfmm =
    cfmm_make_for_test
      ...
      ~last_level:(Ligo.nat_from_literal "0n")
  in
  let expected_returned_liquidity =
    lqt_of_denomination (Ligo.nat_from_literal "2n")
  in
  let expected_returned_kit =
    kit_of_mukit (Ligo.nat_from_literal "5_605_758n")
  in
  let expected_updated_cfmm : cfmm =
    cfmm_make_for_test
  ...
  let returned_liquidity, returned_kit, updated_cfmm =
    cfmm_add_liquidity cfmm
      (ctez_of_muctez (Ligo.nat_from_literal "20_000_000n"))
      (kit_of_mukit (Ligo.nat_from_literal "20_000_000n"))
      (lqt_of_denomination (Ligo.nat_from_literal "2n"))
      (Ligo.timestamp_from_seconds_literal 1)
  in
  assert_lqt_equal ~expected:expected_returned_liquidity
    ~real:returned_liquidity;
  assert_kit_equal ~expected:expected_returned_kit ~real:returned_kit;
  assert_cfmm_equal ~expected:expected_updated_cfmm ~real:updated_cfmm
...
let test_add_liquidity_failures =
  ...
  assert_raises
    ...
    (fun () ->
      cfmm_add_liquidity cfmm ctez_zero
        (kit_of_mukit (Ligo.nat_from_literal "20_000_000n"))
        (lqt_of_denomination (Ligo.nat_from_literal "2n"))
        (Ligo.timestamp_from_seconds_literal 1));
  assert_raises
  ...
  assert_raises
    ...
    (fun () ->
      Ligo.Tezos.reset ();
      let deadline =
        Ligo.add_timestamp_int !Ligo.Tezos.now (Ligo.int_from_literal "0")
      in
      (* No time passed; failure (tests equality) *)
      cfmm_add_liquidity cfmm
        (ctez_of_muctez (Ligo.nat_from_literal "1n"))
        (kit_of_mukit (Ligo.nat_from_literal "1n"))
        lqt_zero deadline);
  assert_raises
    ...
    (fun () ->
      ...
      let deadline =
        Ligo.add_timestamp_int !Ligo.Tezos.now (Ligo.int_from_literal "1")
      in
      (* One second passed; failure (tests inequality) *)
      Ligo.Tezos.new_transaction ~seconds_passed:2 ~blocks_passed:1
        ~sender:alice_addr
        ~amount:(Ligo.tez_from_literal "0mutez");
      cfmm_add_liquidity cfmm
        (ctez_of_muctez (Ligo.nat_from_literal "1n"))
...
let test_remove_liquidity_decreases_product =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_decreases_product"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
  let _withdrawn_ctez, _withdrawn_kit, new_cfmm =
    cfmm_remove_liquidity cfmm lqt_burned min_ctez_withdrawn min_kit_withdrawn
      deadline
  in
...
let test_remove_liquidity_decreases_liquidity =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_decreases_liquidity"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
  let _withdrawn_ctez, _withdrawn_kit, new_cfmm =
    cfmm_remove_liquidity cfmm lqt_burned min_ctez_withdrawn min_kit_withdrawn
...
let test_remove_liquidity_respects_min_ctez_withdrawn =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_respects_min_ctez_withdrawn"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
...
let test_remove_liquidity_respects_min_kit_withdrawn =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_respects_min_kit_withdrawn"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
...
let test_remove_liquidity_respects_ctez_limit =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_respects_ctez_limit"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
  let withdrawn_ctez, _withdrawn_kit, _new_cfmm =
    cfmm_remove_liquidity cfmm lqt_burned min_ctez_withdrawn min_kit_withdrawn
...
let test_remove_liquidity_respects_kit_limit =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_liquidity_respects_kit_limit"
       ~count:property_test_count make_inputs_for_remove_liquidity_to_succeed
  @@ fun (cfmm, lqt_burned, min_ctez_withdrawn, min_kit_withdrawn, deadline) ->
  let _withdrawn_ctez, withdrawn_kit, _new_cfmm =
    cfmm_remove_liquidity cfmm lqt_burned min_ctez_withdrawn min_kit_withdrawn
...
let test_remove_liquidity_failures =
    ...
    cfmm_add_liquidity
      {
        ...
        ctez =
          ctez_add cfmm.ctez
            (ctez_of_muctez (Ligo.nat_from_literal "10_000_000n"));
      }
  ...
  assert_raises
    ...
    (fun () ->
      cfmm_remove_liquidity cfmm lqt_zero
        (ctez_of_muctez (Ligo.nat_from_literal "1n"))
        (kit_of_mukit (Ligo.nat_from_literal "1n"))
  ...
  assert_raises
    ...
    (fun () ->
      cfmm_remove_liquidity cfmm liq ctez_zero
        (kit_of_mukit (Ligo.nat_from_literal "1n"))
        (Ligo.timestamp_from_seconds_literal 100));
  assert_raises
    (Failure (Ligo.string_of_int error_RemoveLiquidityNoKitWithdrawnExpected))
    (fun () ->
      cfmm_remove_liquidity cfmm liq
        (ctez_of_muctez (Ligo.nat_from_literal "1n"))
        (kit_of_mukit (Ligo.nat_from_literal "0n"))
        (Ligo.timestamp_from_seconds_literal 100));
  assert_raises
...
let cfmm_tests_from_mutations =
  ...
  >::: [
         ...
         >:: fun _ ->
           ...
           assert_ctez_equal
             ~expected:
               (Ctez.ctez_of_muctez (Ligo.nat_from_literal "1_000_000n"))
             ~real:ctez_withdrawn );
         (* This test catches the following mutation:
         ...
         >:: fun _ ->
           Ligo.Tezos.reset ();
           let total_ctez =
             ctez_of_muctez (Ligo.nat_from_literal "123_456_789n")
           in
           let total_kit = kit_of_mukit (Ligo.nat_from_literal "37_194_834n") in
           let total_lqt =
             lqt_of_denomination (Ligo.nat_from_literal "999_999n")
           in
           let cfmm =
             cfmm_make_for_test ~ctez:total_ctez ~kit:total_kit ~lqt:total_lqt
               ~kit_in_ctez_in_prev_block:one_ratio
               ~last_level:!Ligo.Tezos.level
           in

           let lqt_burned =
             Lqt.lqt_of_denomination (Ligo.nat_from_literal "312_465n")
           in
           ...
           in
           (* zero sum properties *)
           assert_ctez_equal ~expected:cfmm.ctez
             ~real:(Ctez.ctez_add ctez_withdrawn updated_cfmm.ctez);
           assert_kit_equal ~expected:cfmm.kit
             ~real:(Kit.kit_add kit_withdrawn updated_cfmm.kit);
           assert_lqt_equal ~expected:cfmm.lqt
             ~real:(Lqt.lqt_add lqt_burned updated_cfmm.lqt);
           (* To give *)
           assert_ctez_equal
             ~expected:
               (Ctez.ctez_of_muctez (Ligo.nat_from_literal "38_575_964n"))
             ~real:ctez_withdrawn;
           assert_kit_equal
           ...
           assert_ctez_equal
             ~expected:
               (Ctez.ctez_of_muctez (Ligo.nat_from_literal "84_880_825n"))
           ...
           assert_lqt_equal
             ~expected:
               (Lqt.lqt_of_denomination (Ligo.nat_from_literal "687_534n"))
         ...
         >:: fun _ ->
           ...
           assert_kit_equal ~expected:(kit_add cfmm.kit accrual)
             ~real:updated_cfmm.kit );
       ]
