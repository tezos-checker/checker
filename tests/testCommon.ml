...
let suite =
  ...
  >::: [
         ...
         ( "cdiv_int_int - fails when denominator is zero" >:: fun _ ->
           assert_raises
             (Failure
                (Ligo.string_of_int internalError_CdivIntIntZeroDenominator))
             (fun _ ->
               cdiv_int_int
                 (Ligo.int_from_literal "5")
           ...
           assert_int_equal
             ...
             ~real:
               (cdiv_int_int
                  (Ligo.int_from_literal "-5")
                  (Ligo.int_from_literal "-2")) );
         ( "fdiv_int_int - fails when denominator is zero" >:: fun _ ->
           assert_raises
             (Failure
                (Ligo.string_of_int internalError_FdivIntIntZeroDenominator))
             (fun _ ->
               fdiv_int_int
                 (Ligo.int_from_literal "5")
                 (Ligo.int_from_literal "0")) );
