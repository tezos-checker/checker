...
let rec call_touch_times (index : Ligo.nat) (kit_in_tez : ratio) (n : int)
    (params : parameters)
    : parameters
  =
  if n <= 0 then params
    ...
    let _total_accrual_to_cfmm, new_params =
      parameters_touch index kit_in_tez params
    in
...
let test_compute_drift_derivative_no_acceleration =
  ...
  assert_fixedpoint_equal ~expected:fixedpoint_zero
...
let test_compute_imbalance_all_zero =
  ...
  assert_ratio_equal ~expected:zero_ratio
...
let test_compute_imbalance_negative_small =
  ...
  assert_ratio_equal
    ~expected:
      (make_ratio
         (Ligo.int_from_literal "-187499997")
         (Ligo.int_from_literal "3750000004"))
...
let test_compute_imbalance_positive_small =
  ...
  assert_ratio_equal
    ~expected:
      (make_ratio
         (Ligo.int_from_literal "199999998")
         (Ligo.int_from_literal "4000000000"))
      (* JUST BELOW SATURATION *)
    ~real:(compute_imbalance outstanding circulating)
...
let test_imbalance_upper_bound =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_upper_bound"
...
let test_imbalance_lower_bound =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_lower_bound"
       ~count:property_test_count
       (QCheck.pair TestArbitrary.arb_kit TestArbitrary.arb_kit)
  @@ fun (burrowed, circulating) ->
  geq_ratio_ratio
...
let test_imbalance_sign_preservation =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_sign_preservation"
       ~count:property_test_count
       (QCheck.pair TestArbitrary.arb_kit TestArbitrary.arb_kit)
  @@ fun (burrowed, circulating) ->
  sign_ratio (compute_imbalance burrowed circulating)
  = kit_compare circulating burrowed

(* If burrowed = circulating then imbalance = 0. *)
let test_imbalance_is_zero_when_equal =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_is_zero_when_equal"
       ~count:property_test_count TestArbitrary.arb_kit
  @@ fun kit ->
  eq_ratio_ratio
...
let test_imbalance_positive_tendencies =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_positive_tendencies"
       ~count:property_test_count
  ...
  let circulating, burrowed2, burrowed1 =
    ...
    sort_three_kit_amounts_increasing_order kit1 kit2 kit3
  in
  leq_ratio_ratio
...
let test_imbalance_negative_tendencies =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_imbalance_negative_tendencies"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
  @@ fun (kit1, kit2, kit3) ->
...
let test_protected_index_follows_index =
  ...
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_protected_index_follows_index"
       ~count:property_test_count
  ...
  let lvl = lvl + 1 in
  Ligo.Tezos.new_transaction ~seconds_passed:(lvl * 60) ~blocks_passed:lvl
    ~sender:alice_addr
    ~amount:(Ligo.tez_from_literal "0mutez");
...
let test_protected_index_pace =
  ...
  let very_high_index =
    Ligo.abs
      (Ligo.mul_int_int (Ligo.int_from_literal "1000") (Ligo.int params.index))
  in
  ...
  Ligo.Tezos.reset ();
  let new_params =
    call_touch_times very_high_index kit_in_tez 60 (* 60 blocks ~ 1h *) params
  in
  assert_nat_equal
    ~expected:(Ligo.nat_from_literal "1_030_420n")
  ...
  Ligo.Tezos.reset ();
  let new_params =
    call_touch_times very_high_index kit_in_tez
      (60 * 24 (* 60 blocks ~ 1h *))
      params
  in
  assert_nat_equal
    ~expected:(Ligo.nat_from_literal "2_053_031n")
    ~real:new_params.protected_index;

  (* DOWNWARD MOVES *)
  let very_low_index =
    fraction_to_nat_floor (Ligo.int params.index)
      (Ligo.int_from_literal "1_000")
  in
  ...
  Ligo.Tezos.reset ();
  let new_params =
    call_touch_times very_low_index kit_in_tez 60 (* 60 blocks ~ 1h *) params
  in
  assert_nat_equal
    ~expected:(Ligo.nat_from_literal "970_407n")
    ~real:new_params.protected_index;
  ...
  Ligo.Tezos.reset ();
  let new_params =
    call_touch_times very_low_index kit_in_tez
      (60 * 24 (* 60 blocks ~ 1h *))
      params
  in
  assert_nat_equal
    ~expected:(Ligo.nat_from_literal "486_151n")
    ~real:new_params.protected_index
...
let test_minting_index_low_bounded =
  ...
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_minting_index_low_bounded"
       ~count:property_test_count
       (QCheck.map
  ...
  tz_minting new_params >= Ligo.nat_from_literal "999_500n"
...
let test_minting_index_high_unbounded =
  ...
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_minting_index_high_unbounded"
       ~count:property_test_count
       (QCheck.map
          (fun x -> Ligo.nat_from_literal (string_of_int x ^ "n"))
          QCheck.(1_000_001 -- max_int))
  @@ fun index ->
  Ligo.Tezos.reset ();
  (* just the next block *)
  Ligo.Tezos.new_transaction ~seconds_passed:60 ~blocks_passed:1
  ...
  let _total_accrual_to_cfmm, new_params =
    parameters_touch index kit_in_tez params
  in
  assert_nat_equal ~expected:index ~real:(tz_minting new_params);
  true
...
let test_liquidation_index_high_bounded =
  ...
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_liquidation_index_high_bounded"
       ~count:property_test_count
       (QCheck.map
          (fun x -> Ligo.nat_from_literal (string_of_int x ^ "n"))
          QCheck.(1_000_001 -- max_int))
  ...
  tz_liquidation new_params <= Ligo.nat_from_literal "1_000_500n"
...
let test_liquidation_index_low_unbounded =
  ...
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_liquidation_index_low_unbounded"
       ~count:property_test_count
       (QCheck.map
          (fun x -> Ligo.nat_from_literal (string_of_int x ^ "n"))
          QCheck.(1 -- 999_999))
  ...
  let _total_accrual_to_cfmm, new_params =
    parameters_touch index kit_in_tez params
  in
  assert_nat_equal ~expected:index ~real:(tz_liquidation new_params);
  true
...
let test_touch_0 =
  ...
  let kit_in_tez =
    make_ratio (Ligo.int_from_literal "3") (Ligo.int_from_literal "1")
  in
  ...
  Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
    ~sender:alice_addr
    ~amount:(Ligo.tez_from_literal "0mutez");
  let total_accrual_to_cfmm, out_params =
    parameters_touch index kit_in_tez in_params
  in

  assert_kit_equal ~expected:kit_zero ~real:total_accrual_to_cfmm;
  assert_parameters_equal ~expected:expected_out_params ~real:out_params;
  ()
...
let test_touch_1 =
  ...
  let initial_parameters : parameters =
    {
      q = fixedpoint_of_hex_string "0.E666666666666666";
      (* 0.9 *)
      index = Ligo.nat_from_literal "360_000n";
      target = fixedpoint_of_hex_string "1.147AE147AE147AE1";
      (* 1.08 *)
  ...
  in
  Ligo.Tezos.reset ();
  Ligo.Tezos.new_transaction ~seconds_passed:3600 ~blocks_passed:60
    ~sender:alice_addr
    ~amount:(Ligo.tez_from_literal "0mutez");

  let new_index = Ligo.nat_from_literal "340_000n" in
  let kit_in_tez =
    make_ratio (Ligo.int_from_literal "305") (Ligo.int_from_literal "1000")
  in
  let total_accrual_to_cfmm, new_parameters =
    parameters_touch new_index kit_in_tez initial_parameters
  in
  assert_parameters_equal
    ~expected:
      {
        q = fixedpoint_of_hex_string "0.E6666895A3EC8BA5";
        (* 0.90000013020828555983 *)
  ...
  let initial_parameters : parameters =
    {
      ...
      burrow_fee_index = fixedpoint_one;
      imbalance_index = fixedpoint_one;
      outstanding_kit = kit_of_mukit (Ligo.nat_from_literal "1_753_165n");
      (* 1_753_164n should leave as is *)
...
let test_add_remove_circulating_kit_inverses =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_remove_circulating_kit"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
...
let test_add_circulating_kit_effect =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_circulating_kit_effect"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
  @@ fun (burrowed, circulating, kit) ->
  ...
  assert_parameters_equal ~expected:params1
...
let test_add_outstanding_and_circulating_kit_effect =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_add_outstanding_and_circulating_kit_effect"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
  @@ fun (burrowed, circulating, kit) ->
...
let test_remove_circulating_kit_effect =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"test_remove_circulating_kit_effect"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
  @@ fun (kit1, kit2, kit3) ->
  let kit_to_remove = kit_min (kit_min kit1 kit2) kit3 in

  (* to avoid underflows *)
  let params1 =
    { initial_parameters with outstanding_kit = kit1; circulating_kit = kit2 }
  in
...
let test_remove_outstanding_and_circulating_kit_effect =
  qcheck_to_ounit
  @@ QCheck.Test.make ~name:"remove_outstanding_and_circulating_kit"
       ~count:property_test_count
       (QCheck.triple TestArbitrary.arb_kit TestArbitrary.arb_kit
          TestArbitrary.arb_kit)
  @@ fun (kit1, kit2, kit3) ->
...
let test_add_remove_outstanding_circulating_kit_unit =
  ...
  let params = initial_parameters in
  (* initially both zero *)
  assert_kit_equal ~expected:kit_zero ~real:params.circulating_kit;
  assert_kit_equal ~expected:kit_zero ~real:params.outstanding_kit;
  (* add some circulating kit only *)
  let kit_to_add_to_circulating =
    Kit.kit_of_mukit (Ligo.nat_from_literal "172_635_932_647n")
  in
  ...
  let kit_to_add_to_both =
    Kit.kit_of_mukit (Ligo.nat_from_literal "5_473_635_298_465n")
  in
  ...
  let kit_to_remove_from_both =
    Kit.kit_of_mukit (Ligo.nat_from_literal "765_601_721n")
  in
  let params =
    remove_outstanding_and_circulating_kit params kit_to_remove_from_both
  in
  ...
  let kit_to_remove_from_circulating =
    Kit.kit_of_mukit (Ligo.nat_from_literal "4_123_827_936_001n")
  in
...
let test_compute_current_q =
  ...
  let duration_in_seconds = Ligo.int_from_literal "193" in

  (* around three blocks *)
  let current_q =
    compute_current_q last_q last_drift last_drift_derivative
...
let test_compute_current_drift =
  ...
  (* around three blocks *)
  let current_drift =
    compute_current_drift last_drift last_drift_derivative
      current_drift_derivative duration_in_seconds
  in
