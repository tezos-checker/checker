...
let empty_checker_with_cfmm (cfmm : CfmmTypes.cfmm) =
  let checker_kit =
    kit_sub cfmm.kit (kit_of_mukit (Ligo.nat_from_literal "1n"))
  in
  let checker_liquidity =
    lqt_sub cfmm.lqt (lqt_of_denomination (Ligo.nat_from_literal "1n"))
  in
  let checker =
    {
      empty_checker with
      parameters =
        { empty_checker.parameters with circulating_kit = checker_kit };
      cfmm;
      fa2_state =
        (let fa2_state = initial_fa2_state in
         let fa2_state =
           ledger_issue_lqt
             (fa2_state, !Ligo.Tezos.self_address, checker_liquidity)
         in
         let fa2_state =
           ledger_issue_kit (fa2_state, !Ligo.Tezos.self_address, checker_kit)
         in
         fa2_state);
    }
  in
...
let newly_created_burrow (checker : checker) (burrow_no : string)
    : burrow_id * checker
  =
  let _ops, checker =
    Checker.entrypoint_create_burrow
      (checker, (Ligo.nat_from_literal "0n", None))
  in
  ((!Ligo.Tezos.sender, Ligo.nat_from_literal burrow_no), checker)

let get_balance_of (checker : checker) (addr : Ligo.address)
    (tok : fa2_token_id)
    : Ligo.nat
  =
  let ops, _checker =
    Checker.strict_entrypoint_balance_of
      ( checker,
        {
          requests = [ { owner = addr; token_id = tok } ];
          callback = Ligo.contract_of_address addr;
        } )
  in
  match ops with
  | [
   Transaction
     ( FA2BalanceOfResponseTransactionValue [ { request = _; balance = kit } ],
       _,
       _ );
  ] ->
      kit
...
let suite =
  "Checker tests"
  >::: [
         ( "initial touch (noop)" >:: fun _ ->
           Ligo.Tezos.reset ();
           let checker1 = empty_checker in
           let ops, checker2 =
             Checker.touch_with_index checker1 (Ligo.nat_from_literal "0n")
           in

           assert_operation_list_equal ~expected:[] ~real:ops;
           assert_equal checker1 checker2;
           (* NOTE: we really want them to be identical here, hence the '='. *)
           () );
         ( "create_burrow - updates checker storage" >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           assert_bool "No matching burrow found after calling create_burrow"
             (Option.is_some (Ligo.Big_map.find_opt burrow_id checker.burrows));
           assert_bool "The burrow existed before calling create_burrow"
             (Option.is_none
                (Ligo.Big_map.find_opt burrow_id empty_checker.burrows)) );
         ( "create_burrow - collateral in burrow representation does not \
            include creation deposit"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr ~amount:Constants.creation_deposit;
           ...
           | Some burrow ->
               assert_tez_equal ~expected:expected_collateral
                 ~real:(burrow_collateral burrow)
           | None ->
               assert_failure
                 "Expected a burrow representation to exist but none was found"
         );
         ( "create_burrow - fails when transaction amount is one mutez below \
            creation deposit"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           let amount =
             Ligo.sub_tez_tez Constants.creation_deposit
               (Ligo.tez_from_literal "1mutez")
           in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr ~amount;

           assert_raises
             (Failure (Ligo.string_of_int error_InsufficientFunds))
             (fun () ->
               Checker.entrypoint_create_burrow
                 (empty_checker, (Ligo.nat_from_literal "0n", None))) );
         ( "create_burrow - passes when transaction amount is exactly the \
            creation deposit"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
         ...
         ( "deposit_tez - owner can deposit" >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           let expected_collateral =
             Ligo.add_tez_tez deposit
               (Ligo.sub_tez_tez initial_deposit Constants.creation_deposit)
           in

           (* Create the burrow *)
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr ~amount:initial_deposit;
           let ((_, burrow_no) as burrow_id), checker =
             newly_created_burrow empty_checker "0n"
           in
           (* Make a deposit *)
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr ~amount:deposit;
           let _, checker =
             Checker.entrypoint_deposit_tez (checker, burrow_no)
           in
         ...
         ( "deposit_tez - non-owner cannot deposit" >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "3_000_000mutez");
           ...
           assert_raises
             ...
             (fun () ->
               Checker.entrypoint_deposit_tez
                 (checker, Ligo.nat_from_literal "0n")) );
         ( "withdraw_tez - owner can withdraw" >:: fun _ ->
           Ligo.Tezos.reset ();
           let initial_deposit = Ligo.tez_from_literal "3_000_000mutez" in
           let withdrawal = Ligo.tez_from_literal "1_000_000mutez" in
           let expected_collateral =
             Ligo.sub_tez_tez initial_deposit
               (Ligo.add_tez_tez Constants.creation_deposit withdrawal)
           in
           ...
           let _, checker =
             Checker.entrypoint_withdraw_tez
               (checker, (withdrawal, Ligo.nat_from_literal "0n"))
           in
         ...
         ( "withdraw_tez - transaction with value > 0 fails" >:: fun _ ->
           Ligo.Tezos.reset ();
           let initial_deposit = Ligo.tez_from_literal "3_000_000mutez" in
           let withdrawal = Ligo.tez_from_literal "1_000_000mutez" in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "42mutez");
           assert_raises
             (Failure (Ligo.string_of_int error_UnwantedTezGiven))
             (fun () ->
               Checker.entrypoint_withdraw_tez
                 (checker, (withdrawal, Ligo.nat_from_literal "0n"))) );
         ( "withdraw_tez - non-owner cannot withdraw" >:: fun _ ->
           Ligo.Tezos.reset ();
           let initial_deposit = Ligo.tez_from_literal "3_000_000mutez" in
           let withdrawal = Ligo.tez_from_literal "1_000_000mutez" in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
         ...
         >:: fun _ ->
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:time_delta
             ~blocks_passed:1 ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");

           let actual_reward =
             kit_to_mukit_int (Checker.calculate_touch_reward last_touched)
           in
         ...
         >:: fun _ ->
           ...
           let last_touched = Ligo.timestamp_from_seconds_literal 0 in
           Ligo.Tezos.new_transaction ~seconds_passed:time_delta
             ~blocks_passed:2 ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           ...
           assert_int_equal ~expected:expected_reward ~real:actual_reward );
         ( "burn_kit - transaction with value > 0 fails" >:: fun _ ->
           Ligo.Tezos.reset ();
           (* Create a burrow *)
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
           ...
           assert_raises
             ...
             (fun () ->
               ...
               Checker.entrypoint_burn_kit
                 (checker, (Ligo.nat_from_literal "0n", some_kit))) );
         ( "burn_kit - owner can burn" >:: fun _ ->
           Ligo.Tezos.reset ();

           let sender = alice_addr in

           (* Create a burrow *)
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
           ...
           let kit_token =
             kit_of_mukit
               (Fa2Interface.get_fa2_ledger_value checker.fa2_state.ledger
                  (Fa2Interface.kit_token_id, sender))
           in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _ =
             Checker.entrypoint_burn_kit
               (checker, (Ligo.nat_from_literal "0n", kit_token))
           in

           () );
         ( "burn_kit - non-owner cannot burn" >:: fun _ ->
           Ligo.Tezos.reset ();
           (* Create a burrow *)
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
           ...
           assert_raises
             ...
             (fun () ->
               let kit_token =
                 kit_of_mukit
                   (Fa2Interface.get_fa2_ledger_value checker.fa2_state.ledger
                      (Fa2Interface.kit_token_id, bob_addr))
               in
               Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
                 ~sender:bob_addr
                 ~amount:(Ligo.tez_from_literal "0mutez");
         ...
         ( Ligo.Tezos.reset ();
           ...
           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_buy_kit_respects_min_kit_expected"
                ~count:property_test_count make_inputs_for_buy_kit_to_succeed
           @@ fun (cfmm, ctez_amount, min_kit_expected, deadline) ->
           let sender = alice_addr in
           let checker = empty_checker_with_cfmm cfmm in

           let senders_old_mukit =
             Fa2Interface.get_fa2_ledger_value checker.fa2_state.ledger
               (Fa2Interface.kit_token_id, sender)
           in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
             ~amount:(Ligo.tez_from_literal "0mutez");
           let ops, checker =
             Checker.entrypoint_buy_kit
               (checker, (ctez_amount, min_kit_expected, deadline))
           in

           let senders_new_mukit =
             Fa2Interface.get_fa2_ledger_value checker.fa2_state.ledger
               (Fa2Interface.kit_token_id, sender)
           in
           ...
           | [ Transaction (FA12TransferTransactionValue transfer, _, _) ] ->
               ...
               assert_address_equal ~expected:checker_address
                 ~real:transfer.address_to;
               assert_nat_equal
                 ~expected:(ctez_to_muctez_nat ctez_amount)
                 ~real:transfer.value
           | _ ->
               failwith
                 ( "Expected [Transaction (FA12TransferTransactionValue _, _, \
                    _)] but got " ^ show_operation_list ops ) );

           Ligo.geq_nat_nat senders_new_mukit
             (Ligo.add_nat_nat senders_old_mukit
                (kit_to_mukit_nat min_kit_expected)) );
         ( Ligo.Tezos.reset ();

           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_buy_kit_preserves_kit"
                ~count:property_test_count make_inputs_for_buy_kit_to_succeed
           @@ fun (cfmm, ctez_amount, min_kit_expected, deadline) ->
           let checker = empty_checker_with_cfmm cfmm in
           let sender = alice_addr in
         ...
         ( Ligo.Tezos.reset ();
           ...
           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_buy_kit_preserves_tez"
                ~count:property_test_count make_inputs_for_buy_kit_to_succeed
           @@ fun (cfmm, ctez_amount, min_kit_expected, deadline) ->
           let checker = empty_checker_with_cfmm cfmm in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
         ...
         ( Ligo.Tezos.reset ();
           ...
           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_sell_kit_respects_min_tez_expected"
                ~count:property_test_count make_inputs_for_sell_kit_to_succeed
               ...
               parameters =
                 {
                   ...
                   circulating_kit =
                     kit_add checker.parameters.circulating_kit kit_amount;
                 };
               fa2_state =
                 ledger_issue_kit (checker.fa2_state, sender, kit_amount);
             }
           in
           Checker.assert_checker_invariants checker;
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
             ~amount:(Ligo.tez_from_literal "0mutez");
           let ops, _ =
             Checker.entrypoint_sell_kit
               (checker, (kit_amount, min_ctez_expected, deadline))
           in
           let bought_muctez =
             match ops with
             | [ Transaction (FA12TransferTransactionValue transfer, _, _) ] ->
                 assert_address_equal ~expected:checker_address
                   ~real:transfer.address_from;
                 assert_address_equal ~expected:sender ~real:transfer.address_to;
                 transfer.value
             | _ ->
                 failwith
                   ( "Expected [Transaction (FA12TransferTransactionValue _, \
                      _, _)] but got " ^ show_operation_list ops )
           in
           ctez_of_muctez bought_muctez >= min_ctez_expected );
         ( Ligo.Tezos.reset ();

           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_sell_kit_preserves_kit"
                ~count:property_test_count make_inputs_for_sell_kit_to_succeed
           @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
           let sender = alice_addr in
           let checker =
         ...
         ( Ligo.Tezos.reset ();
           ...
           qcheck_to_ounit
           @@ QCheck.Test.make ~name:"test_sell_kit_preserves_tez"
                ~count:property_test_count make_inputs_for_sell_kit_to_succeed
           @@ fun (cfmm, kit_amount, min_ctez_expected, deadline) ->
           let sender = alice_addr in
           let checker =
             ...
             | _ ->
                 failwith
                   ( "Expected [Transaction (FA12TransferTransactionValue _, \
                      _, _)] but got " ^ show_operation_list ops )
           in
           ctez_add new_checker.cfmm.ctez (ctez_of_muctez bought_muctez)
           = checker.cfmm.ctez );
         ( "set_burrow_delegate - transaction with value > 0 fails" >:: fun _ ->
           (* Create a burrow *)
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
           ...
           assert_raises
             (Failure (Ligo.string_of_int error_UnwantedTezGiven))
             (fun () ->
               Checker.entrypoint_set_burrow_delegate
                 (checker, (Ligo.nat_from_literal "0n", None))) );
         (let cfmm_kit = Ligo.nat_from_literal "1_000n" in
          ...
          let arb_kit =
            QCheck.map
              (fun x ->
                kit_of_mukit (Ligo.nat_from_literal (string_of_int x ^ "n")))
              QCheck.(1 -- max_buyable_kit)
          in
          ...
          @@ QCheck.Test.make
               ~name:
                 "buy_kit - returns geq min_kit_expected kit for transactions \
                  with sufficient tez"
          ...
          let ratio_minimum_tez =
            div_ratio (ratio_of_nat cfmm_kit)
              (sub_ratio
                 (div_ratio
                    (ratio_of_nat (Ligo.nat_from_literal "998n"))
                    (ratio_of_nat (kit_to_mukit_nat min_expected_kit)))
                 (ratio_of_nat (Ligo.nat_from_literal "1n")))
          in
          let minimum_tez =
            Ligo.mul_nat_tez
              (Ligo.abs
                 (Common.cdiv_int_int ratio_minimum_tez.num
                    ratio_minimum_tez.den))
              (Ligo.tez_from_literal "1mutez")
          in
          ...
          | [ Transaction (FA12TransferTransactionValue transfer, _, _) ] ->
              ...
              assert_address_equal ~expected:checker_address
                ~real:transfer.address_to;
              assert_nat_equal
                ~expected:(Ligo.abs (Common.tez_to_mutez tez_provided))
                ~real:transfer.value
          | _ ->
              failwith
                ( "Expected [Transaction (FA12TransferTransactionValue _, _, \
          ...
          Ligo.geq_nat_nat senders_new_mukit
            (Ligo.add_nat_nat senders_old_mukit
               (kit_to_mukit_nat min_expected_kit))
           ...
           *));
         ( "buy_kit - returns expected kit" >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           () );
         ( "sell_kit - returns expected tez" >:: fun _ ->
           Ligo.Tezos.reset ();

           let kit_to_sell =
             kit_of_mukit (Ligo.nat_from_literal "1_000_000n")
           in
           let min_ctez_expected =
               ...
               parameters =
                 {
                   checker.parameters with
                   circulating_kit =
                     kit_add checker.parameters.circulating_kit kit_to_sell;
                 };
               fa2_state =
                 ledger_issue_kit (checker.fa2_state, alice_addr, kit_to_sell);
             }
           in
           Checker.assert_checker_invariants checker;
         ...
         ( "sell_kit - transaction with value > 0 fails" >:: fun _ ->
           Ligo.Tezos.reset ();
               ...
               Checker.entrypoint_sell_kit
                 ( empty_checker,
                   ( kit_to_sell,
                     min_ctez_expected,
                     Ligo.timestamp_from_seconds_literal 1 ) )) );
         ( "remove_liquidity - returns expected kit and tez" >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           let checker =
             {
               ...
               fa2_state =
                 (let fa2_state = initial_fa2_state in
                  let fa2_state =
                    ledger_issue_lqt (fa2_state, sender, my_liquidity_tokens)
                  in
                  let fa2_state =
                    ledger_issue_kit
                      ( fa2_state,
                        !Ligo.Tezos.self_address,
                        kit_of_mukit (Ligo.nat_from_literal "1n") )
                  in
                  fa2_state);
             }
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
             ~amount:(Ligo.tez_from_literal "0mutez");
           let ops, checker =
             Checker.entrypoint_remove_liquidity
               ( checker,
                 ( my_liquidity_tokens,
                   min_ctez_expected,
                   min_kit_expected,
           ...
           let ctez =
             ...
             | _ ->
                 failwith
                   ( "Expected [Transaction (FA12TransferTransactionValue _, \
                      _, _); Transaction (KitTransactionValue _, _, _)] but \
                      got " ^ show_operation_list ops )
           in
           ...
           () );
         ( "remove_liquidity - transaction with value > 0 fails" >:: fun _ ->
           Ligo.Tezos.reset ();
           let min_kit_expected = kit_of_mukit (Ligo.nat_from_literal "1n") in
           let min_ctez_expected =
             ctez_of_muctez (Ligo.nat_from_literal "1n")
         ...
         ( "strict_entrypoint_balance_of (FA2) - transaction with value > 0 \
            fails"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
           ...
           let fa2_balance_of_param =
             {
               requests = [];
               callback =
                 Ligo.contract_of_address
                   (Ligo.address_of_string "test address");
             }
           in
           assert_raises
             (Failure (Ligo.string_of_int error_UnwantedTezGiven))
             (fun () ->
               Checker.strict_entrypoint_balance_of
                 (empty_checker, fa2_balance_of_param)) );
         ...
         ( "fa2 scenario" >:: fun _ ->
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:10 ~blocks_passed:1
             ~sender:initial_addr
             ~amount:(Ligo.tez_from_literal "100_000_000mutez");
           let _, checker =
             Checker.entrypoint_create_burrow
               (checker, (Ligo.nat_from_literal "0n", None))
           in
           let max_kit =
             Checker.view_burrow_max_mintable_kit
               ((initial_addr, Ligo.nat_from_literal "0n"), checker)
           in
           ...
           let _, checker =
             Checker.entrypoint_mint_kit
               (checker, (Ligo.nat_from_literal "0n", max_kit))
           in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:10 ~blocks_passed:1
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _, checker =
             Checker.strict_entrypoint_transfer
               ( checker,
                 [
                   {
                     from_ = initial_addr;
                       ...
                       [
                         {
                           ...
                           amount = Ligo.nat_from_literal "5n";
                         };
                         {
                           to_ = bob_addr;
           ...
           let balance chk addr tok =
             Checker.view_get_balance ((addr, tok), chk)
           in

           (* you can see the initial balances here for reference *)
           assert_nat_equal
             ~real:(balance checker alice_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "5n");
           assert_nat_equal
             ~real:(balance checker alice_addr lqt_token_id)
             ~expected:(Ligo.nat_from_literal "0n");

           assert_nat_equal
             ~real:(balance checker bob_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "0n");
           assert_nat_equal
             ~real:(balance checker bob_addr lqt_token_id)
             ~expected:(Ligo.nat_from_literal "5n");

           assert_nat_equal
             ~real:(balance checker leena_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "0n");
           assert_nat_equal
             ~real:(balance checker leena_addr lqt_token_id)
             ~expected:(Ligo.nat_from_literal "0n");
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:10 ~blocks_passed:1
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _, checker =
             Checker.entrypoint_update_operators
               ( checker,
                 [
                   Add_operator
                     {
           ...
           assert_equal true
             (Checker.view_is_operator
                ((bob_addr, (leena_addr, kit_token_id)), checker));
           assert_equal false
             (Checker.view_is_operator
                ((bob_addr, (leena_addr, lqt_token_id)), checker));
           assert_equal false
             (Checker.view_is_operator
                ((leena_addr, (bob_addr, kit_token_id)), checker));
                       ...
                       [
                         {
                           to_ = bob_addr;
                           token_id = kit_token_id;
                           amount = Ligo.nat_from_literal "2n";
                         };
                       ];
                   };
                 ] )
           ...
           assert_nat_equal
             ~real:(balance checker alice_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "3n");
           assert_nat_equal
             ~real:(balance checker bob_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "2n");

           (* but she can not transfer more than she has *)
           assert_raises (Failure "FA2_INSUFFICIENT_BALANCE") (fun () ->
               Checker.strict_entrypoint_transfer
                 ( checker,
                   [
                     {
                         ...
                         [
                           {
                             to_ = bob_addr;
                             token_id = kit_token_id;
                             amount = Ligo.nat_from_literal "10n";
                           };
                         ];
                     };
                   ] ));
           ...
           assert_nat_equal
             ~real:(balance checker alice_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "4n");
           assert_nat_equal
             ~real:(balance checker bob_addr kit_token_id)
             ~expected:(Ligo.nat_from_literal "1n");
           ...
           assert_raises (Failure "FA2_NOT_OPERATOR") (fun () ->
               Checker.strict_entrypoint_transfer
                 ( checker,
                   [
                     {
           ...
           () );
         ( "view_total_supply (FA2) - initial kit supply" >:: fun _ ->
           Ligo.Tezos.reset ();
           let total_mukit_amount =
             Checker.view_total_supply (Fa2Interface.kit_token_id, empty_checker)
           in
           assert_nat_equal
             ~expected:(Ligo.nat_from_literal "0n")
             ~real:total_mukit_amount;
           () );
         ( "view_total_supply (FA2) - initial lqt supply" >:: fun _ ->
           Ligo.Tezos.reset ();
           let total_mulqt_amount =
             Checker.view_total_supply (Fa2Interface.lqt_token_id, empty_checker)
           in
           assert_nat_equal
             ~expected:(Ligo.nat_from_literal "0n")
             ~real:total_mulqt_amount;
           () );
         ( "view_total_supply (FA2) - undefined token id" >:: fun _ ->
           assert_raises (Failure "FA2_TOKEN_UNDEFINED") (fun () ->
               Checker.view_total_supply
                 (Ligo.nat_from_literal "3n", empty_checker)) );
         ( "view_all_tokens (FA2)" >:: fun _ ->
           Ligo.Tezos.reset ();
         ...
         (*                       LiquidationAuctions                                 *)
         (* ************************************************************************* *)
         ( "entrypoint_liquidation_auction_place_bid: should only allow the \
            current auction"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           let checker =
             {
               empty_checker with
               last_price = Some (Ligo.nat_from_literal "1_000_000n");
             }
           in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:10 ~blocks_passed:1
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "200_000_000mutez");
           let _, checker =
             Checker.entrypoint_create_burrow
               (checker, (Ligo.nat_from_literal "0n", None))
           in
           let max_kit =
             Checker.view_burrow_max_mintable_kit
               ((alice_addr, Ligo.nat_from_literal "0n"), checker)
           in
           ...
           let checker =
             {
               checker with
               last_price = Some (Ligo.nat_from_literal "10_000_000n");
             }
           in
           let _, checker = Checker.entrypoint_touch (checker, ()) in

           Ligo.Tezos.new_transaction ~seconds_passed:1_000_000 ~blocks_passed:1
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _, checker = Checker.entrypoint_touch (checker, ()) in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:10 ~blocks_passed:1
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _, checker =
             Checker.entrypoint_touch_burrow
               (checker, (alice_addr, Ligo.nat_from_literal "0n"))
           in
           let _, checker =
             Checker.entrypoint_mark_for_liquidation
               (checker, (alice_addr, Ligo.nat_from_literal "0n"))
           in
           let _, checker = Checker.entrypoint_touch (checker, ()) in

           let res =
             Checker.view_current_liquidation_auction_minimum_bid ((), checker)
           in
           let other_ptr =
             match res.auction_id with AVLPtr i -> Ptr.ptr_next i
           in
           ...
           assert_raises
             ...
             (fun () ->
               Checker.entrypoint_liquidation_auction_place_bid
                 (checker, (AVLPtr other_ptr, res.minimum_bid))) );
         ( "can complete a liquidation auction" >:: fun _ ->
           Ligo.Tezos.reset ();
           let checker = empty_checker in
           ...
           let _, checker =
             Checker.entrypoint_mint_kit
               ( checker,
                 ( Ligo.nat_from_literal "0n",
                   kit_of_mukit (Ligo.nat_from_literal "10_000_000n") ) )
           in
           ...
           let () =
             ...
             let () =
               ...
               | [ CreateContract (_, _, sent_tez, _) ] ->
                   assert_tez_equal ~expected:tez ~real:sent_tez
               | _ ->
                   assert_failure
                     ( "Expected [CreateContract (_, _, _, _)] but got "
                     ^ show_operation_list ops )
             in

             let burrow_addr =
               burrow_address
                 (Option.get
                    (Ligo.Big_map.find_opt
                       (bob_addr, Ligo.nat_from_literal "0n")
                       checker0.burrows))
             in
             Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
               ~sender:bob_addr
               ~amount:(Ligo.tez_from_literal "0mutez");
             let ops, checker1 =
               Checker.entrypoint_deactivate_burrow
                 (checker0, (Ligo.nat_from_literal "0n", alice_addr))
             in
             assert_operation_list_equal
               ~expected:
                 [
                   LigoOp.Tezos.tez_address_transaction (tez, alice_addr)
                     (Ligo.tez_from_literal "0mutez")
                     (Option.get
                        (LigoOp.Tezos.get_entrypoint_opt "%burrowSendTezTo"
                           burrow_addr));
                 ]
               ~real:ops;
             ...
             Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
               ~sender:bob_addr ~amount:tez;
             let _ops, checker2 =
               Checker.entrypoint_activate_burrow
                 (checker1, Ligo.nat_from_literal "0n")
             in
           ...
           assert_nat_equal
             ~expected:(Ligo.nat_from_literal "4_285_714n")
             ~real:kit;

           assert_bool "should not be overburrowed right after minting"
             ( not
             @@ burrow_is_overburrowed checker.parameters
           ...
           let _ops, checker =
             Checker.touch_with_index checker
               (Ligo.nat_from_literal "1_000_001n")
           in

           let ops, checker =
             Checker.entrypoint_touch_burrow (checker, burrow_id)
           in
           assert_operation_list_equal ~expected:[] ~real:ops;
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:(211 * 60)
             ~blocks_passed:211 ~sender:bob_addr
             ~amount:(Ligo.tez_from_literal "0mutez");

           let kit_before_reward =
             get_balance_of checker bob_addr kit_token_id
           in
           let _, checker =
             Checker.touch_with_index checker
               (Ligo.nat_from_literal "1_200_000n")
           in
           let kit_after_reward =
             get_balance_of checker bob_addr kit_token_id
           in

           let touch_reward =
             Ligo.sub_nat_nat kit_after_reward kit_before_reward
           in
           ...
           assert_int_equal
             ~expected:
               (Ligo.int_from_literal "202_000_000")
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let ops, checker =
             Checker.entrypoint_mark_for_liquidation (checker, burrow_id)
           in
           ...
           let slice =
             Ligo.Big_map.find_opt burrow_id
               checker.liquidation_auctions.burrow_slices
             |> Option.get
             |> fun i -> i.youngest_slice
           in
           ...
           assert_raises
             ...
             (fun () ->
               Checker.entrypoint_cancel_liquidation_slice (checker, slice));
           ...
           assert_raises
             ...
             (fun () ->
               let undefined_slice =
                 LiquidationAuctionPrimitiveTypes.LeafPtr
                   (ptr_next checker.liquidation_auctions.avl_storage.last_ptr)
               in
               Checker.entrypoint_cancel_liquidation_slice
                 (checker, undefined_slice));

           Ligo.Tezos.new_transaction ~seconds_passed:(5 * 60) ~blocks_passed:5
             ~sender:bob_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           assert_raises
           ...
           let kit_before_reward =
             get_balance_of checker alice_addr kit_token_id
           in
           let _, checker =
             Checker.touch_with_index checker
               (Ligo.nat_from_literal "1_200_000n")
           ...
           in
           let auction_id =
             (Checker.view_current_liquidation_auction_minimum_bid
                ((), checker))
               .auction_id
           in
           ...
           let auction_id =
             ...
             | None ->
                 assert_failure
                   "entrypoint_liquidation_auction_place_bid should have \
                    succeeded"
             | Some current_auction -> current_auction.contents
           in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:(30 * 60)
             ~blocks_passed:30 ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           ...
           | _ ->
               assert_failure
                 ( "Unexpected operations/operation order: "
                 ^ show_operation_list ops ) );
           ...
           assert_raises
             (Failure (Ligo.string_of_int error_InvalidLeafPtr))
             (fun () ->
               Checker.entrypoint_touch_liquidation_slices (checker, [ slice ]));
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let ops, checker =
             Checker.entrypoint_liquidation_auction_claim_win
               (checker, auction_id)
           in
             ...
             ~expected:
               [
                 LigoOp.Tezos.unit_transaction ()
                   (Ligo.tez_from_literal "3_156_446mutez")
                   (Option.get (LigoOp.Tezos.get_contract_opt alice_addr));
               ]
             ~real:ops;
         ...
         >:: fun _ ->
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0 ~sender
             ~amount:(Ligo.tez_from_literal "2_001_001mutez");
           let ((_, burrow_no) as burrow_id), checker =
             newly_created_burrow checker "0n"
           in
           ...
           *)
           ...
           let _ops, checker =
             Checker.entrypoint_mint_kit
               ( checker,
                 (burrow_no, kit_of_mukit (Ligo.nat_from_literal "476_667n")) )
           in
           ...
           let blocks_passed = 211 in
           (* NOTE: I am a little surprised/worried about this being again 211... *)
           Ligo.Tezos.new_transaction ~seconds_passed:(60 * blocks_passed)
             ~blocks_passed ~sender:bob_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _ops, checker =
             Checker.touch_with_index checker
               (Ligo.nat_from_literal "1_105_283n")
           in
           (* sup *)
           let _ops, checker =
             Checker.entrypoint_touch_burrow (checker, burrow_id)
           ...
           | Some burrow ->
               assert_bool
                 "burrow needs to be liquidatable for the test to be potent."
                 (Burrow.burrow_is_liquidatable checker.parameters burrow) );
           ...
           (* Ensures no empty slices in the queue. *)
           () );
         ( "deposit_tez - does not fail on untouched burrows" >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr ~amount;
           let _ =
             Checker.entrypoint_deposit_tez (checker, Ligo.nat_from_literal "0n")
           in
           () );
         ...
         >:: fun _ ->
           ...
           let _ =
             Checker.entrypoint_withdraw_tez
               ( checker,
                 (Constants.creation_deposit, Ligo.nat_from_literal "0n") )
           in
           () );
         ...
         >:: fun _ ->
           ...
           let _ops, checker =
             Checker.entrypoint_deactivate_burrow
               (checker, (Ligo.nat_from_literal "0n", !Ligo.Tezos.sender))
           in
           (* Touch checker *)
           Ligo.Tezos.new_transaction ~seconds_passed:1 ~blocks_passed:1
             ~sender:alice_addr
         ...
         let with_cfmm_setup f _ =
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "10_000_000mutez");
           let _ops, checker =
             Checker.entrypoint_create_burrow (checker, (burrow_id, None))
           in
           (* Mint some kit *)
           Ligo.Tezos.new_transaction ~seconds_passed:62 ~blocks_passed:1
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let _ops, checker =
             Checker.entrypoint_mint_kit (checker, (burrow_id, kit_one))
           in
           (* Add some liquidity *)
           ...
           in
           let kit_to_give =
             Kit.kit_of_mukit (Ligo.nat_from_literal "400_000n")
           in
           let min_lqt_to_mint =
           ...
           let _ops, checker =
             Checker.entrypoint_add_liquidity
               (checker, (ctez_to_give, kit_to_give, min_lqt_to_mint, deadline))
           in
         ...
         [
           ...
           >:: with_cfmm_setup (fun checker ->
                   let ctez_to_sell =
                     Ctez.ctez_of_muctez (Ligo.nat_from_literal "100_000n")
                   in
                   let min_kit_to_buy =
                     Checker.view_buy_kit_min_kit_expected
                       (ctez_to_sell, checker)
                   in
                   let deadline =
                   ...
                   Checker.entrypoint_buy_kit
                     (checker, (ctez_to_sell, min_kit_to_buy, deadline)));
           ...
           >:: with_cfmm_setup (fun checker ->
                   ...
                   in
                   let min_ctez_to_buy =
                     Checker.view_sell_kit_min_ctez_expected
                       (kit_to_sell, checker)
                   in
                   let deadline =
                     Ligo.add_timestamp_int !Ligo.Tezos.now
                   ...
                   Checker.entrypoint_sell_kit
                     (checker, (kit_to_sell, min_ctez_to_buy, deadline)));
           ...
           >:: with_cfmm_setup (fun checker ->
                   assert_raises
                     ...
                     (fun () ->
                       Checker.view_sell_kit_min_ctez_expected
                         (Kit.kit_zero, checker)));
           "view_add_liquidity_max_kit_deposited / \
            view_add_liquidity_min_lqt_minted"
           >:: with_cfmm_setup (fun checker ->
                   let ctez_to_sell =
                     Ctez.ctez_of_muctez (Ligo.nat_from_literal "100_000n")
                   in
                   let max_kit_to_sell =
                     Checker.view_add_liquidity_max_kit_deposited
                       (ctez_to_sell, checker)
                   in
                   let min_lqt_to_buy =
                     Checker.view_add_liquidity_min_lqt_minted
                       (ctez_to_sell, checker)
                   in
                   let deadline =
                     Ligo.add_timestamp_int !Ligo.Tezos.now
                   ...
                   Checker.entrypoint_add_liquidity
                     ( checker,
                       (ctez_to_sell, max_kit_to_sell, min_lqt_to_buy, deadline)
                     ));
           ...
           >:: with_cfmm_setup (fun checker ->
                   assert_raises
                     ...
                     (fun () ->
                       Checker.view_add_liquidity_min_lqt_minted
                         (Ctez.ctez_zero, checker)));
           "view_remove_liquidity_min_ctez_withdrawn / \
            view_remove_liquidity_min_kit_withdrawn"
           >:: with_cfmm_setup (fun checker ->
                   let lqt_to_sell =
                     Lqt.lqt_of_denomination (Ligo.nat_from_literal "5n")
                   in
                   let min_ctez_to_buy =
                     Checker.view_remove_liquidity_min_ctez_withdrawn
                       (lqt_to_sell, checker)
                   in
                   let min_kit_to_buy =
                     Checker.view_remove_liquidity_min_kit_withdrawn
                       (lqt_to_sell, checker)
                   in
                   let deadline =
                     Ligo.add_timestamp_int !Ligo.Tezos.now
                   ...
                   Checker.entrypoint_remove_liquidity
                     ( checker,
                       (lqt_to_sell, min_ctez_to_buy, min_kit_to_buy, deadline)
                     ));
           "view_remove_liquidity_min_ctez_withdrawn - fail if no liquidity is \
            given"
           >:: with_cfmm_setup (fun checker ->
                   assert_raises
                     (Failure
                        (Ligo.string_of_int
                           error_RemoveLiquidityNoLiquidityBurned))
                     (fun () ->
                       Checker.view_remove_liquidity_min_ctez_withdrawn
                         (Lqt.lqt_zero, checker)));
           "view_remove_liquidity_min_ctez_withdrawn - too much lqt withdrawn \
            (equal)"
           >:: with_cfmm_setup (fun checker ->
                   ...
                   assert_raises
                     (Failure
                        (Ligo.string_of_int
                           error_RemoveLiquidityTooMuchLiquidityWithdrawn))
                     (fun () ->
                       Checker.view_remove_liquidity_min_ctez_withdrawn
                         (lqt_to_withdraw, checker)));
           "view_remove_liquidity_min_ctez_withdrawn - too much lqt withdrawn \
            (more than)"
           >:: with_cfmm_setup (fun checker ->
                   let lqt_to_withdraw =
                     Lqt.lqt_add checker.cfmm.lqt
                       (Lqt.lqt_of_denomination (Ligo.nat_from_literal "1n"))
                   in
                   assert_raises
                     (Failure
                     ...
                     (fun () ->
                       Checker.view_remove_liquidity_min_ctez_withdrawn
                         (lqt_to_withdraw, checker)));
           "view_remove_liquidity_min_kit_withdrawn - fail if no liquidity is \
            given"
           >:: with_cfmm_setup (fun checker ->
                   assert_raises
                     (Failure
                     ...
                     (fun () ->
                       Checker.view_remove_liquidity_min_kit_withdrawn
                         (Lqt.lqt_zero, checker)));
           "view_remove_liquidity_min_kit_withdrawn - too much lqt withdrawn \
            (equal)"
           >:: with_cfmm_setup (fun checker ->
                   let lqt_to_withdraw = checker.cfmm.lqt in
                   assert_raises
