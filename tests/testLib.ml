...
let cfmm_token_fa12_addr = Ligo.address_of_string "cfmm_token_fa12_addr"
...
let assert_ctok_equal ~expected ~real = OUnit2.assert_equal ~printer:Ctok.show_ctok expected real
...
let eq_cfmm (u1: CfmmTypes.cfmm) (u2: CfmmTypes.cfmm) : bool =
  Ctok.ctok_compare u1.ctok u2.ctok = 0
  && Kit.kit_compare u1.kit u2.kit = 0
  && Lqt.lqt_compare u1.lqt u2.lqt = 0
  && Ratio.eq_ratio_ratio u1.kit_in_ctok_in_prev_block u2.kit_in_ctok_in_prev_block
...
let cfmm_make_for_test ~ctok ~kit ~lqt ~kit_in_ctok_in_prev_block ~last_level =
  { CfmmTypes.ctok = ctok;
    CfmmTypes.kit = kit;
    CfmmTypes.lqt = lqt;
    CfmmTypes.kit_in_ctok_in_prev_block = kit_in_ctok_in_prev_block;
    CfmmTypes.last_level = last_level;
  }

(* Create an arbitrary state for the cfmm contract (NB: some values are fixed). *)
let arbitrary_non_empty_cfmm (kit_in_ctok_in_prev_block: Common.ratio) (last_level: Ligo.nat) =
  QCheck.map
    (fun (ctok, kit, lqt) ->
       (ctok, kit, lqt, cfmm_make_for_test ~ctok ~kit ~lqt ~kit_in_ctok_in_prev_block ~last_level)
    )
    (QCheck.triple TestArbitrary.arb_positive_ctok TestArbitrary.arb_positive_kit TestArbitrary.arb_lqt)
...
(* 1mutez <= min_ctok_expected < FLOOR{kit * (cfmm_tez / (cfmm_kit + kit)) * FACTOR} *)
(* NB: some values are fixed *)
