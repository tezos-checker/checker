...
let nTez (i : int) : Ligo.tez =
  Ligo.tez_from_literal (string_of_int (1_000_000 * i) ^ "mutez")

let add_all (mem : mem) (root : avl_ptr) (xs : liquidation_slice list) : mem =
  List.fold_left
    (fun mem value ->
...
let debug_avl (mem : mem) (AVLPtr root) : unit =
  let rec go curr =
    ...
    | Root (Some r, r') ->
        "Root(" ^ show_auction_outcome_option r' ^ ")\n" ^ indent (go r)
    | Leaf leaf ->
        sprintf "Leaf { value: %s; parent: %s }"
          (show_liquidation_slice leaf.value)
          (show_ptr leaf.parent)
    | Branch branch ->
...
let avl_to_list (mem : mem) (AVLPtr ptr) : liquidation_slice list =
  let rec go ptr : liquidation_slice list =
    ...
    | Branch branch -> List.append (go branch.left) (go branch.right)
  in
  go ptr

let avl_from_list (mem : mem) (root_data : auction_outcome option)
    (elements : liquidation_slice list)
    : mem * avl_ptr
  =
    ...
    contents =
      {
        ...
        burrow =
          (Ligo.address_of_string "someburrow", Ligo.nat_from_literal "0n");
      };
    older = None;
    younger = None;
  }

let rec range (f : int) (t : int) = if f >= t then [] else f :: range (f + 1) t
...
let suite =
  "AVLTests"
  >::: [
         ( "test_push_back_singleton" >:: fun _ ->
           ...
           assert_liquidation_slice_list_equal ~expected ~real:actual );
         ( "test_push_front_singleton" >:: fun _ ->
           let mem, root = avl_mk_empty mem_empty None in
           let mem, _ = avl_push mem root (mk_liquidation_slice 0) Right in
           let actual = avl_to_list mem root in
           let expected = [ mk_liquidation_slice 0 ] in
           assert_liquidation_slice_list_equal ~expected ~real:actual );
         ( "test_from_list" >:: fun _ ->
           let elements =
             List.map mk_liquidation_slice [ 1; 2; 3; 4; 5; 6; 7; 8 ]
           in
         ...
         ( "test_pop_front_empty" >:: fun _ ->
           let mem, root = avl_from_list mem_empty None [] in
           let mem, x = avl_pop_front mem root in
           assert_liquidation_slice_list_equal ~expected:[]
             ~real:(avl_to_list mem root);
           assert_slice_option_equal ~expected:None ~real:(Option.map snd x) );
         ( "test_pop_front" >:: fun _ ->
           let elements = [ mk_liquidation_slice 1; mk_liquidation_slice 2 ] in
           let mem, root = avl_from_list mem_empty None elements in
           let mem, x = avl_pop_front mem root in
           assert_liquidation_slice_list_equal
             ~expected:[ mk_liquidation_slice 2 ] ~real:(avl_to_list mem root);
           assert_slice_option_equal
             ~expected:(Some (mk_liquidation_slice 1))
             ~real:(Option.map snd x);
           let mem, x = avl_pop_front mem root in
           assert_liquidation_slice_list_equal ~expected:[]
             ~real:(avl_to_list mem root);
           assert_slice_option_equal
             ~expected:(Some (mk_liquidation_slice 2))
             ~real:(Option.map snd x) );
         ( "test_del_singleton" >:: fun _ ->
           Mem.reset_ops ();
           let mem, root = avl_mk_empty mem_empty None in
           ...
           assert_liquidation_slice_list_equal ~expected:[]
             ~real:(avl_to_list mem root);
           assert_stdlib_int_equal ~expected:1
             ~real:(List.length (Mem.mem_bindings mem)) );
         ( "test_del" >:: fun _ ->
           let fst_elements = List.map mk_liquidation_slice [ 1; 2; 3; 4; 5 ] in
           let mid = mk_liquidation_slice 6 in
           let snd_elements = List.map mk_liquidation_slice [ 7; 8; 9 ] in
           ...
           assert_liquidation_slice_list_equal ~expected ~real:actual );
         ( "test_empty_from_list_to_list" >:: fun _ ->
           let elements = [] in
           let mem, root = avl_from_list mem_empty None elements in
           let actual = avl_to_list mem root in
           let expected = [] in
           assert_liquidation_slice_list_equal ~expected ~real:actual );
         ( qcheck_to_ounit
         @@ QCheck.Test.make ~name:"prop_from_list_to_list"
              ~count:property_test_count
              (QCheck.list TestArbitrary.arb_liquidation_slice)
         @@ fun xs ->
         ...
         @@ QCheck.Test.make ~name:"prop_del" ~count:property_test_count
              QCheck.(
                triple
                  (list TestArbitrary.arb_liquidation_slice)
                  TestArbitrary.arb_liquidation_slice
                  (list TestArbitrary.arb_liquidation_slice))
     ...
     *)
         ...
         true );
         ( qcheck_to_ounit
         @@ QCheck.Test.make ~name:"prop_append" ~count:property_test_count
              QCheck.(
                pair
                  (list TestArbitrary.arb_liquidation_slice)
                  (list TestArbitrary.arb_liquidation_slice))
         ...
         true );
         ( qcheck_to_ounit
         @@ QCheck.Test.make ~name:"prop_take" ~count:property_test_count
              QCheck.(
                pair TestArbitrary.arb_tez
                  (list TestArbitrary.arb_liquidation_slice))
         @@ fun (limit, xs) ->
         ...
         assert_liquidation_slice_list_equal ~expected:expected_left
           ~real:actual_left;
         assert_liquidation_slice_list_equal ~expected:expected_right
           ~real:actual_right;

         true );
         ( qcheck_to_ounit
         @@ QCheck.Test.make ~name:"prop_take_append" ~count:property_test_count
              QCheck.(
                pair TestArbitrary.arb_tez
                  (list TestArbitrary.arb_liquidation_slice))
         @@ fun (limit, xs) ->
         ...
         ( "bench_large_ops" >:: fun _ ->
           ...
           Mem.reset_ops ();
           let _ =
             avl_take mem root (Ligo.tez_from_literal "50_000mutez") None
           in

           assert_equal
             { reads = 104; writes = 69 }
             !Mem.ops ~printer:Mem.show_ops );
         ( "test_all_permutations" >:: fun _ ->
           range 0 9 |> permutations
           |> Stream.iter (fun xs ->
