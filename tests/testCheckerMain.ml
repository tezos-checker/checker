...
let suite =
  ...
  >::: [
         ...
         @@ QCheck.Test.make
              ...
              ~count:property_test_count TestArbitrary.arb_address
         @@ fun addr ->
         let wrapper = initial_wrapper addr in
         wrapper.deployment_state = Unsealed addr );
         ( "initial wrapper - lazy_functions bigmap is empty" >:: fun _ ->
           assert_bool "initial lazy_functions bigmap must be empty"
             ( List.length
                 (Ligo.Big_map.bindings
                    (initial_wrapper bob_addr).lazy_functions)
             = 0 ) );
         ( "initial wrapper - metadata bigmap is empty" >:: fun _ ->
           assert_bool "initial metadata bigmap must be empty"
             ( List.length
                 (Ligo.Big_map.bindings (initial_wrapper bob_addr).metadata)
             = 0 ) );
         (* Succeeding cases when checker is not sealed yet *)
         ( "If checker is not sealed, the deployer should be able to call \
            DeployFunction"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           let wrapper = CheckerMain.initial_wrapper leena_addr in

           (* unsealed *)
           let fn_id, fn_bytes =
             CheckerEntrypoints.(
               lazyParamsToLazyFunctionId
                 (Create_burrow
                    (Ligo.nat_from_literal "63n", Some charles_key_hash)))
           in
         ...
         ( "If checker is not sealed, only the deployer should be able to work \
            with it"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
         ...
         >:: fun _ ->
           ...
           (* unsealed *)
           let op =
             CheckerMain.(CheckerEntrypoint (StrictParams (Transfer [])))
           in
           assert_raises
             (Failure (Ligo.string_of_int error_ContractNotDeployed))
             (fun () -> CheckerMain.main (op, wrapper)) );
         (* Succeeding cases when checker is already sealed *)
         "If checker is sealed, users should be able to call \
          CheckerEntrypoint/StrictParams/Transfer"
         >:: with_sealed_wrapper (fun sealed_wrapper ->
                 ...
                 ());
         "If checker is sealed, users should be able to call \
          CheckerEntrypoint/LazyParams"
         >:: with_sealed_wrapper (fun sealed_wrapper ->
                 let op =
                   CheckerMain.(
                     CheckerEntrypoint
                       (LazyParams
                          (Receive_price (Ligo.nat_from_literal "756n"))))
                 in
                 (* This call should succeed *)
                 Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
                   ~sender:oracle_addr
                 ...
                 assert_raises
                   (Failure (Ligo.string_of_int error_UnauthorisedCaller))
                   (fun () ->
                     Ligo.Tezos.new_transaction ~seconds_passed:0
                       ~blocks_passed:0 ~sender:alice_addr
                       ~amount:(Ligo.tez_from_literal "0mutez");
         ...
         >:: with_sealed_wrapper (fun sealed_wrapper ->
                 let fn_id, fn_bytes =
                   CheckerEntrypoints.lazyParamsToLazyFunctionId
                     (CheckerEntrypoints.Touch ())
                 in
                 let op = CheckerMain.DeployFunction (fn_id, fn_bytes) in
                 assert_raises
         ...
         >:: with_sealed_wrapper (fun sealed_wrapper ->
                 let op =
                   CheckerMain.DeployMetadata
                     (Ligo.bytes_from_literal "0x0123456789ABCDEF")
                 in
                 assert_raises
                   (Failure (Ligo.string_of_int error_ContractAlreadyDeployed))
                   (fun () -> CheckerMain.main (op, sealed_wrapper)));
