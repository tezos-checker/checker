...
let addr_gen =
  QCheck.Gen.(map (fun x -> Ligo.address_of_string x) (string_size (return 36)))
...
let tez_gen : Ligo.tez QCheck.Gen.t =
  QCheck.Gen.(
    map
...
let kit_gen =
  QCheck.Gen.(
    map
      (fun x ->
        Kit.kit_of_mukit (Ligo.nat_from_literal (string_of_int x ^ "n")))
      (0 -- max_int))
  ...
  QCheck.Gen.(
    map
      (fun (addr, tez, kit) ->
        ...
        in
        { contents; older = None; younger = None })
...
let op_gen =
  QCheck.Gen.(
    ...
    | 5 ->
        map
          (fun tez ->
            Take (Ligo.tez_from_literal (string_of_int tez ^ "mutez")))
          (int_range 0 max_int)
    | _ ->
        failwith
          "Generated more cases than there are in queue_op. Check the \
           int_range bounds in op_gen.")
...
let show_model queue =
  ...
  let _ =
    iteri queue ~f:(fun i e ->
        Format.printf "(%d, %s);" i (show_liquidation_slice e))
  in
  let _ = Format.printf "]" in
  ()

let model_empty () =
  (Core_kernel.Deque.create () : liquidation_slice Core_kernel.Deque.t)
...
let model_delete (queue : model) (index : int) : model =
  let filter_index (index : int) (cur_index : int) acc element =
    if cur_index = index then acc
    else
      let _ = enqueue_front acc element in
      acc
  in
...
let model_take (limit : Ligo.tez) (model : model) : liquidation_slice list =
  let rec rec_model_take limit current_total current_slices current_model =
    if Ligo.geq_tez_tez current_total limit then current_slices
    else
      match peek_front current_model with
      | None -> current_slices
      | Some slice ->
          let total = Ligo.add_tez_tez current_total slice.contents.tez in
          if Ligo.gt_tez_tez total limit then current_slices
          else
            rec_model_take limit total
              (List.append current_slices [ dequeue_front_exn current_model ])
              current_model
  in
...
let rec avl_foldli (impl : Mem.mem * avl_ptr) (acc : 'b)
    (f : leaf_ptr -> 'b -> liquidation_slice -> 'b)
    : 'b
  =
  let mem, root_ptr = impl in
  let new_mem, element = Avl.avl_pop_front mem root_ptr in
  match element with
  | None -> acc
  | Some (leaf_ptr, slice) ->
      avl_foldli (new_mem, root_ptr) (f leaf_ptr acc slice) f

let rec avl_foldl (impl : Mem.mem * avl_ptr) (acc : 'b)
    (f : 'b -> liquidation_slice -> 'b)
    : 'b
  =
  let mem, root_ptr = impl in
  let new_mem, element = Avl.avl_pop_front mem root_ptr in
...
let model_indices model =
  foldi' model `front_to_back ~init:[] ~f:(fun index acc _ ->
      List.append acc [ index ])

let avl_indices impl =
  avl_foldli impl [] (fun index acc _ -> List.append acc [ index ])
...
let apply_op ((impl : Mem.mem * avl_ptr), (model : model)) op =
  ...
  | Pop ->
      ...
      let popped_impl =
        match popped with None -> None | Some (_, slice) -> Some slice
      in
  ...
  | Get p ->
      let _ =
        ...
        | _ ->
            ...
            let got_impl =
              Avl.avl_read_leaf mem (percent_to_index p impl_indices)
            in
  ...
  | Delete p -> (
      ...
      | _ ->
          (* Delete from model *)
          let new_model =
            model_delete model (percent_to_index p model_indices)
          in
  ...
  | Take tez_limit ->
      ...
      let slices_impl =
        avl_foldl (new_mem, split_ptr) [] (fun acc e -> List.append acc [ e ])
      in
      assert_liquidation_slice_list_equal ~expected:slices_model
        ~real:slices_impl;
...
let suite =
  ...
  >::: [
         ...
         @@ QCheck.Test.make
              ~name:
                "Does AVL behave like a model double-ended queue? \u{1F914}"
                (* FIXME: Increase this count once long running tests are moved to their own suite *)
              ~count:100 (QCheck.list arb_op)
         @@ fun ops ->
         ...
         let _, _ =
           List.fold_left
             (fun acc op ->
               ...
               let impl_elements, model_elements =
                 get_all_elements impl model
               in
               (* Items in implementation queue must match items in model queue. *)
               assert_liquidation_slice_list_equal ~expected:model_elements
                 ~real:impl_elements;
