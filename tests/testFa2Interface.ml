...
(* Utility functions *)
let ask_kit_of fa2_state addr =
  kit_of_mukit (fa2_get_balance (fa2_state, addr, kit_token_id))

let ask_lqt_of fa2_state addr =
  lqt_of_denomination (fa2_get_balance (fa2_state, addr, lqt_token_id))

let mk_kit_tx ~from_ ~to_ ~amount =
  {
    from_;
    txs = [ { to_; token_id = kit_token_id; amount = kit_to_mukit_nat amount } ];
  }
...
let add_kit_operator ~owner ~operator =
  add_operator ~owner ~operator ~token_id:kit_token_id

let add_lqt_operator ~owner ~operator =
  add_operator ~owner ~operator ~token_id:lqt_token_id

let remove_kit_operator ~owner ~operator =
  Remove_operator { owner; operator; token_id = kit_token_id }
...
let suite =
  ...
  >::: [
         ...
         >:: fun _ ->
           assert_raises (Failure "FA2_TOKEN_UNDEFINED") (fun _ ->
         ...
         >:: fun _ ->
           ...
           let update_op =
             add_kit_operator ~owner:alice_addr ~operator:bob_addr
           in
           let _fa2_state =
             fa2_run_update_operators (fa2_state, [ update_op ])
           in
           () );
         ...
         >:: fun _ ->
           ...
           let fa2_state = initial_fa2_state in
           let update_op =
             add_kit_operator ~owner:bob_addr ~operator:leena_addr
           in
           assert_raises (Failure "FA2_NOT_OWNER") (fun () ->
               fa2_run_update_operators (fa2_state, [ update_op ])) );
         ...
         >:: fun _ ->
           ...
           let fa2_state = initial_fa2_state in
           let update_op =
             remove_kit_operator ~owner:alice_addr ~operator:bob_addr
           in
         ...
         >:: fun _ ->
           ...
           let fa2_state = initial_fa2_state in
           let update_op =
             remove_kit_operator ~owner:bob_addr ~operator:leena_addr
           in

           assert_raises (Failure "FA2_NOT_OWNER") (fun () ->
               fa2_run_update_operators (fa2_state, [ update_op ])) );
         ( "fa2_run_update_operators - Add/Remove_operator - add/remove the \
         ...
         >:: fun _ ->
           ...
           assert_bool "Test potency: starting operators"
             (not
                (fa2_is_operator
                   (fa2_state, bob_addr, alice_addr, kit_token_id)));

           let fa2_state =
             fa2_run_update_operators (fa2_state, [ update_op ])
           in

           assert_bool "Add_operator did not add the operator"
           ...
           assert_bool "Remove_operator did not remove the operator"
             (not
                (fa2_is_operator
                   (fa2_state, bob_addr, alice_addr, kit_token_id)));

           () );
         ( "fa2_run_update_operators - list should be executed from left to \
            right"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
           ...
           let fa2_state =
             fa2_run_update_operators
               (fa2_state, [ add_operator_op; remove_operator_op ])
           in

           assert_bool "Add/Remove should leave no privileges"
             (not
                (fa2_is_operator
                   (fa2_state, bob_addr, alice_addr, kit_token_id)));
           ...
           let fa2_state =
             fa2_run_update_operators
               (fa2_state, [ remove_operator_op; add_operator_op ])
           in

           assert_bool "Remove/Add should leave privileges"
             (fa2_is_operator (fa2_state, bob_addr, alice_addr, kit_token_id));
         ...
         >:: fun _ ->
           ...
           let fa2_state_out =
             ledger_issue_kit (fa2_state_in, leena_addr, kit_amount)
           in
           assert_kit_equal
           ...
           let fa2_state_in = fa2_state_out in
           let kit_amount =
             kit_of_mukit (Ligo.nat_from_literal "789_012_345n")
           in
           let fa2_state_out =
             ledger_issue_kit (fa2_state_in, leena_addr, kit_amount)
           in
           ...
           let kit_amount =
             kit_sub
               (ask_kit_of fa2_state_in leena_addr)
               (kit_of_mukit (Ligo.nat_from_literal "1_234n"))
           in
           let fa2_state_out =
             ledger_withdraw_kit (fa2_state_in, leena_addr, kit_amount)
           in
         ...
         @@ QCheck.Test.make
              ~name:
                "ledger_issue_then_withdraw_kit should behave like an issue \
                 and then a withdraw (kit_to_withdraw <= kit_to_issue)"
         ...
         let fa2_state_out_1 =
           ledger_withdraw_kit
             ( ledger_issue_kit (fa2_state_in, leena_addr, kit_to_issue),
               leena_addr,
               kit_to_withdraw )
         in
         let fa2_state_out_2 =
           ledger_issue_then_withdraw_kit
             (fa2_state_in, leena_addr, kit_to_issue, kit_to_withdraw)
         in
         ask_kit_of fa2_state_out_1 leena_addr
         = ask_kit_of fa2_state_out_2 leena_addr );
         ( qcheck_to_ounit
         @@ QCheck.Test.make
              ~name:
                "ledger_issue_then_withdraw_kit should behave like an issue \
              ...
              (QCheck.pair arb_kit arb_kit)
         @@ fun (kit_1, kit_2) ->
         let kit_to_issue = kit_min kit_1 kit_2 in
         let kit_to_withdraw = kit_max kit_1 kit_2 in
         ...
         assert_raises (Failure "FA2_INSUFFICIENT_BALANCE") (fun () ->
             ledger_issue_then_withdraw_kit
               (fa2_state_in, leena_addr, kit_to_issue, kit_to_withdraw));
         true );
         ( (* Added this just because it's near impossible to get two values to be
            * equal so the above two tests rarely (never) checks the equals case. *)
           qcheck_to_ounit
         @@ QCheck.Test.make
              ~name:
                "ledger_issue_then_withdraw_kit should behave like an issue \
         ...
         let fa2_state_out_1 =
           ledger_withdraw_kit
             (ledger_issue_kit (fa2_state_in, leena_addr, kit), leena_addr, kit)
         in
         let fa2_state_out_2 =
           ledger_issue_then_withdraw_kit (fa2_state_in, leena_addr, kit, kit)
         in
         ask_kit_of fa2_state_out_1 leena_addr
         = ask_kit_of fa2_state_out_2 leena_addr );
         (* ************************************************************************* *)
         ...
         (* ************************************************************************* *)
         ( "ledger_issue_lqt/ledger_withdraw_lqt issues/withdraws expected \
            liquidity"
         >:: fun _ ->
           Ligo.Tezos.reset ();
           ...
           (* Leena's account initially empty. *)
           let fa2_state_in = initial_fa2_state in
           assert_lqt_equal ~expected:lqt_zero
           ...
           let fa2_state_in = fa2_state_out in
           let lqt_amount =
             lqt_of_denomination (Ligo.nat_from_literal "123_456n")
           in
           let fa2_state_out =
             ledger_issue_lqt (fa2_state_in, leena_addr, lqt_amount)
           in
           assert_lqt_equal
           ...
           let fa2_state_in = fa2_state_out in
           let lqt_amount =
             lqt_of_denomination (Ligo.nat_from_literal "789_012_345n")
           in
           let fa2_state_out =
             ledger_issue_lqt (fa2_state_in, leena_addr, lqt_amount)
           in
           ...
           let lqt_amount =
             lqt_sub
               (ask_lqt_of fa2_state_in leena_addr)
               (lqt_of_denomination (Ligo.nat_from_literal "1_234n"))
           in
           let fa2_state_out =
             ledger_withdraw_lqt (fa2_state_in, leena_addr, lqt_amount)
           in
         ...
         (* ************************************************************************* *)
         ( "fa2_run_transfer - succeeds from owner" >:: fun _ ->
           Ligo.Tezos.reset ();
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           ...
           let fa2_state = initial_fa2_state in
           let tx =
             mk_kit_tx ~from_:alice_addr ~to_:bob_addr ~amount:kit_zero
           in
         ...
         >:: fun _ ->
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let tx =
             mk_kit_tx ~from_:alice_addr ~to_:leena_addr ~amount:kit_zero
           in
           let _fa2_state = fa2_run_transfer (fa2_state, [ tx ]) in
           () );
         ( "fa2_run_transfer - transfers (kit) funds as expected" >:: fun _ ->
           Ligo.Tezos.reset ();
           let fa2_state_in = initial_fa2_state in
           ...
           assert_kit_equal ~expected:kit_zero
             ~real:(ask_kit_of fa2_state_in leena_addr);
           assert_kit_equal ~expected:kit_zero
             ~real:(ask_kit_of fa2_state_in bob_addr);
           assert_kit_equal ~expected:kit_zero
             ~real:(ask_kit_of fa2_state_in alice_addr);
           ...
           let fa2_state_in =
             let fa2_state_in =
               ledger_issue_kit
                 ( fa2_state_in,
                   leena_addr,
                   kit_of_mukit (Ligo.nat_from_literal "7_000_000n") )
             in
             let fa2_state_in =
               ledger_issue_kit
                 ( fa2_state_in,
             ...
             let fa2_state_in =
               ledger_issue_kit
                 ( fa2_state_in,
                   alice_addr,
                   kit_of_mukit (Ligo.nat_from_literal "9_000_000n") )
             in
             fa2_state_in
           in
           ...
           assert_kit_equal
             ~expected:(kit_of_mukit (Ligo.nat_from_literal "7_000_000n"))
             ~real:(ask_kit_of fa2_state_in leena_addr);
           assert_kit_equal
             ~expected:(kit_of_mukit (Ligo.nat_from_literal "8_000_000n"))
             ~real:(ask_kit_of fa2_state_in bob_addr);
           assert_kit_equal
             ~expected:(kit_of_mukit (Ligo.nat_from_literal "9_000_000n"))
             ~real:(ask_kit_of fa2_state_in alice_addr);
           ...
           let tx = mk_kit_tx ~from_:alice_addr ~to_:leena_addr ~amount in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let update_op =
             add_kit_operator ~owner:leena_addr ~operator:alice_addr
           in
           let fa2_state_in =
             fa2_run_update_operators (fa2_state_in, [ update_op ])
           in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ~sender:alice_addr
             ~amount:(Ligo.tez_from_literal "0mutez");
           let amount = kit_of_mukit (Ligo.nat_from_literal "1_500_000n") in
           let tx = mk_kit_tx ~from_:leena_addr ~to_:bob_addr ~amount in
           let fa2_state_out = fa2_run_transfer (fa2_state_in, [ tx ]) in
           ...
           let tx1 =
             mk_kit_tx ~from_:leena_addr ~to_:alice_addr ~amount:amount1
           in
           let amount2 = kit_of_mukit (Ligo.nat_from_literal "14_500_000n") in
           let tx2 =
             mk_kit_tx ~from_:alice_addr ~to_:bob_addr ~amount:amount2
           in

           (* The numbers are too high for the operations to be executed in the wrong order. *)
           assert_raises (Failure "FA2_INSUFFICIENT_BALANCE") (fun () ->
               fa2_run_transfer (fa2_state_in, [ tx2; tx1 ]));
           ...
           assert_kit_equal
           (* alice's amount goes up by amount1 by down by amount2 *)
         ...
         >:: fun _ ->
           ...
           assert_lqt_equal ~expected:lqt_zero
             ~real:(ask_lqt_of fa2_state_in leena_addr);
           assert_lqt_equal ~expected:lqt_zero
             ~real:(ask_lqt_of fa2_state_in bob_addr);
           assert_lqt_equal ~expected:lqt_zero
             ~real:(ask_lqt_of fa2_state_in alice_addr);
           ...
           let fa2_state_in =
             let fa2_state_in =
               ledger_issue_lqt
                 ( fa2_state_in,
                   leena_addr,
                   lqt_of_denomination (Ligo.nat_from_literal "7_000_000n") )
             in
             let fa2_state_in =
               ledger_issue_lqt
                 ( fa2_state_in,
             ...
             let fa2_state_in =
               ledger_issue_lqt
                 ( fa2_state_in,
                   alice_addr,
                   lqt_of_denomination (Ligo.nat_from_literal "9_000_000n") )
             in
             fa2_state_in
           in
           ...
           let amount =
             lqt_of_denomination (Ligo.nat_from_literal "123_456n")
           in
           let tx = mk_lqt_tx ~from_:alice_addr ~to_:leena_addr ~amount in
           let fa2_state_out = fa2_run_transfer (fa2_state_in, [ tx ]) in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let update_op =
             add_lqt_operator ~owner:leena_addr ~operator:alice_addr
           in
           let fa2_state_in =
             fa2_run_update_operators (fa2_state_in, [ update_op ])
           in
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let amount =
             lqt_of_denomination (Ligo.nat_from_literal "1_500_000n")
           in
           let tx = mk_lqt_tx ~from_:leena_addr ~to_:bob_addr ~amount in
           let fa2_state_out = fa2_run_transfer (fa2_state_in, [ tx ]) in
           ...
           Ligo.Tezos.new_transaction ~seconds_passed:0 ~blocks_passed:0
             ...
             ~amount:(Ligo.tez_from_literal "0mutez");
           let amount1 =
             lqt_of_denomination (Ligo.nat_from_literal "5_623_456n")
           in
           let tx1 =
             mk_lqt_tx ~from_:leena_addr ~to_:alice_addr ~amount:amount1
           in
           let amount2 =
             lqt_of_denomination (Ligo.nat_from_literal "14_500_000n")
           in
           let tx2 =
             mk_lqt_tx ~from_:alice_addr ~to_:bob_addr ~amount:amount2
           in
