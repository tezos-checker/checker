...
type burrow_id = Ligo.address * Ligo.nat [@@deriving show]
...
type completed_liquidation_auctions = { youngest : avl_ptr; oldest : avl_ptr }
[@@deriving show]
...
let liquidation_auction_empty : liquidation_auctions =
  ...
  let avl_storage, queued_slices =
    avl_mk_empty avl_storage (None : auction_outcome option)
  in
  {
    avl_storage;
    queued_slices;
    ...
    burrow_slices =
      (Ligo.Big_map.empty : (burrow_id, burrow_liquidation_slices) Ligo.big_map);
  }
