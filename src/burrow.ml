...
let burrow_touch (p : parameters) (burrow : burrow) : burrow =
  ...
  let burrow_out =
    if p.last_touched = burrow.last_touched then burrow
    else
...
let[@inline] undercollateralization_condition (f : ratio) (price : ratio)
    (tez : ratio) (kit : ratio)
    : bool
  =
  ...
  let lhs =
    Ligo.mul_int_int
      ...
      (Ligo.mul_int_int den_kt den_p)
  in
  let rhs =
    Ligo.mul_int_int
      (Ligo.mul_int_int num_f num_kt)
      (Ligo.mul_int_int den_tz num_p)
  in
...
let burrow_is_overburrowed (p : parameters) (b : burrow) : bool =
  ...
  assert_burrow_invariants b;
  let tez =
    { num = tez_to_mutez b.collateral; den = Ligo.int_from_literal "1_000_000" }
  in
  let kit =
    { num = kit_to_mukit_int b.outstanding_kit; den = kit_scaling_factor_int }
  in
...
let burrow_max_mintable_kit (p : parameters) (b : burrow) : kit =
  ...
  let numerator =
    Ligo.mul_int_int
      ...
      (Ligo.mul_int_int den_fm den_mp)
  in
  let denominator =
    Ligo.mul_int_int
      (Ligo.int_from_literal "1_000_000")
      (Ligo.mul_int_int num_fm num_mp)
  in
...
let burrow_return_slice_from_auction (slice : liquidation_slice_contents)
    (burrow : burrow)
    : burrow
  =
  assert_burrow_invariants burrow;
  ...
  let burrow_out =
    {
      ...
      collateral_at_auction =
        Ligo.sub_tez_tez burrow.collateral_at_auction slice.tez;
    }
  in
  assert (burrow.address = burrow_out.address);
  burrow_out

let burrow_return_kit_from_auction (slice : liquidation_slice_contents)
    (kit : kit) (burrow : burrow)
    : burrow
  =
  assert_burrow_invariants burrow;
  assert (Ligo.geq_tez_tez burrow.collateral_at_auction slice.tez);
...
let burrow_create (p : parameters) (addr : Ligo.address) (tez : Ligo.tez)
    (delegate_opt : Ligo.key_hash option)
    : burrow
  =
...
let[@inline] burrow_deposit_tez (p : parameters) (t : Ligo.tez) (b : burrow)
    : burrow
  =
...
let burrow_withdraw_tez (p : parameters) (t : Ligo.tez) (b : burrow) : burrow =
  ...
  let burrow_out =
    if burrow_is_overburrowed p burrow then
      (Ligo.failwith error_WithdrawTezFailure : burrow)
    else burrow
  in
  assert (b.address = burrow_out.address);
  burrow_out
...
let burrow_mint_kit (p : parameters) (kit : kit) (b : burrow) : burrow =
  ...
  let burrow_out =
    let burrow =
      rebalance_kit { b with outstanding_kit = kit_add b.outstanding_kit kit }
    in
    if burrow_is_overburrowed p burrow then
      (Ligo.failwith error_MintKitFailure : burrow)
    else burrow
  in
  assert (b.address = burrow_out.address);
  burrow_out
...
let burrow_activate (p : parameters) (tez : Ligo.tez) (b : burrow) : burrow =
  ...
  let burrow_out =
    if Ligo.lt_tez_tez tez creation_deposit then
      (Ligo.failwith error_InsufficientFunds : burrow)
    else if b.active then (Ligo.failwith error_BurrowIsAlreadyActive : burrow)
    else
      {
        b with
        active = true;
...
let burrow_deactivate (p : parameters) (b : burrow) : burrow * Ligo.tez =
  ...
  let burrow_out, return =
    if burrow_is_overburrowed p b then
      ...
      Ligo.gt_tez_tez b.collateral_at_auction (Ligo.tez_from_literal "0mutez")
    then
      ( Ligo.failwith error_DeactivatingWithCollateralAtAuctions
        : burrow * Ligo.tez )
    else
      let return = Ligo.add_tez_tez b.collateral creation_deposit in
      let updated_burrow =
        { b with active = false; collateral = Ligo.tez_from_literal "0mutez" }
      in
...
let burrow_set_delegate (p : parameters) (new_delegate : Ligo.key_hash option)
    (b : burrow)
    : burrow
  =
  let b = burrow_touch p b in
...
let compute_tez_to_auction (p : parameters) (b : burrow) : Ligo.int =
  ...
  let numerator =
    Ligo.sub_int_int
      (Ligo.mul_int_int
         (Ligo.int_from_literal "1_000_000")
         (Ligo.mul_int_int den_lp
            (Ligo.mul_int_int num_fm
               (Ligo.mul_int_nat num_mp (kit_to_mukit_nat b.outstanding_kit)))))
      (Ligo.mul_int_int
         (Ligo.mul_int_int kit_scaling_factor_int den_mp)
         (Ligo.add_int_int
            (Ligo.mul_int_int num_lp
               (Ligo.mul_int_int num_fm (tez_to_mutez b.collateral_at_auction)))
            (Ligo.mul_int_int den_lp
               (Ligo.mul_int_int den_fm (tez_to_mutez b.collateral)))))
  in
  (* denominator = (kit_sf * den_mp * tez_sf) * (num_lp * num_fm - den_lp * den_fm) *)
  let denominator =
    Ligo.mul_int_int kit_scaling_factor_int
      (Ligo.mul_int_int den_mp
         (Ligo.mul_int_int
            (Ligo.int_from_literal "1_000_000")
            (Ligo.sub_int_int
  ...
  in
  cdiv_int_int
    (Ligo.mul_int_int numerator (Ligo.int_from_literal "1_000_000"))
    denominator
...
let compute_expected_kit (p : parameters) (tez_to_auction : Ligo.tez) : ratio =
  ...
  let numerator =
    Ligo.mul_int_int
      ...
      (Ligo.mul_int_int (Ligo.sub_int_int den_lp num_lp) den_mp)
  in
  let denominator =
    Ligo.mul_int_int
      (Ligo.int_from_literal "1_000_000")
      (Ligo.mul_int_int den_lp num_mp)
  in
...
let burrow_is_liquidatable (p : parameters) (b : burrow) : bool =
  ...
  let kit =
    ...
    let { num = num_ek; den = den_ek } =
      compute_expected_kit p b.collateral_at_auction
    in
    {
      num =
        Ligo.sub_int_int
  ...
  in
  b.active
  && undercollateralization_condition fliquidation (liquidation_price p) tez kit
...
(** Check whether the return of a slice to its burrow (cancellation) is
  ...
  * restored. *)
let burrow_is_cancellation_warranted (p : parameters) (b : burrow)
    (slice_tez : Ligo.tez)
    : bool
  =
  assert (p.last_touched = b.last_touched);
  ...
  let kit =
    ...
    let { num = num_ek; den = den_ek } =
      compute_expected_kit p
        (Ligo.sub_tez_tez b.collateral_at_auction slice_tez)
    in
    {
      num =
        Ligo.sub_int_int
  ...
  b.active
  && not (undercollateralization_condition fminting (minting_price p) tez kit)
...
*)
let[@inline] compute_min_kit_for_unwarranted (p : parameters) (b : burrow)
    (tez_to_auction : Ligo.tez)
    : kit option
  =
  assert (p.last_touched = b.last_touched);
  ...
  then
      ...
      let numerator =
        Ligo.mul_int_int
          ...
          (Ligo.sub_int_int
             ...
             (Ligo.mul_int_int kit_scaling_factor_int num_ek))
      in
      max_int (Ligo.int_from_literal "0") numerator
    in
    ...
    let denominator =
      Ligo.mul_int_int
        ...
        (Ligo.mul_int_int kit_scaling_factor_int den_ek)
    in

    Some (kit_of_fraction_ceil numerator denominator)

(* Round up here; safer for the system, less so for the burrow *)

let burrow_request_liquidation (p : parameters) (b : burrow)
    : liquidation_result
  =
  let b = burrow_touch p b in
  assert_burrow_invariants b;
  ...
  if not (burrow_is_liquidatable p b) then
    ...
    if
      Ligo.lt_tez_tez
        (Ligo.sub_tez_tez b.collateral partial_reward)
        creation_deposit
    then
      ...
      let final_burrow =
        {
          ...
          collateral_at_auction =
            Ligo.add_tez_tez b.collateral_at_auction tez_to_auction;
        }
      in
      Some
        ( Close,
          { liquidation_reward; tez_to_auction; burrow_state = final_burrow } )
    else
      ...
      if
        Ligo.lt_int_int tez_to_auction (Ligo.int_from_literal "0")
        || Ligo.gt_int_int tez_to_auction
             (tez_to_mutez b_without_reward.collateral)
      then
         ...
         * shall be returned. *)
        let tez_to_auction =
          match Ligo.is_nat tez_to_auction with
          | Some mutez ->
              Ligo.mul_nat_tez mutez (Ligo.tez_from_literal "1mutez")
          (* Note: disabling coverage for this line since it really should be impossible to reach this line *)
          | None -> (
              ( Ligo.failwith internalError_ComputeTezToAuctionNegativeResult
                : Ligo.tez ) [@coverage off] )
        in
        let final_burrow =
          {
            b with
            collateral =
              Ligo.sub_tez_tez b_without_reward.collateral tez_to_auction;
            collateral_at_auction =
              Ligo.add_tez_tez b.collateral_at_auction tez_to_auction;
          }
        in
...
let make_burrow_for_test ~active ~address ~delegate ~collateral ~outstanding_kit
    ~excess_kit ~adjustment_index ~collateral_at_auction ~last_touched
  =
  {
    delegate;
    address;
