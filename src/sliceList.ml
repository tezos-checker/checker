...
type slice_list = SliceList of slice_list_meta [@@deriving show]

[@@@coverage on]

let slice_list_element_contents (e : slice_list_element)
    : liquidation_slice_contents
  =
  match e with SliceListElement (_, contents) -> contents.contents

let slice_list_empty (burrow : burrow_id) : slice_list =
  SliceList
    {
      slice_list_burrow = burrow;
      slice_list_bounds = (None : slice_list_bounds option);
    }

let slice_list_is_empty (l : slice_list) : bool =
  let meta = match l with SliceList meta -> meta in
  match meta.slice_list_bounds with Some _ -> false | None -> true

(* Constructs a burrow slice list for the given burrow id using the provided auction state *)
let[@inline] slice_list_from_auction_state (auctions : liquidation_auctions)
    (burrow_id : burrow_id)
    : slice_list
  =
...
let[@inline] slice_list_from_leaf_ptr (auctions : liquidation_auctions)
    (ptr : leaf_ptr)
    : slice_list_element * slice_list
  =
...
let[@inline] slice_list_from_queue_head (auctions : liquidation_auctions)
    : (slice_list_element * slice_list) option
  =
  ...
  | Some (ptr, slice) ->
      ...
      let list =
        slice_list_from_auction_state auctions slice.value.contents.burrow
      in
      Some (element, list)
...
let[@inline] slice_list_to_auction_state (auctions : liquidation_auctions)
    (l : slice_list)
    : liquidation_auctions
  =
  match l with
  | SliceList meta ->
      let burrow_liquidation_slice =
      ...
      in
      let burrow_slices =
        Ligo.Big_map.update meta.slice_list_burrow burrow_liquidation_slice
          auctions.burrow_slices
      in
      { auctions with burrow_slices }
...
*)
let[@inline] slice_list_append (l : slice_list)
    (auctions : liquidation_auctions) (root : liquidation_auction_id)
    (queue_end : queue_end) (slice_contents : liquidation_slice_contents)
    : liquidation_auctions * slice_list * slice_list_element
  =
  ...
  | None ->
      ...
      in
      let bounds =
        { slice_list_youngest_ptr = ptr; slice_list_oldest_ptr = ptr }
      in
      ( { auctions with avl_storage = storage },
        SliceList { meta with slice_list_bounds = Some bounds },
        SliceListElement (ptr, slice) )
  ...
  | Some bounds ->
      ...
      let slice =
        {
          younger = (None : leaf_ptr option);
          older = Some bounds.slice_list_youngest_ptr;
          contents = slice_contents;
        }
      in
      (* Write slice to AVL backend *)
      ...
      let storage =
        avl_update_leaf storage former_youngest (fun (s : liquidation_slice) ->
            { s with younger = Some ptr })
      in
...
let[@inline] slice_list_remove (l : slice_list)
    (auctions : liquidation_auctions) (e : slice_list_element)
    : liquidation_auctions
      * slice_list
  ...
  let meta = match l with SliceList m -> m in
  let ptr, slice =
    match e with SliceListElement (ptr, slice) -> (ptr, slice)
  in
  ...
  | None ->
      ( Ligo.failwith internalError_SliceListRemoveEmptyList
        : liquidation_auctions
          * slice_list
          * avl_ptr
          * liquidation_slice_contents )
  | Some bounds ->
      ...
      let bounds =
        if ptr = bounds.slice_list_youngest_ptr then
          ...
          | Some older_ptr ->
              Some { bounds with slice_list_youngest_ptr = older_ptr }
        else if
          (* Case 2: We are removing the oldest slice *)
          ptr = bounds.slice_list_oldest_ptr
        then
          ...
          | Some younger_ptr ->
              Some { bounds with slice_list_oldest_ptr = younger_ptr }
      ...
      let storage =
        ...
        | None -> storage
        | Some older_ptr ->
            avl_update_leaf storage older_ptr (fun (s : liquidation_slice) ->
                { s with younger = slice.younger })
      in
      let storage =
        match slice.younger with
        | None -> storage
        | Some younger_ptr ->
            avl_update_leaf storage younger_ptr (fun (s : liquidation_slice) ->
                { s with older = slice.older })
      in
      (* Delete the element from the AVL backend *)
      let storage, root_ptr = avl_del storage ptr in
      ( { auctions with avl_storage = storage },
        SliceList { meta with slice_list_bounds = bounds },
        root_ptr,
        slice.contents )
...
let slice_list_youngest (l : slice_list) (auctions : liquidation_auctions)
    : slice_list_element option
  =
  let storage = auctions.avl_storage in
  let meta = match l with SliceList meta -> meta in
  match meta.slice_list_bounds with
  | Some bounds ->
      Some
        (SliceListElement
           ( bounds.slice_list_youngest_ptr,
             avl_read_leaf storage bounds.slice_list_youngest_ptr ))
  | None -> None

(* Gets the oldest element of the list *)
let slice_list_oldest (l : slice_list) (auctions : liquidation_auctions)
    : slice_list_element option
  =
  let storage = auctions.avl_storage in
  let meta = match l with SliceList meta -> meta in
  ...
  | Some bounds ->
      Some
        (SliceListElement
           ( bounds.slice_list_oldest_ptr,
             avl_read_leaf storage bounds.slice_list_oldest_ptr ))
  | None -> None
