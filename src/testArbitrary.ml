...
let arb_tez = QCheck.map (fun x -> Tez.of_mutez (Z.of_int x)) QCheck.(0 -- max_int)

let arb_positive_tez = QCheck.map (fun x -> Tez.of_mutez (Z.of_int x)) QCheck.(1 -- max_int)

(* somewhere between 0 and 3 tez *)
let arb_small_tez =
  QCheck.map
    (fun x -> Tez.of_mutez (Z.of_int x))
