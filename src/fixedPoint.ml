...
let fixedpoint_scaling_factor = Ligo.int_from_literal "18446744073709551616"

(* 2 (scaling_base) ^ 64 (scaling_exponent) *)
...
let[@inline] fixedpoint_add (x : fixedpoint) (y : fixedpoint) =
  Ligo.add_int_int x y

let[@inline] fixedpoint_sub (x : fixedpoint) (y : fixedpoint) =
  Ligo.sub_int_int x y

let fixedpoint_pow (x : fixedpoint) (y : Ligo.nat) =
  if Ligo.eq_nat_nat y (Ligo.nat_from_literal "0n") then fixedpoint_one
  else
    Ligo.div_int_int (pow_int_nat x y)
      (pow_int_nat fixedpoint_scaling_factor
         (Ligo.abs (Ligo.sub_nat_nat y (Ligo.nat_from_literal "1n"))))

(* Conversions to/from other types. *)
let fixedpoint_of_ratio_ceil (amnt : ratio) =
  cdiv_int_int (Ligo.mul_int_int amnt.num fixedpoint_scaling_factor) amnt.den

let fixedpoint_of_ratio_floor (amnt : ratio) =
  fdiv_int_int (Ligo.mul_int_int amnt.num fixedpoint_scaling_factor) amnt.den
...
let fixedpoint_to_ratio (amnt : fixedpoint) =
  make_ratio amnt fixedpoint_scaling_factor
...
let fixedpoint_of_hex_string str =
  ...
  let mantissa =
    ...
    | Some pos ->
        pow_int_nat
          (Ligo.int_from_literal "16")
          (Ligo.abs
             (Ligo.int_from_literal
                (string_of_int (String.length str - pos - 1))))
  in
  Ligo.div_int_int
    (Ligo.mul_int_int
       (Ligo.of_string_base_int 16 without_dot)
...
let show_fixedpoint amnt =
  ...
  let upper, lower =
    Ligo.div_rem_int_int (Ligo.int (Ligo.abs amnt)) fixedpoint_scaling_factor
  in

  (* in hex, otherwise it's massive *)
  Format.sprintf "%s%s.%s" sign
