...
let lqt_of_fraction_ceil (x_num : Ligo.int) (x_den : Ligo.int) : lqt =
  ...
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_LqtOfFractionCeilNegative : lqt)
  else
    Ligo.abs
      (cdiv_int_int (Ligo.mul_int_int x_num lqt_scaling_factor_int) x_den)

let lqt_of_fraction_floor (x_num : Ligo.int) (x_den : Ligo.int) : lqt =
  assert (Ligo.gt_int_int x_den (Ligo.int_from_literal "0"));
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_LqtOfFractionFloorNegative : lqt)
  else
    Ligo.abs
      (fdiv_int_int (Ligo.mul_int_int x_num lqt_scaling_factor_int) x_den)
...
let[@inline] lqt_to_ratio (amnt : lqt) : ratio =
  make_ratio (Ligo.int amnt) lqt_scaling_factor_int

let lqt_compare = compare_nat

let show_lqt amnt =
  let zfill s width =
    match Stdlib.(width - String.length s) with
  ...
  let as_string =
    if lqt_decimal_digits = Ligo.nat_from_literal "0n" then
      ...
      let lqt_decimal_digits =
        Stdlib.int_of_string (Ligo.string_of_nat lqt_decimal_digits)
      in
      (* little hacky *)
      Ligo.string_of_nat d ^ "."
      ^ zfill (Ligo.string_of_nat r) lqt_decimal_digits
  in
  as_string ^ "lqt"
