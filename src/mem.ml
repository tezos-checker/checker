...
type mem = { mem : (ptr, node) Ligo.big_map; last_ptr : ptr }

(* BEGIN_OCAML *)
[@@@coverage off]

type ops = { reads : int; writes : int } [@@deriving show]
...
let mem_empty =
  { last_ptr = ptr_null; mem = (Ligo.Big_map.empty : (ptr, node) Ligo.big_map) }
...
let[@inline] mem_del (mem : mem) (k : ptr) : mem =
  ...
  ops := { !ops with writes = !ops.writes + 1 };
  (* END_OCAML *)
  let { mem; last_ptr } = mem in
  assert (Ligo.Big_map.mem k mem);
  { mem = Ligo.Big_map.update k (None : node option) mem; last_ptr }
