  ...
  if Ligo.leq_nat_nat !Ligo.Tezos.level cfmm.last_level then
  ...
  else
    {
      ...
      kit_in_ctez_in_prev_block =
        make_ratio
          (Ligo.mul_nat_int
             (ctez_to_muctez_nat cfmm.ctez)
             kit_scaling_factor_int)
          (Ligo.mul_nat_int
             (kit_to_mukit_nat cfmm.kit)
             (Ligo.int_from_literal "1_000_000"));
      last_level = !Ligo.Tezos.level;
    }

let cfmm_view_min_kit_expected_buy_kit (cfmm : cfmm) (ctez_amount : ctez)
  ...
  if eq_ctez_ctez ctez_amount ctez_zero then
    ...
    let { num = num_uf; den = den_uf } =
      ...
      { num = Ligo.sub_int_int den_uf num_uf; den = den_uf }
      (* 1 - cfmm_fee *)
    in
    ...
    let numerator =
      Ligo.mul_nat_int
        ...
        (Ligo.mul_nat_int (kit_to_mukit_nat cfmm.kit) num_uf)
    in
    let denominator =
      Ligo.mul_int_int kit_scaling_factor_int
        (Ligo.mul_nat_int (ctez_to_muctez_nat new_cfmm_ctez) den_uf)
    in
    ...
    ( bought_kit,
      { cfmm with kit = kit_sub cfmm.kit bought_kit; ctez = new_cfmm_ctez } )

let cfmm_buy_kit (cfmm : cfmm) (ctez_amount : ctez) (min_kit_expected : kit)
  ...
  if eq_ctez_ctez ctez_amount ctez_zero then
    ...
    let bought_kit, cfmm =
      cfmm_view_min_kit_expected_buy_kit cfmm ctez_amount
    in
    if lt_kit_kit bought_kit min_kit_expected then
      (Ligo.failwith error_BuyKitPriceFailure : kit * cfmm)
    else (bought_kit, cfmm)

let cfmm_view_min_ctez_expected_cfmm_sell_kit (cfmm : cfmm) (kit_amount : kit)
    : ctez * cfmm
  =
  let cfmm = cfmm_sync_last_observed cfmm in
  let cfmm = cfmm_assert_initialized cfmm in
  if eq_kit_kit kit_amount kit_zero then
    ...
    let numerator =
      Ligo.mul_nat_int
        ...
        (Ligo.mul_nat_int (ctez_to_muctez_nat cfmm.ctez) num_uf)
    in
    let denominator =
      Ligo.mul_int_int
        (Ligo.int_from_literal "1_000_000")
        (Ligo.mul_nat_int (kit_to_mukit_nat new_cfmm_kit) den_uf)
    in
    ...
    ( bought_ctez,
      { cfmm with kit = new_cfmm_kit; ctez = ctez_sub cfmm.ctez bought_ctez } )

let cfmm_sell_kit (cfmm : cfmm) (kit_amount : kit) (min_ctez_expected : ctez)
    (deadline : Ligo.timestamp)
    : ctez * cfmm
  =
  if eq_kit_kit kit_amount kit_zero then
    ...
    let bought_ctez, cfmm =
      cfmm_view_min_ctez_expected_cfmm_sell_kit cfmm kit_amount
    in
    if lt_ctez_ctez bought_ctez min_ctez_expected then
      (Ligo.failwith error_SellKitPriceFailure : ctez * cfmm)
    else (bought_ctez, cfmm)

let cfmm_view_max_kit_deposited_min_lqt_minted_cfmm_add_liquidity (cfmm : cfmm)
    (ctez_amount : ctez)
    : lqt * kit * cfmm
  =
  let cfmm = cfmm_sync_last_observed cfmm in
  ...
  if eq_ctez_ctez ctez_amount ctez_zero then
    ...
    let lqt_minted =
      lqt_of_fraction_floor
        (Ligo.mul_int_nat
           (lqt_to_denomination_int cfmm.lqt)
           (ctez_to_muctez_nat ctez_amount))
        (Ligo.mul_int_nat lqt_scaling_factor_int cfmm_ctez)
    in
    let kit_deposited =
      kit_of_fraction_ceil
        (Ligo.mul_int_nat
           (kit_to_mukit_int cfmm.kit)
           (ctez_to_muctez_nat ctez_amount))
        (Ligo.mul_int_nat kit_scaling_factor_int cfmm_ctez)
    in
...
let cfmm_add_liquidity (cfmm : cfmm) (ctez_amount : ctez)
    (max_kit_deposited : kit) (min_lqt_minted : lqt) (deadline : Ligo.timestamp)
    : lqt * kit * cfmm
  =
  if Ligo.geq_timestamp_timestamp !Ligo.Tezos.now deadline then
    ...
    let lqt_minted, kit_deposited, cfmm =
      cfmm_view_max_kit_deposited_min_lqt_minted_cfmm_add_liquidity cfmm
        ctez_amount
    in
...
let cfmm_view_min_ctez_withdrawn_min_kit_withdrawn_cfmm_remove_liquidity
  ...
  if eq_lqt_lqt lqt_burned lqt_zero then
    ...
    ( Ligo.failwith error_RemoveLiquidityTooMuchLiquidityWithdrawn
      : ctez * kit * cfmm )
  else
    let ctez_withdrawn =
      ctez_of_fraction_floor
        (Ligo.mul_nat_int
           (ctez_to_muctez_nat cfmm.ctez)
           (lqt_to_denomination_int lqt_burned))
        (Ligo.mul_int_nat
           (Ligo.int_from_literal "1_000_000")
           (lqt_to_denomination_nat cfmm.lqt))
    in
    let kit_withdrawn =
      kit_of_fraction_floor
        (Ligo.mul_int_nat
           (kit_to_mukit_int cfmm.kit)
           (lqt_to_denomination_nat lqt_burned))
        (Ligo.mul_int_nat kit_scaling_factor_int
           (lqt_to_denomination_nat cfmm.lqt))
    in
...
let cfmm_remove_liquidity (cfmm : cfmm) (lqt_burned : lqt)
    (min_ctez_withdrawn : ctez) (min_kit_withdrawn : kit)
    (deadline : Ligo.timestamp)
    : ctez * kit * cfmm
  =
  if Ligo.geq_timestamp_timestamp !Ligo.Tezos.now deadline then
    ...
    ( Ligo.failwith error_RemoveLiquidityNoCtezWithdrawnExpected
      : ctez * kit * cfmm )
  else if eq_kit_kit min_kit_withdrawn kit_zero then
    ( Ligo.failwith error_RemoveLiquidityNoKitWithdrawnExpected
      : ctez * kit * cfmm )
  else
    let ctez_withdrawn, kit_withdrawn, cfmm =
      cfmm_view_min_ctez_withdrawn_min_kit_withdrawn_cfmm_remove_liquidity cfmm
        lqt_burned
    in
    if lt_ctez_ctez ctez_withdrawn min_ctez_withdrawn then
      ( Ligo.failwith error_RemoveLiquidityCantWithdrawEnoughCtez
        : ctez * kit * cfmm )
    else if lt_kit_kit kit_withdrawn min_kit_withdrawn then
      ( Ligo.failwith error_RemoveLiquidityCantWithdrawEnoughKit
        : ctez * kit * cfmm )
    else (ctez_withdrawn, kit_withdrawn, cfmm)
