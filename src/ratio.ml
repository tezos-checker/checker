...
let fraction_to_tez_floor (x_num : Ligo.int) (x_den : Ligo.int) : Ligo.tez =
  ...
  | Some n -> (
      ...
      | None -> (
          ( Ligo.failwith internalError_FractionToTezFloorZeroDenominator
            : Ligo.tez ) [@coverage off] )
      | Some quot_and_rem ->
          let quot, _ = quot_and_rem in
          Ligo.mul_nat_tez quot (Ligo.tez_from_literal "1mutez")
      (* ignore the remainder; we floor towards zero here *) )
...
let fraction_to_nat_floor (x_num : Ligo.int) (x_den : Ligo.int) : Ligo.nat =
  ...
  | Some n -> (
      ...
      (* Note: Ignoring coverage for the case below since the assertion above makes it unreachable in OCaml *)
      | None -> (
          ( Ligo.failwith internalError_FractionToNatFloorZeroDenominator
            : Ligo.nat ) [@coverage off] )
      | Some quot_and_rem ->
          let quot, _ = quot_and_rem in
          quot
          (* ignore the remainder; we floor towards zero here *) )
...
(* Predefined values *)
let[@inline] zero_ratio : ratio =
  { num = Ligo.int_from_literal "0"; den = Ligo.int_from_literal "1" }

let[@inline] one_ratio : ratio =
  { num = Ligo.int_from_literal "1"; den = Ligo.int_from_literal "1" }

(* BEGIN_OCAML *)
[@@@coverage off]

let[@inline] ratio_of_tez (x : Ligo.tez) : ratio =
  { num = tez_to_mutez x; den = Ligo.int_from_literal "1_000_000" }

(* NOTE: this implementation relies on the fact that the denominator is always positive. *)
let lt_ratio_ratio (x : ratio) (y : ratio) : bool =
  let { num = x_num; den = x_den } = x in
  let { num = y_num; den = y_den } = y in
  Ligo.lt_int_int (Ligo.mul_int_int x_num y_den) (Ligo.mul_int_int y_num x_den)
...
let mul_ratio (x : ratio) (y : ratio) : ratio =
  let { num = x_num; den = x_den } = x in
  let { num = y_num; den = y_den } = y in
  make_ratio (Ligo.mul_int_int x_num y_num) (Ligo.mul_int_int x_den y_den)

(* NOTE: this implementation relies on the fact that the denominator is always positive. *)
let leq_ratio_ratio (x : ratio) (y : ratio) : bool =
  let { num = x_num; den = x_den } = x in
  let { num = y_num; den = y_den } = y in
  Ligo.leq_int_int (Ligo.mul_int_int x_num y_den) (Ligo.mul_int_int y_num x_den)

let[@inline] geq_ratio_ratio (x : ratio) (y : ratio) : bool =
  leq_ratio_ratio y x
...
let div_ratio (x : ratio) (y : ratio) : ratio =
  ...
  else mul_ratio x { num = neg_int y_den; den = neg_int y_num }
...
let sign_ratio x =
  if Ligo.lt_int_int x.num (Ligo.int_from_literal "0") then -1
  else if Ligo.eq_int_int x.num (Ligo.int_from_literal "0") then 0
  else 1

let[@inline] ratio_of_nat (n : Ligo.nat) : ratio =
  { num = Ligo.int n; den = Ligo.int_from_literal "1" }

let min_ratio (a : ratio) (b : ratio) : ratio =
  if leq_ratio_ratio a b then a else b

let max_ratio (a : ratio) (b : ratio) : ratio =
  if geq_ratio_ratio a b then a else b
...
let[@inline] ratio_of_int (i : Ligo.int) : ratio =
  { num = i; den = Ligo.int_from_literal "1" }
...
let eq_ratio_ratio (x : ratio) (y : ratio) : bool =
  let { num = x_num; den = x_den } = x in
  let { num = y_num; den = y_den } = y in
  Ligo.eq_int_int (Ligo.mul_int_int x_num y_den) (Ligo.mul_int_int y_num x_den)
