...
let assert_initialized (u: t) =
  ...
  let is_kit_pool_empty (u: t) =
    let kit, _same_token = read_kit u.kit in
    kit = kit_zero in
...
let sync_last_observed (uniswap: t) =
  ...
  if uniswap.last_level = !Ligo.tezos_level then
    ...
    let uniswap_kit, _same_token = read_kit uniswap.kit in (* NOTE: replace? *)
    { uniswap with
...
let sell_kit (uniswap: t) ~amount (token: kit_token) ~min_tez_expected ~deadline =
  let uniswap = sync_last_observed uniswap in
...
let add_accrued_kit (uniswap: t) (accrual: kit_token) : t =
  let uniswap = sync_last_observed uniswap in
