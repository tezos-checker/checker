...
let node_height (n : node) : Ligo.nat =
  ...
  | Branch branch ->
      let max =
        if Ligo.gt_nat_nat branch.left_height branch.right_height then
          branch.left_height
        else branch.right_height
      in
...
let deref_avl_ptr (mem : mem) (p : avl_ptr)
    : ptr option * auction_outcome option
  =
  let p = match p with AVLPtr p -> p in
  match mem_get mem p with
  | Root p -> p
  | _ ->
      ( Ligo.failwith internalError_DerefAvlPtrFoundNonRoot
        : ptr option * auction_outcome option )
...
let update_matching_child (mem : mem) (ptr : ptr) (from_ptr : ptr)
    (to_ptr : ptr)
    : mem
  =
  ...
  | Leaf _ -> (Ligo.failwith internalError_UpdateMatchingChildFoundLeaf : mem)
  | Branch old_branch ->
      let to_ = mem_get mem to_ptr in
      let new_branch =
        if old_branch.left = from_ptr then
          Branch
            {
              old_branch with
              left = to_ptr;
...
let ref_rotate_left (mem : mem) (curr_ptr : ptr) : mem * ptr =
  let curr =
    ...
    | Branch curr -> curr
    | _ ->
        (Ligo.failwith internalError_RefRotateLeftCurrentPtrNotBranch : branch)
  in
  ...
  let right =
    ...
    | Branch right -> right
    | _ -> (Ligo.failwith internalError_RefRotateLeftRightPtrNotBranch : branch)
  in
...
let ref_rotate_right (mem : mem) (curr_ptr : ptr) : mem * ptr =
  let curr =
    ...
    | Branch curr -> curr
    | _ ->
        (Ligo.failwith internalError_RefRotateRightCurrentPtrNotBranch : branch)
  in
  ...
  let left =
    ...
    | Branch left -> left
    | _ -> (Ligo.failwith internalError_RefRotateRightLeftPtrNotBranch : branch)
  in
...
let rebalance (mem : mem) (curr_ptr : ptr) : mem * ptr =
  ...
  | Branch branch ->
      if
        Ligo.gt_nat_nat
          (Ligo.abs (Ligo.sub_nat_nat branch.left_height branch.right_height))
      ...
      then (
        ...
        let heavy_child_ptr =
          if Ligo.lt_int_int diff (Ligo.int_from_literal "0") then branch.left
          else branch.right
        in
        let heavy_child =
          match mem_get mem heavy_child_ptr with
          | Branch b -> b
          | _ ->
              ( Ligo.failwith internalError_RebalanceHeavyChildNonBranchch
                : branch )
        in
        let heavy_child_balance =
          Ligo.sub_nat_nat heavy_child.right_height heavy_child.left_height
        in

        let mem, ptr =
          if Ligo.lt_int_int diff int_zero then
            if (* (diff < 0) *)
               Ligo.leq_int_int heavy_child_balance int_zero
            then
              (* (hcb <= 0) *)
              (* Left, Left *)
              ...
              let mem =
                update_matching_child mem curr_ptr heavy_child_ptr new_
              in
              ref_rotate_right mem curr_ptr )
          else (
            (* (diff >= 0) *)
...
let ref_join_post_processing (data : ref_join_data)
...
let rec left_fold_ref_join_data
  ...
  | [] -> mem_and_child_ptr
  | d :: ds ->
      let new_mem_and_child_ptr =
        ref_join_post_processing d mem_and_child_ptr
      in
...
let rec ref_join_rec
    ((mem, join_direction, left_ptr, right_ptr, stack) :
      mem * direction * ptr * ptr * ref_join_data list)
    : mem * ptr
  =
  ...
  then
    ...
    let new_join_direction, left_p, right_p, (ptr, to_fix) =
      ...
      if Ligo.gt_nat_nat (node_height left) (node_height right) then
        ...
        (Left, left_p, right_ptr, (left_ptr, left_p)) (* Or vice versa. *)
      else
    ...
    ref_join_rec
      ( mem,
        ...
        left_p,
        right_p,
        { join_direction; ptr; to_fix; parent_ptr } :: stack )

let ref_join (mem : mem) (join_direction : direction) (left_ptr : ptr)
    (right_ptr : ptr)
    : mem * ptr
  =
  ref_join_rec
    (mem, join_direction, left_ptr, right_ptr, ([] : ref_join_data list))

(* ************************** *)

(* Left for pushing back and Right for pushing front *)
let avl_push (mem : mem) (root_ptr : avl_ptr) (value : liquidation_slice)
    (d : direction)
    : mem * leaf_ptr
  =
  ...
  let node = Leaf { value; parent = root_ptr } in
...
let rec balance_bottom_up ((mem, curr_ptr) : mem * ptr) : mem * avl_ptr =
  ...
  | Leaf _ ->
      (Ligo.failwith internalError_BalanceBottomUpFoundLeaf : mem * avl_ptr)
  | Branch b ->
...
let ref_del (mem : mem) (ptr : ptr) : mem * avl_ptr =
  ...
  | Branch parent ->
      let sibling_ptr =
        if parent.left = ptr then parent.right else parent.left
      in
      ...
      let mem =
        update_matching_child mem grandparent_ptr parent_ptr sibling_ptr
      in
...
let avl_update_leaf (mem : mem) (ptr : leaf_ptr)
    (f : liquidation_slice -> liquidation_slice)
    : mem
  =
  let l = deref_leaf_ptr mem ptr in
...
let rec ref_delete_tree (mem : mem) (ptrs : ptr list) : mem =
  ...
  | ptr :: ptrs -> (
      ...
      | Branch branch ->
          ref_delete_tree mem (branch.left :: branch.right :: ptrs) )

let avl_delete_tree (mem : mem) (AVLPtr ptr) : mem = ref_delete_tree mem [ ptr ]
...
let rec ref_peek_front ((mem, ptr) : mem * ptr) : leaf_ptr * leaf =
  ...
  | Root _ ->
      (Ligo.failwith internalError_RefPeekFrontFoundRoot : leaf_ptr * leaf)
...
let avl_pop_front (mem : mem) (root_ptr : avl_ptr)
    : mem * (leaf_ptr * liquidation_slice) option
  =
...
type ref_split_data = { rec_direction : direction; branch : branch }

let ref_split_post_processing (data : ref_split_data)
    ((mem, maybe_left, maybe_right) : mem * ptr option * ptr option)
    : mem * ptr option * ptr option
  =
  let { rec_direction; branch } = data in
  match rec_direction with
  | Left -> (
      match maybe_right with
      | None ->
          ( Ligo.failwith internalError_RefSplitPostProcessingInvariantFailed
            : mem * ptr option * ptr option )
      | Some right ->
  ...
  | Right -> (
      ...
      | None -> (mem, Some branch.left, maybe_right) )

(* Nice and tail-recursive left fold we can write in ligo more or less as-is. *)
let rec left_fold_ref_split_data
    ((mem_and_left_ptr_and_right_ptr, stack) :
      (mem * ptr option * ptr option) * ref_split_data list)
    : mem * ptr option * ptr option
  =
  match stack with
  | [] -> mem_and_left_ptr_and_right_ptr
  | d :: ds ->
      let new_mem_and_left_ptr_and_right_ptr =
        ref_split_post_processing d mem_and_left_ptr_and_right_ptr
      in
...
let rec ref_split_rec
  ...
  match mem_get mem curr_ptr with
  | Root _ ->
      ( Ligo.failwith internalError_RefSplitRecFoundRoot
        : mem * ptr option * ptr option )
  | Leaf leaf ->
      if Ligo.leq_tez_tez leaf.value.contents.tez limit then
        (* Case 1a. Single leaf with not too much tez in it. Include it. *)
        let mem = mem_update mem curr_ptr (node_set_parent ptr_null) in
        left_fold_ref_split_data
          ((mem, Some curr_ptr, (None : ptr option)), stack)
      else
        (* Case 1b. Single leaf with too much tez in it. Exclude it. *)
        left_fold_ref_split_data
          ((mem, (None : ptr option), Some curr_ptr), stack)
  | Branch branch ->
      if
        Ligo.leq_tez_tez
          (Ligo.add_tez_tez branch.left_tez branch.right_tez)
          limit
      then
        (* total_tez <= limit *)
        ...
        if branch.left_tez = limit then
          (* Case 3a. left_tez = limit (no need for recursion, split the tree right here) *)
          left_fold_ref_split_data
            ((mem, Some branch.left, Some branch.right), stack)
        else
          let rec_direction, tree_to_recurse_into, limit_to_use =
            if Ligo.lt_tez_tez limit branch.left_tez then
              (* Case 3b. limit < left_tez < total_tez (we have to recurse into and split the left tree) *)
              (Left, branch.left, limit)
            else
              (* Case 3c. left_tez < limit < total_tez (we have to recurse into and split the right tree) *)
              let left_branch = mem_get mem branch.left in
              ( Right,
                branch.right,
                Ligo.sub_tez_tez limit (node_tez left_branch) )
          in
          ref_split_rec
            ( mem,
              tree_to_recurse_into,
              limit_to_use,
              { rec_direction; branch } :: stack )

let ref_split (mem : mem) (curr_ptr : ptr) (limit : Ligo.tez)
    : mem * ptr option * ptr option
  =
  ref_split_rec (mem, curr_ptr, limit, ([] : ref_split_data list))
 ...
 * given amount of tez.
 *)
let avl_take (mem : mem) (root_ptr : avl_ptr) (limit : Ligo.tez)
    (root_data : auction_outcome option)
    : mem * avl_ptr
  =
...
let[@inline] avl_root_data (mem : mem) (ptr : avl_ptr) : auction_outcome option =
  let _, d = deref_avl_ptr mem ptr in
  d

let[@inline] avl_modify_root_data (mem : mem) (ptr : avl_ptr)
    (f : auction_outcome option -> auction_outcome option)
  =
...
let avl_append (mem : mem) (AVLPtr left_ptr) (AVLPtr right_ptr) : mem =
  let mem =
    match (mem_get mem left_ptr, mem_get mem right_ptr) with
    | Root _, Root (None, _) -> mem
...
let assert_avl_invariants (mem : mem) (AVLPtr root) : unit =
  let rec go (parent : ptr) (curr : ptr) =
    ...
    | Leaf leaf -> assert (leaf.parent = parent)
    | Branch branch ->
