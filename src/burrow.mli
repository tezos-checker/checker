...
val burrow_return_kit_from_auction
  :  LiquidationAuctionPrimitiveTypes.liquidation_slice_contents ->
  kit ->
  burrow ->
  burrow
...
(* NOTE: the liquidation slice must be the one pointed to by the leaf pointer. *)
val burrow_return_slice_from_auction
  :  LiquidationAuctionPrimitiveTypes.liquidation_slice_contents ->
  burrow ->
  burrow
...
val compute_min_kit_for_unwarranted
  :  parameters ->
  burrow ->
  Ligo.tez ->
  kit option
