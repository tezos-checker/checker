...
let initial_wrapper (addr : Ligo.address) =
  {
    lazy_functions =
      (Ligo.Big_map.empty : (lazy_function_id, Ligo.bytes) Ligo.big_map);
...
let main ((op, state) : params * wrapper) : LigoOp.operation list * wrapper =
  let { lazy_functions; metadata; deployment_state } = state in

  let ops, lazy_functions, metadata, deployment_state =
    match deployment_state with
    | Unsealed deployer -> (
        match !Ligo.Tezos.sender = deployer with
        | true -> (
            match op with
            | DeployFunction p ->
                let lfi, bs = p in
                let lazy_functions =
                  ...
                  | Some prev ->
                      Ligo.Big_map.add lfi
                        (Ligo.Bytes.concat prev bs)
                        lazy_functions
                in
            ...
            | DeployMetadata bs ->
                let metadata =
                  ...
                  | Some prev ->
                      Ligo.Big_map.add "m" (Ligo.Bytes.concat prev bs) metadata
                in
                ( ([] : LigoOp.operation list),
                  lazy_functions,
                  metadata,
            ...
            | SealContract (oracle_addr, ctez_addr) ->
                let external_contracts =
                  { oracle = oracle_addr; ctez = ctez_addr }
                in
                ...
                let touchOp =
                  match
                    ( LigoOp.Tezos.get_entrypoint_opt "%touch"
                  ...
                  | Some c ->
                      LigoOp.Tezos.unit_transaction ()
                        ...
                        c
                  | None ->
                      ( Ligo.failwith (Ligo.int_from_literal "-4")
                        : LigoOp.operation )
                in
                ...
                let metadata_url =
                  Ligo.bytes_from_literal "0x74657a6f732d73746f726167653a6d"
                in
                let metadata = Ligo.Big_map.add "" metadata_url metadata in

                ([ touchOp ], lazy_functions, metadata, Sealed checker)
            | CheckerEntrypoint _ ->
                ( Ligo.failwith error_ContractNotDeployed
                  : LigoOp.operation list
                    * lazy_function_map
                    * (string, Ligo.bytes) Ligo.big_map
                    * deployment_state ) )
        | false ->
            ( Ligo.failwith error_UnauthorisedCaller
              : LigoOp.operation list
                * lazy_function_map
                * (string, Ligo.bytes) Ligo.big_map
                * deployment_state ) )
    | Sealed checker ->
        let ops, checker =
          match op with
          | DeployFunction _ ->
              ( Ligo.failwith error_ContractAlreadyDeployed
  ...
  in
  (ops, { lazy_functions; metadata; deployment_state })
