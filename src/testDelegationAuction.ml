  ...
  "Delegation auction tests" >::: [
    ...
    ("test single bidder" >::
     fun _ ->
       ...
       let (ticket, auction) = delegation_auction_place_bid auction bidder amount for_delegate in
       ...
       assert_raises
         ...
         (fun () -> delegation_auction_claim_win auction ticket);
       ...
       assert_raises
         ...
         (fun () -> delegation_auction_reclaim_bid auction ticket);
       let auction = delegation_auction_claim_win auction ticket in
       let auction = delegation_auction_touch auction in
       let delegate = delegation_auction_delegate auction in
    ...
    ("test outbidding" >::
     fun _ ->
       ...
       let for_delegate1 = Ligo.key_hash_from_literal "delegate1111" in
       let for_delegate2 = Ligo.key_hash_from_literal "delegate2222" in
       let for_delegate3 = Ligo.key_hash_from_literal "delegate3333" in
       ...
       let (ticket1, auction) = delegation_auction_place_bid auction bidder1 amount1 for_delegate1 in
       assert_raises
         (Failure "BidTooLow")
         (fun () -> delegation_auction_place_bid auction bidder2 amount1 for_delegate2);

       let (ticket2, auction) = delegation_auction_place_bid auction bidder2 amount2 for_delegate2 in
       let (ticket3, auction) = delegation_auction_place_bid auction bidder3 amount3 for_delegate3 in
       let (ticket4, auction) = delegation_auction_place_bid auction bidder4 amount4 for_delegate4 in
       ...
       let auction = delegation_auction_claim_win auction ticket4 in
       let auction = delegation_auction_touch auction in
       let delegate = delegation_auction_delegate auction in
    ...
    ("test sanity when skipping multiple levels" >::
     fun _ ->
       ...
       let auction = delegation_auction_empty in
       let bidder = Ligo.address_from_literal "5678" in
       let for_delegate = Ligo.key_hash_from_literal "asdfalkjlsjdf" in
       let amount = Ligo.tez_from_literal "1mutez" in
       let (ticket, auction) = delegation_auction_place_bid auction bidder amount for_delegate in
       (* And in the subsequent cycle they cease to be the delegate again *)
