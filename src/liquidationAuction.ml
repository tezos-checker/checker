...
let liquidation_auction_current_auction_tez (auctions : liquidation_auctions)
    : Ligo.tez option
  =
...
let fold_burrow_slices ?(direction = FromYoungest)
    (f : 'a -> liquidation_slice_contents -> 'a) (acc : 'a) (avl_storage : mem)
...
let assert_burrow_slices_invariants auctions burrow_id =
  ...
  let collect_slices acc slice = List.append acc [ slice ] in
  let slices_using_oldest_ptr =
    fold_burrow_slices ~direction:FromOldest collect_slices []
      auctions.avl_storage burrow_slices
  in
  let slices_using_youngest_ptr =
    fold_burrow_slices ~direction:FromYoungest collect_slices []
      auctions.avl_storage burrow_slices
  in
  ...
  List.iter
    (fun slice -> assert (slice.burrow = burrow_id))
    slices_using_oldest_ptr

(* Checks if some invariants of auctions structure holds. *)
let assert_liquidation_auction_invariants (auctions : liquidation_auctions)
    : unit
  =
  ...
  let roots =
    ...
    |> List.filter (fun (_, n) ->
           match n with
           | LiquidationAuctionPrimitiveTypes.Root _ -> true
           | _ -> false)
    |> List.map (fun (p, _) -> AVLPtr p)
  in
  ...
  |> Option.iter (fun completed_auctions ->
         let rec go (curr : avl_ptr) (prev : avl_ptr option) =
           ...
           | None -> assert (curr = completed_auctions.oldest)
         in
  ...
  let nodes =
    Ligo.Big_map.bindings mem.mem
    |> List.filter_map (fun (_, n) ->
           match n with
           | LiquidationAuctionPrimitiveTypes.Leaf l -> Some l
           | _ -> None)
  in
  let _ =
    List.iter
      (fun leaf ->
        assert (leaf.value.contents.tez <> Ligo.tez_from_literal "0mutez"))
      nodes
  in
  ...
  let _ =
    List.map
      (fun (burrow_id, _) -> assert_burrow_slices_invariants auctions burrow_id)
      (Ligo.Big_map.bindings auctions.burrow_slices)
  in
 ...
 *)
let liquidation_auction_send_to_auction (auctions : liquidation_auctions)
  ...
  if
    Ligo.geq_nat_nat
      ...
      max_liquidation_queue_height
  then
    ( Ligo.failwith error_LiquidationQueueTooLong
      : liquidation_auctions * leaf_ptr )
  else
    let burrow_slices =
      slice_list_from_auction_state auctions contents.burrow
    in
    let auctions, burrow_slices, SliceListElement (ret, _) =
      slice_list_append burrow_slices auctions auctions.queued_slices QueueBack
        contents
    in
...
let split_liquidation_slice_contents (amnt : Ligo.tez)
    (contents : liquidation_slice_contents)
    : liquidation_slice_contents * liquidation_slice_contents
  =
  let {
    burrow = contents_burrow;
    tez = contents_tez;
  ...
  let lkit, rkit =
    ...
    | Some contents_min_kit_for_unwarranted ->
        let min_kit_for_unwarranted =
          kit_to_mukit_nat contents_min_kit_for_unwarranted
        in
        let lkit =
...
let take_with_splitting (auctions : liquidation_auctions)
    (split_threshold : Ligo.tez)
    : liquidation_auctions * avl_ptr (* liquidation_auction_id *)
  =
  let queued_slices = auctions.queued_slices in

  let storage, new_auction =
    avl_take auctions.avl_storage queued_slices split_threshold
      (None : auction_outcome option)
  in
  let auctions = { auctions with avl_storage = storage } in

  let queued_amount = avl_tez storage new_auction in
  let auctions =
    if Ligo.lt_tez_tez queued_amount split_threshold then
      (* split next thing *)
      ...
      | Some (element, burrow_slices) ->
          ...
          let part1_contents, part2_contents =
            split_liquidation_slice_contents
              (Ligo.sub_tez_tez split_threshold queued_amount)
              (slice_list_element_contents element)
          in
          (* Remove the element we are splitting *)
          let auctions, burrow_slices, _, _ =
            slice_list_remove burrow_slices auctions element
          in
          (* Push the first portion of the slice to the back of the new auction *)
          let auctions, burrow_slices, _ =
            slice_list_append burrow_slices auctions new_auction QueueBack
              part1_contents
          in
          (* Push the remainder of the slice to the front of the auction queue *)
          let auctions, burrow_slices, _ =
            slice_list_append burrow_slices auctions queued_slices QueueFront
              part2_contents
          in
...
let start_liquidation_auction_if_possible (start_price : ratio)
  ...
  | None ->
      ...
      in
      let auctions, new_auction =
        take_with_splitting auctions split_threshold
      in
      if avl_is_empty auctions.avl_storage new_auction then
        ...
        { auctions with avl_storage = storage; current_auction }
      else
        let start_value =
          let { num = num_sp; den = den_sp } = start_price in
          kit_of_fraction_ceil
            (Ligo.mul_int_int
               (tez_to_mutez (avl_tez auctions.avl_storage new_auction))
               den_sp)
        ...
        in
        let auctions = { auctions with current_auction } in
        assert_liquidation_auction_invariants auctions;
        auctions
...
(** Compute the current threshold for a bid to be accepted. For a descending
  ...
  * a fixed factor. *)
let liquidation_auction_current_auction_minimum_bid
    (auction : current_liquidation_auction)
    : kit
  =
  kit_max
    ...
    | Descending (start_value, start_time) ->
        ...
        let time_passed =
          Ligo.sub_timestamp_timestamp !Ligo.Tezos.now start_time
        in
        let seconds_passed = Ligo.abs time_passed in
        assert (Some seconds_passed = Ligo.is_nat time_passed);
        (* should be non-negative *)
        let decay =
          fixedpoint_pow
            (fixedpoint_sub fixedpoint_one auction_decay_rate)
            seconds_passed
        in
        kit_scale start_value decay
    | Ascending (leading_bid, _timestamp, _level) ->
        let bid_improvement_factor =
          fixedpoint_of_ratio_floor bid_improvement_factor
        in
        kit_scale leading_bid.kit
          (fixedpoint_add fixedpoint_one bid_improvement_factor) )
...
let is_liquidation_auction_complete (auction_state : liquidation_auction_state)
    : bid option
  =
...
let complete_liquidation_auction_if_possible (auctions : liquidation_auctions)
    : liquidation_auctions
  =
  let auctions =
    match auctions.current_auction with
    ...
    | Some curr -> (
        ...
        | None -> auctions
        | Some winning_bid ->
            let storage, completed_auctions =
              ...
              | None ->
                  ...
                  in
                  let storage =
                    avl_modify_root_data auctions.avl_storage curr.contents
              ...
              | Some params ->
                  ...
                  in
                  let storage =
                    avl_modify_root_data storage youngest
                      (fun (prev : auction_outcome option) ->
                        match prev with
                        | None ->
                            ( Ligo.failwith
                                internalError_CompletedAuctionWithoutOutcome
                              : auction_outcome option )
                        | Some xs ->
                            Some
                              { xs with younger_auction = Some curr.contents })
                  in
                  (storage, { youngest = curr.contents; oldest })
            in
            {
              auctions with
              avl_storage = storage;
...
let liquidation_auction_place_bid (auction : current_liquidation_auction)
    (bid : bid)
    : current_liquidation_auction * bid option
  =
  if
    geq_kit_kit bid.kit
      (liquidation_auction_current_auction_minimum_bid auction)
  then
    let updated_auction =
      {
        auction with
        state = Ascending (bid, !Ligo.Tezos.now, !Ligo.Tezos.level);
      }
    in

    match auction.state with
    (* For descending auctions we don't have to return kit to anyone. *)
    | Descending _ -> (updated_auction, (None : bid option))
...
let liquidation_auction_get_current_auction (auctions : liquidation_auctions)
    : current_liquidation_auction
  =
  match auctions.current_auction with
 ...
 *)
let pop_slice (auctions : liquidation_auctions) (leaf_ptr : leaf_ptr)
    : liquidation_slice_contents * avl_ptr * liquidation_auctions
  =
  let element, burrow_slices = slice_list_from_leaf_ptr auctions leaf_ptr in
  let auctions, burrow_slices, root_ptr, contents =
    slice_list_remove burrow_slices auctions element
  in
  let auctions = slice_list_to_auction_state auctions burrow_slices in
  assert_liquidation_auction_invariants auctions;
  (contents, root_ptr, auctions)

let liquidation_auctions_cancel_slice (auctions : liquidation_auctions)
    (leaf_ptr : leaf_ptr)
    : liquidation_slice_contents * liquidation_auctions
  =
  assert_liquidation_auction_invariants auctions;
  ...
  if
    ptr_of_avl_ptr root <> ptr_of_avl_ptr auctions.queued_slices
    (* FIXME: I (Dorran) think that we might be overloading the term 'unwarranted' here? *)
  then
    ( Ligo.failwith error_UnwarrantedCancellation
      : liquidation_slice_contents * liquidation_auctions )
  else (contents, auctions)

let completed_liquidation_auction_won_by_sender (avl_storage : mem)
    (auction_id : liquidation_auction_id)
    : auction_outcome option
  =
  match avl_root_data avl_storage auction_id with
  | Some outcome ->
      if outcome.winning_bid.address = !Ligo.Tezos.sender then Some outcome
...
let liquidation_auction_pop_completed_auction (auctions : liquidation_auctions)
    (tree : avl_ptr)
    : liquidation_auctions
  =
  assert_liquidation_auction_invariants auctions;
  ...
  let outcome =
    match avl_root_data storage tree with
    | None ->
        ( Ligo.failwith internalError_PopCompletedAuctionAuctionNotCompleted
          : auction_outcome )
    | Some r -> r
  in
  let completed_auctions =
    match auctions.completed_auctions with
    | None ->
        ( Ligo.failwith internalError_PopCompletedAuctionNoCompletedAuction
          : completed_liquidation_auctions )
    | Some r -> r
  in
  ...
  let storage =
    ...
    | None -> storage
    | Some younger ->
        avl_modify_root_data storage younger
          (fun (i : auction_outcome option) ->
            let i =
              match i with
              | None ->
                  ( Ligo.failwith
                      internalError_PopCompletedAuctionCompletedAuctionNoOutcome
                    : auction_outcome )
              | Some i -> i
            in
            assert (i.older_auction = Some tree);
            Some { i with older_auction = outcome.older_auction })
  in
  let storage =
    match outcome.older_auction with
    | None -> storage
    | Some older ->
        avl_modify_root_data storage older (fun (i : auction_outcome option) ->
            ...
            Some { i with younger_auction = outcome.younger_auction })
  in

  let storage =
    avl_modify_root_data storage tree (fun (_ : auction_outcome option) ->
        Some
          {
            outcome with
            younger_auction = (None : liquidation_auction_id option);
  ...
  let auctions = { auctions with completed_auctions; avl_storage = storage } in
  assert_liquidation_auction_invariants auctions;
  auctions

let liquidation_auctions_pop_completed_slice (auctions : liquidation_auctions)
    (leaf_ptr : leaf_ptr)
    : liquidation_slice_contents * auction_outcome * liquidation_auctions
  =
  assert_liquidation_auction_invariants auctions;
  let contents, root, auctions = pop_slice auctions leaf_ptr in
  ...
  let auctions =
    if avl_is_empty auctions.avl_storage root then
      liquidation_auction_pop_completed_auction auctions root
    else auctions
  in
  let outcome =
...
let[@inline] liquidation_auction_claim_win (auctions : liquidation_auctions)
    (auction_id : liquidation_auction_id)
    : Ligo.tez * liquidation_auctions
  =
  assert_liquidation_auction_invariants auctions;
  let sold_tez, auctions =
    match
      completed_liquidation_auction_won_by_sender auctions.avl_storage
        auction_id
    with
    | Some outcome ->
        (* A winning bid can only be claimed when all the liquidation slices
         * for that lot is cleaned. *)
        if not (avl_is_empty auctions.avl_storage auction_id) then
          ( Ligo.failwith error_NotAllSlicesClaimed
            : Ligo.tez * liquidation_auctions )
        else (
          ...
          let auctions =
            {
              auctions with
              avl_storage =
                avl_delete_empty_tree auctions.avl_storage auction_id;
            }
          in
          (outcome.sold_tez, auctions) )
    | None ->
        (Ligo.failwith error_NotAWinningBid : Ligo.tez * liquidation_auctions)
  in
  assert_liquidation_auction_invariants auctions;
  (sold_tez, auctions)

let liquidation_auction_touch (auctions : liquidation_auctions) (price : ratio)
    : liquidation_auctions
  =
  assert_liquidation_auction_invariants auctions;
  let auctions =
    start_liquidation_auction_if_possible price
      (complete_liquidation_auction_if_possible auctions)
  in
  assert_liquidation_auction_invariants auctions;
  auctions
...
let liquidation_auction_oldest_completed_liquidation_slice
  ...
  | Some completed_auctions -> (
      ...
      | None ->
          ( Ligo.failwith internalError_OldestCompletedSliceEmptyCompletedAuction
            : leaf_ptr option )
      | Some p ->
          let leaf_ptr, _ = p in
          Some leaf_ptr )

let is_burrow_done_with_liquidations (auctions : liquidation_auctions)
    (burrow : burrow_id)
  =
  ...
  | Some bs -> (
      ...
      match outcome with None -> true | Some _ -> false )
