...
let kit_of_fraction_ceil (x_num : Ligo.int) (x_den : Ligo.int) : kit =
  ...
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_KitOfFractionCeilNegative : kit)
  else
    Ligo.abs
      (cdiv_int_int (Ligo.mul_int_int x_num kit_scaling_factor_int) x_den)

let kit_of_fraction_floor (x_num : Ligo.int) (x_den : Ligo.int) : kit =
  assert (Ligo.gt_int_int x_den (Ligo.int_from_literal "0"));
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_KitOfFractionFloorNegative : kit)
  else
    Ligo.abs
      (fdiv_int_int (Ligo.mul_int_int x_num kit_scaling_factor_int) x_den)
...
let[@inline] kit_to_ratio (amnt : kit) : ratio =
  make_ratio (Ligo.int amnt) kit_scaling_factor_int
