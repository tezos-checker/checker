...
let fminting : ratio = make_ratio (Ligo.int_from_literal "21") (Ligo.int_from_literal "10") (* 2.1 *)

(** Dimensionless. Factor used for setting the liquidation limit. *)
let fliquidation : ratio = make_ratio (Ligo.int_from_literal "19") (Ligo.int_from_literal "10") (* 1.9 *)
...
let liquidation_reward_percentage : fixedpoint =
  fixedpoint_of_ratio_floor (make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "1000")) (* 0.001 *)

(** Percentage kept by the uniswap contract from the return asset. *)
let uniswap_fee : ratio = make_ratio (Ligo.int_from_literal "2") (Ligo.int_from_literal "1000") (* 0.002 *)
...
let min_lot_auction_queue_fraction : fixedpoint =
  fixedpoint_of_ratio_floor (make_ratio (Ligo.int_from_literal "5") (Ligo.int_from_literal "100"))

(** The percentage of additional collateral that we charge when liquidating
  * a burrow, to penalize it for liquidation. *)
let liquidation_penalty : ratio = make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "10") (* 0.1 *)
...
let target_high_bracket : ratio = make_ratio (Ligo.int_from_literal "5") (Ligo.int_from_literal "100") (* 0.05 *)
...
let auction_decay_rate : ratio = make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "6000") (* 0.01/60 *)
...
(** Every bid in an ascending auction needs to improve over the previous bid by
  * at least 0.33 cNp. *)
let bid_improvement_factor : ratio = make_ratio (Ligo.int_from_literal "33") (Ligo.int_from_literal "10000") (* 0.33/100 *)
...
let touch_low_reward : ratio = make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "600") (* 0.1/60 *)

(** We want the reward in the first bracket to be 1 kit / minute, so we just
  * divide by 60 to get roughly how much should it be per second. *)
let touch_high_reward : ratio = make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "60") (* 1/60 *)
