...
let[@inline] fminting : ratio =
  make_ratio (Ligo.int_from_literal "21") (Ligo.int_from_literal "10")

(* 2.1 *)

(** Dimensionless. Factor used for setting the liquidation limit. *)
let[@inline] fliquidation : ratio =
  make_ratio (Ligo.int_from_literal "19") (Ligo.int_from_literal "10")

(* 1.9 *)

(** Number of tez needed to be given for the creation of a burrow; it does
    not count towards the burrow's collateral. *)
let[@inline] creation_deposit : Ligo.tez =
  Ligo.tez_from_literal "1_000_000mutez"

(** Yearly burrow fee percentage. *)
let[@inline] burrow_fee_percentage : ratio =
  make_ratio (Ligo.int_from_literal "5") (Ligo.int_from_literal "1000")

(* 0.005 *)

(** Factor used to scale down the imbalance rate. The higher the value, the
    faster the imbalance rate saturates. *)
let[@inline] imbalance_scaling_factor : ratio =
  make_ratio (Ligo.int_from_literal "3") (Ligo.int_from_literal "4")

(* 0.75 *)

(** Maximum yearly imbalance rate. *)
let[@inline] imbalance_limit =
...
let[@inline] liquidation_reward_percentage : ratio =
  make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "1000")

(* 0.001 *)

(** Percentage kept by the cfmm contract from the return asset. *)
let[@inline] cfmm_fee : ratio =
  make_ratio (Ligo.int_from_literal "2") (Ligo.int_from_literal "1000")
...
let[@inline] protected_index_inverse_epsilon : Ligo.int =
  Ligo.int_from_literal "120_000"

(** The maximum number of tez that can be in an auction lot. *)
let[@inline] max_lot_size : Ligo.tez =
  Ligo.tez_from_literal "10_000_000_000mutez"
...
let[@inline] liquidation_penalty : ratio =
  make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "10")
...
let[@inline] target_low_bracket : ratio =
  make_ratio (Ligo.int_from_literal "5") (Ligo.int_from_literal "1000")

(* 0.005 *)

(** High bracket used for the calculation of the drift derivative. *)
let[@inline] target_high_bracket : ratio =
  make_ratio (Ligo.int_from_literal "5") (Ligo.int_from_literal "100")
...
*)
let[@inline] low_positive_acceleration : fixedpoint =
  fixedpoint_of_raw (Ligo.int_from_literal "247111")

let[@inline] low_negative_acceleration : fixedpoint =
  fixedpoint_of_raw (Ligo.int_from_literal "-247111")

let[@inline] high_positive_acceleration : fixedpoint =
  fixedpoint_of_raw (Ligo.int_from_literal "1235555")

let[@inline] high_negative_acceleration : fixedpoint =
  fixedpoint_of_raw (Ligo.int_from_literal "-1235555")
...
let[@inline] auction_decay_rate : ratio =
  make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "6000")
...
let[@inline] max_bid_interval_in_seconds : Ligo.int =
  Ligo.int_from_literal "1200"
...
(** Every bid in an ascending auction needs to improve over the previous bid by
    at least 0.33 cNp. *)
let[@inline] bid_improvement_factor : ratio =
  make_ratio (Ligo.int_from_literal "33") (Ligo.int_from_literal "10000")
...
let[@inline] touch_reward_low_bracket : Ligo.int = Ligo.int_from_literal "600"
...
let[@inline] touch_low_reward : ratio =
  make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "600")

(* 0.1/60 *)

(** We want the reward in the first bracket to be 1 kit / minute, so we just
    divide by 60 to get roughly how much should it be per second. *)
let[@inline] touch_high_reward : ratio =
  make_ratio (Ligo.int_from_literal "1") (Ligo.int_from_literal "60")
...
*)
let[@inline] max_liquidation_queue_height : Ligo.nat =
  Ligo.nat_from_literal "12n"
