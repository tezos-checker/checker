...
let assert_checker_invariants (state : checker) : unit =
  ...
  assert (
    fa2_get_total_kit_balance state.fa2_state = state.parameters.circulating_kit
  );
  ...
  assert (
    geq_kit_kit
      (kit_add
         (kit_of_mukit
            (fa2_get_balance
               (state.fa2_state, !Ligo.Tezos.self_address, kit_token_id)))
  ...
  assert (
    lqt_add
      (fa2_get_total_lqt_balance state.fa2_state)
  ...
  List.iter
    (fun (burrow_address, burrow) ->
      ...
      match
        Ligo.Big_map.find_opt burrow_address
          state.liquidation_auctions.burrow_slices
      with
      | None ->
          assert (
            burrow_collateral_at_auction burrow = Ligo.tez_from_literal "0mutez"
          )
      | Some slices ->
          (* Check if the linked list of slices are correct, and the amount of
           * tez inside is consistent with collateral_at_auction.
           *)
          let rec go (curr : leaf_ptr) (prev : leaf_ptr option) : Ligo.tez =
            let slice =
              avl_read_leaf state.liquidation_auctions.avl_storage curr
            in
...
let[@inline] get_transfer_ctez_entrypoint
    (external_contracts : external_contracts)
    : fa12_transfer Ligo.contract
  =
  match
    ( LigoOp.Tezos.get_entrypoint_opt "%transfer" external_contracts.ctez
      : fa12_transfer Ligo.contract option )
  with
  | Some c -> c
  | None ->
      ( Ligo.failwith error_GetEntrypointOptFailureFA12Transfer
        : fa12_transfer Ligo.contract )

let[@inline] get_oracle_entrypoint (external_contracts : external_contracts)
    : Ligo.nat Ligo.contract Ligo.contract
  =
  match
    ( LigoOp.Tezos.get_entrypoint_opt oracle_entrypoint external_contracts.oracle
      : Ligo.nat Ligo.contract Ligo.contract option )
  with
  | Some c -> c
  | None ->
      ( Ligo.failwith error_GetEntrypointOptFailureOracleEntrypoint
        : Ligo.nat Ligo.contract Ligo.contract )
 ...
 * any operation. *)
let ensure_burrow_has_no_unclaimed_slices (auctions : liquidation_auctions)
    (burrow_id : burrow_id)
    : unit
  =
  if is_burrow_done_with_liquidations auctions burrow_id then ()
...
let ensure_no_tez_given () =
  if !Ligo.Tezos.amount <> Ligo.tez_from_literal "0mutez" then
    Ligo.failwith error_UnwantedTezGiven
  else ()
...
let[@inline] entrypoint_create_burrow
    ((state, (burrow_no, delegate_opt)) :
      checker * (Ligo.nat * Ligo.key_hash option))
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
  let () =
    if Ligo.Big_map.mem burrow_id state.burrows then
      Ligo.failwith error_BurrowAlreadyExists
    else ()
  in
    ...
    LigoOp.Tezos.create_contract
      (fun ((p, storage) : burrow_parameter * burrow_storage) ->
        if !Ligo.Tezos.sender <> storage.checker_address then
          ( Ligo.failwith (Ligo.int_from_literal "-1")
            : LigoOp.operation list * burrow_storage )
        else
          match p with
          | BurrowSetDelegate kho -> ([ LigoOp.Tezos.set_delegate kho ], storage)
          | BurrowStoreTez -> (([] : LigoOp.operation list), storage)
          | BurrowSendTezTo p ->
              let tez, addr = p in
              let op =
                match
                  ( LigoOp.Tezos.get_contract_opt addr
                    : unit Ligo.contract option )
                with
                | Some c -> LigoOp.Tezos.unit_transaction () tez c
                | None ->
                    ( Ligo.failwith (Ligo.int_from_literal "-2")
                      : LigoOp.operation )
              in
              ([ op ], storage)
          | BurrowSendSliceToChecker tz ->
              let op =
                match
                  ( LigoOp.Tezos.get_entrypoint_opt "%receive_slice_from_burrow"
                      !Ligo.Tezos.sender
                    : burrow_id Ligo.contract option )
                with
                | Some c ->
                    LigoOp.Tezos.address_nat_transaction storage.burrow_id tz c
                | None ->
                    ( Ligo.failwith (Ligo.int_from_literal "-3")
                      : LigoOp.operation )
              in
              ([ op ], storage))
      delegate_opt !Ligo.Tezos.amount
      (* NOTE!!! The creation deposit is in the burrow too, even if we don't consider it to be collateral! *)
      { checker_address = !Ligo.Tezos.self_address; burrow_id }
  in

  let burrow =
    burrow_create state.parameters burrow_address !Ligo.Tezos.amount
      delegate_opt
  in
  let state =
    {
...
let entrypoint_touch_burrow ((state, burrow_id) : checker * burrow_id)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
...
let entrypoint_deposit_tez ((state, burrow_no) : checker * Ligo.nat)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
  let burrow = find_burrow state.burrows burrow_id in
  let _ =
    ensure_burrow_has_no_unclaimed_slices state.liquidation_auctions burrow_id
  in
  let burrow = burrow_deposit_tez state.parameters !Ligo.Tezos.amount burrow in
  let op =
    match
      ( LigoOp.Tezos.get_entrypoint_opt "%burrowStoreTez" (burrow_address burrow)
        : unit Ligo.contract option )
    with
    | Some c -> LigoOp.Tezos.unit_transaction () !Ligo.Tezos.amount c
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowStoreTez
          : LigoOp.operation )
  in
  let state =
    {
...
let entrypoint_mint_kit ((state, (burrow_no, kit)) : checker * (Ligo.nat * kit))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
...
let entrypoint_withdraw_tez
    ((state, (tez, burrow_no)) : checker * (Ligo.tez * Ligo.nat))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
  ...
  let op =
    match
      ( LigoOp.Tezos.get_entrypoint_opt "%burrowSendTezTo"
          (burrow_address burrow)
        : (Ligo.tez * Ligo.address) Ligo.contract option )
    with
    | Some c ->
        LigoOp.Tezos.tez_address_transaction (tez, !Ligo.Tezos.sender)
          (Ligo.tez_from_literal "0mutez")
          c
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowSendTezTo
          : LigoOp.operation )
  in
  assert_checker_invariants state;
  ([ op ], state)

let entrypoint_burn_kit ((state, (burrow_no, kit)) : checker * (Ligo.nat * kit))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
...
let entrypoint_activate_burrow ((state, burrow_no) : checker * Ligo.nat)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow_id = (!Ligo.Tezos.sender, burrow_no) in
  ...
  let _ =
    ensure_burrow_has_no_unclaimed_slices state.liquidation_auctions burrow_id
  in
  let burrow = burrow_activate state.parameters !Ligo.Tezos.amount burrow in
  let op =
    match
      ( LigoOp.Tezos.get_entrypoint_opt "%burrowStoreTez" (burrow_address burrow)
        : unit Ligo.contract option )
...
let entrypoint_deactivate_burrow
    ((state, (burrow_no, receiver)) : checker * (Ligo.nat * Ligo.address))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  ...
  let _ =
    ensure_burrow_has_no_unclaimed_slices state.liquidation_auctions burrow_id
  in
  let burrow, returned_tez = burrow_deactivate state.parameters burrow in
  let state =
    {
      state with
      burrows = Ligo.Big_map.update burrow_id (Some burrow) state.burrows;
  ...
  let op =
    ...
    with
    | Some c ->
        LigoOp.Tezos.tez_address_transaction (returned_tez, receiver)
          (Ligo.tez_from_literal "0mutez")
          c
        (* NOTE: returned_tez inlcudes creation deposit! *)
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowSendTezTo
          : LigoOp.operation )
  in
...
let entrypoint_set_burrow_delegate
    ((state, (burrow_no, delegate_opt)) :
      checker * (Ligo.nat * Ligo.key_hash option))
    : LigoOp.operation list * checker
  =
  ...
  let op =
    match
      ( LigoOp.Tezos.get_entrypoint_opt "%burrowSetDelegate"
          (burrow_address burrow)
        : Ligo.key_hash option Ligo.contract option )
    with
    | Some c ->
        LigoOp.Tezos.opt_key_hash_transaction delegate_opt
          (Ligo.tez_from_literal "0mutez")
          c
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowSetDelegate
          : LigoOp.operation )
  in
  let state =
    {
...
let[@inline] entrypoint_mark_for_liquidation
    ((state, burrow_id) : checker * burrow_id)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  ...
  let { liquidation_reward; tez_to_auction; burrow_state = burrow } =
    ...
    | Some type_and_details ->
        let _, details = type_and_details in
        details
  in
      ...
      let contents =
        {
          ...
          min_kit_for_unwarranted =
            compute_min_kit_for_unwarranted state.parameters burrow
              tez_to_auction;
        }
      in
      let updated_liquidation_auctions, _leaf_ptr =
        liquidation_auction_send_to_auction state.liquidation_auctions contents
      in
      {
        state with
        burrows = Ligo.Big_map.update burrow_id (Some burrow) state.burrows;
  ...
  let op =
    ...
    | Some c ->
        LigoOp.Tezos.tez_address_transaction
          (liquidation_reward, !Ligo.Tezos.sender)
          (Ligo.tez_from_literal "0mutez")
          c
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowSendTezTo
...
let entrypoint_cancel_liquidation_slice ((state, leaf_ptr) : checker * leaf_ptr)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  let _ =
    ensure_valid_leaf_ptr state.liquidation_auctions.avl_storage leaf_ptr
  in
  let cancelled, auctions =
    liquidation_auctions_cancel_slice state.liquidation_auctions leaf_ptr
  in
  ...
  let _ =
    ...
    then ()
    else Ligo.failwith error_UnwarrantedCancellation
  in
...
let touch_liquidation_slice (ops : LigoOp.operation list)
    (auctions : liquidation_auctions) (state_burrows : burrow_map)
    (leaf_ptr : leaf_ptr)
  ...
  let slice, outcome, auctions =
    liquidation_auctions_pop_completed_slice auctions leaf_ptr
  in
  ...
  let kit_to_repay, kit_to_burn =
    let corresponding_kit =
      kit_of_fraction_floor
        (Ligo.mul_int_nat (tez_to_mutez slice.tez)
           (kit_to_mukit_nat outcome.winning_bid.kit))
        (Ligo.mul_int_int
           (tez_to_mutez outcome.sold_tez)
    ...
    let penalty =
      ...
      let liquidation_was_warranted =
        ...
        | None -> true
        | Some min_kit_for_unwarranted ->
            lt_kit_kit corresponding_kit min_kit_for_unwarranted
      in
  ...
  let state_burrows =
    Ligo.Big_map.update slice.burrow
  ...
  let op =
    match
      ( LigoOp.Tezos.get_entrypoint_opt "%burrowSendSliceToChecker"
          (burrow_address burrow)
        : Ligo.tez Ligo.contract option )
    with
    | Some c ->
        LigoOp.Tezos.tez_transaction slice.tez
          (Ligo.tez_from_literal "0mutez")
          c
    | None ->
        ( Ligo.failwith error_GetEntrypointOptFailureBurrowSendSliceToChecker
          : LigoOp.operation )
  in
...
let rec touch_liquidation_slices_rec
  ...
  | x :: xs ->
      let ( new_ops,
            ...
            new_kit_to_burn )
        =
        touch_liquidation_slice ops state_liquidation_auctions state_burrows x
      in
 ...
 * the order the input slices were processed in). However, since the operations
 * computed are independent from each other, this needs not be a problem. *)
let[@inline] entrypoint_touch_liquidation_slices
    ((state, slices) : checker * leaf_ptr list)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  ...
  let state_parameters =
    let state_parameters =
      remove_outstanding_and_circulating_kit state_parameters kit_to_repay
    in
    let state_parameters =
      remove_circulating_kit state_parameters kit_to_burn
    in
    state_parameters
  in
  ...
  let state_fa2_state =
    let state_fa2_state =
      ledger_withdraw_kit
        (state_fa2_state, !Ligo.Tezos.self_address, kit_to_repay)
    in
    let state_fa2_state =
      ledger_withdraw_kit
        (state_fa2_state, !Ligo.Tezos.self_address, kit_to_burn)
    in
    state_fa2_state
  in
...
let entrypoint_buy_kit ((state, p) : checker * (ctez * kit * Ligo.timestamp))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let ctez, min_kit_expected, deadline = p in
  let _ = ensure_no_tez_given () in
  let kit_tokens, updated_cfmm =
    cfmm_buy_kit state.cfmm ctez min_kit_expected deadline
  in
  let transfer =
    {
  ...
  in
  let op =
    LigoOp.Tezos.fa12_transfer_transaction transfer
      (Ligo.tez_from_literal "0mutez")
      (get_transfer_ctez_entrypoint state.external_contracts)
  in

  let state_fa2_state =
    let state_fa2_state = state.fa2_state in
    let state_fa2_state =
      ledger_withdraw_kit (state_fa2_state, !Ligo.Tezos.self_address, kit_tokens)
    in
    let state_fa2_state =
      ledger_issue_kit (state_fa2_state, !Ligo.Tezos.sender, kit_tokens)
    in
    state_fa2_state
  in

  assert (
    fa2_get_total_kit_balance state.fa2_state
    = fa2_get_total_kit_balance state_fa2_state );
  (* preservation of kit *)
  assert (
    fa2_get_total_lqt_balance state.fa2_state
    = fa2_get_total_lqt_balance state_fa2_state );
...
let entrypoint_sell_kit ((state, p) : checker * (kit * ctez * Ligo.timestamp))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let kit, min_ctez_expected, deadline = p in
  let _ = ensure_no_tez_given () in
  let ctez, updated_cfmm =
    cfmm_sell_kit state.cfmm kit min_ctez_expected deadline
  in
  let transfer =
    {
  ...
  let state_fa2_state =
    let state_fa2_state = state.fa2_state in
    let state_fa2_state =
      ledger_withdraw_kit (state_fa2_state, !Ligo.Tezos.sender, kit)
    in
    let state_fa2_state =
      ledger_issue_kit (state_fa2_state, !Ligo.Tezos.self_address, kit)
    in
    state_fa2_state
  in
  ...
  (* preservation of lqt *)
  let state = { state with cfmm = updated_cfmm; fa2_state = state_fa2_state } in
  assert_checker_invariants state;

  ([ op ], state)

let entrypoint_add_liquidity
    ((state, p) : checker * (ctez * kit * lqt * Ligo.timestamp))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  ...
  let lqt_tokens, kit_tokens, updated_cfmm =
    cfmm_add_liquidity state.cfmm ctez_deposited max_kit_deposited
      min_lqt_minted deadline
  in
  let transfer =
    {
      address_from = !Ligo.Tezos.sender;
  ...
  let state_fa2_state =
    let state_fa2_state = state.fa2_state in
    let state_fa2_state =
      ledger_withdraw_kit (state_fa2_state, !Ligo.Tezos.sender, deposited_kit)
    in
    let state_fa2_state =
      ledger_issue_kit (state_fa2_state, !Ligo.Tezos.self_address, deposited_kit)
    in
    let state_fa2_state =
      ledger_issue_lqt (state_fa2_state, !Ligo.Tezos.sender, lqt_tokens)
    in
    (* create *)
    state_fa2_state
  in
  ...
  (* preservation of kit *)
  assert (
    lqt_add (fa2_get_total_lqt_balance state.fa2_state) lqt_tokens
    = fa2_get_total_lqt_balance state_fa2_state );
...
let entrypoint_remove_liquidity
    ((state, p) : checker * (lqt * ctez * kit * Ligo.timestamp))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  ...
  let ctez, kit_tokens, updated_cfmm =
    cfmm_remove_liquidity state.cfmm lqt_burned min_ctez_withdrawn
      min_kit_withdrawn deadline
  in
  let transfer =
    {
      address_from = !Ligo.Tezos.self_address;
  ...
  let state_fa2_state =
    let state_fa2_state = state.fa2_state in
    let state_fa2_state =
      ledger_withdraw_lqt (state_fa2_state, !Ligo.Tezos.sender, lqt_burned)
    in
    (* destroy *)
    let state_fa2_state =
      ledger_withdraw_kit (state_fa2_state, !Ligo.Tezos.self_address, kit_tokens)
  ...
  assert (
    fa2_get_total_lqt_balance state.fa2_state
    = lqt_add (fa2_get_total_lqt_balance state_fa2_state) lqt_burned );
...
let entrypoint_liquidation_auction_place_bid
    ((state, (auction_id, kit)) : checker * (liquidation_auction_id * kit))
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in

  let bid = { address = !Ligo.Tezos.sender; kit } in
  let current_auction =
    liquidation_auction_get_current_auction state.liquidation_auctions
  in
  let () =
    if current_auction.contents = auction_id then ()
    else Ligo.failwith error_InvalidLiquidationAuction
  in

  let new_current_auction, old_winning_bid =
    liquidation_auction_place_bid current_auction bid
  in
  ...
  let state_fa2_state =
    ...
    let state_fa2_state =
      ...
      | Some old_winning_bid ->
          let state_fa2_state =
            ledger_withdraw_kit
              (state_fa2_state, !Ligo.Tezos.self_address, old_winning_bid.kit)
          in
          let state_fa2_state =
            ledger_issue_kit
              (state_fa2_state, old_winning_bid.address, old_winning_bid.kit)
          in
          state_fa2_state
    in
    assert (
...
let entrypoint_liquidation_auction_claim_win
    ((state, auction_id) : checker * liquidation_auction_id)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  let _ =
    ensure_valid_avl_ptr state.liquidation_auctions.avl_storage auction_id
  in
  let tez, liquidation_auctions =
    liquidation_auction_claim_win state.liquidation_auctions auction_id
  in
  let op =
    match
      ( LigoOp.Tezos.get_contract_opt !Ligo.Tezos.sender
        : unit Ligo.contract option )
    with
    | Some c -> LigoOp.Tezos.unit_transaction () tez c
    | None -> (Ligo.failwith error_GetContractOptFailure : LigoOp.operation)
  in
  let state = { state with liquidation_auctions } in
  assert_checker_invariants state;

  ([ op ], state)

let[@inline] entrypoint_receive_slice_from_burrow
    ((state, burrow_id) : checker * burrow_id)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let burrow = find_burrow state.burrows burrow_id in
  (* only accept from burrows! *)
  if !Ligo.Tezos.sender = burrow_address burrow then
    (([] : LigoOp.operation list), state)
  else
    (Ligo.failwith error_AuthenticationError : LigoOp.operation list * checker)
...
let calculate_touch_reward (last_touched : Ligo.timestamp) : kit =
  let duration_in_seconds =
    Ligo.sub_timestamp_timestamp !Ligo.Tezos.now last_touched
  in
  let low_duration = min_int duration_in_seconds touch_reward_low_bracket in
  let high_duration =
    max_int
      (Ligo.int_from_literal "0")
      (Ligo.sub_int_int duration_in_seconds touch_reward_low_bracket)
  in
...
let rec touch_oldest
  ...
  | Some maximum -> (
      match
        liquidation_auction_oldest_completed_liquidation_slice
          state_liquidation_auctions
      with
      | None ->
          ( ops,
...
let[@inline] touch_with_index (state : checker) (index : Ligo.nat)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let {
  ...
  in
  assert (
    Ligo.geq_timestamp_timestamp !Ligo.Tezos.now state.parameters.last_touched
  );
  let _ = ensure_no_tez_given () in
  if state_parameters.last_touched = !Ligo.Tezos.now then
     ...
     * cfmm sub-contract. *)
    let kit_in_tez_in_prev_block = cfmm_kit_in_ctez_in_prev_block state_cfmm in
    (* FIXME: times ctez_in_tez *)
    let total_accrual_to_cfmm, state_parameters =
      parameters_touch index kit_in_tez_in_prev_block state_parameters
    in
    ...
    let state_liquidation_auctions =
      liquidation_auction_touch state_liquidation_auctions
        (liquidation_price state_parameters)
    in
    ...
    let cb =
      match
        ( LigoOp.Tezos.get_entrypoint_opt "%receive_price"
            !Ligo.Tezos.self_address
          : Ligo.nat Ligo.contract option )
      with
      | Some cb -> cb
      | None ->
          ( Ligo.failwith error_GetEntrypointOptFailureReceivePrice
            : Ligo.nat Ligo.contract )
    in
    let op =
      LigoOp.Tezos.nat_contract_transaction cb
        (Ligo.tez_from_literal "0mutez")
    ...
    let state_parameters =
      {
        ...
        circulating_kit =
          kit_sub state_parameters.circulating_kit
            (kit_add kit_to_repay kit_to_burn);
      }
    in
    ...
    let state_fa2_state =
      let state_fa2_state =
        ledger_issue_kit (state_fa2_state, !Ligo.Tezos.sender, reward)
      in
      let state_fa2_state =
        ledger_issue_then_withdraw_kit
          ( state_fa2_state,
            !Ligo.Tezos.self_address,
...
let entrypoint_touch ((state, _) : checker * unit)
    : LigoOp.operation list * checker
  =
  let index =
    match state.last_price with
    | None -> state.parameters.index (* use the old one *)
    | Some i -> i
  in
  (* FIXME: Is the nat supposed to represent tez? *)
...
let entrypoint_receive_price ((state, price) : checker * Ligo.nat)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  ...
  else (([] : LigoOp.operation list), { state with last_price = Some price })
...
let strict_entrypoint_transfer ((state, xs) : checker * fa2_transfer list)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  let state =
    { state with fa2_state = fa2_run_transfer (state.fa2_state, xs) }
  in
  assert_checker_invariants state;
  (([] : LigoOp.operation list), state)

let[@inline] strict_entrypoint_balance_of
    ((state, param) : checker * fa2_balance_of_param)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  let { requests; callback } = param in
  let response = fa2_run_balance_of (state.fa2_state, requests) in
  let op =
    LigoOp.Tezos.fa2_balance_of_response_transaction response
      (Ligo.tez_from_literal "0mutez")
      callback
  in
  assert_checker_invariants state;
...
let entrypoint_update_operators
    ((state, xs) : checker * fa2_update_operator list)
    : LigoOp.operation list * checker
  =
  assert_checker_invariants state;
  let _ = ensure_no_tez_given () in
  let state =
    { state with fa2_state = fa2_run_update_operators (state.fa2_state, xs) }
  in
  assert_checker_invariants state;
  (([] : LigoOp.operation list), state)
...
let view_add_liquidity_max_kit_deposited ((ctez, state) : ctez * checker) : kit =
  assert_checker_invariants state;
  let _lqt, kit, _cfmm =
    cfmm_view_max_kit_deposited_min_lqt_minted_cfmm_add_liquidity state.cfmm
      ctez
  in
  kit
...
let view_remove_liquidity_min_ctez_withdrawn ((lqt, state) : lqt * checker)
    : ctez
  =
  assert_checker_invariants state;
  let ctez, _kit, _cfmm =
...
let view_burrow_max_mintable_kit ((burrow_id, state) : burrow_id * checker)
    : kit
  =
  assert_checker_invariants state;
  let burrow = find_burrow state.burrows burrow_id in
...
let view_is_burrow_overburrowed ((burrow_id, state) : burrow_id * checker)
    : bool
  =
  assert_checker_invariants state;
  let burrow = find_burrow state.burrows burrow_id in
...
let view_is_burrow_liquidatable ((burrow_id, state) : burrow_id * checker)
    : bool
  =
  assert_checker_invariants state;
  let burrow = find_burrow state.burrows burrow_id in
...
let view_current_liquidation_auction_minimum_bid (((), state) : unit * checker)
    : view_current_liquidation_auction_minimum_bid_result
  =
  assert_checker_invariants state;
  let auction =
    liquidation_auction_get_current_auction state.liquidation_auctions
...
let view_get_balance
    (((owner, token_id), state) : (Ligo.address * fa2_token_id) * checker)
    : Ligo.nat
  =
  assert_checker_invariants state;
...
let view_total_supply ((token_id, state) : fa2_token_id * checker) : Ligo.nat =
  ...
  if token_id = kit_token_id then
    ...
    lqt_to_denomination_nat
      (lqt_sub state.cfmm.lqt
         (lqt_of_denomination (Ligo.nat_from_literal "1n")))
...
let view_is_operator
    (((owner, (operator, token_id)), state) :
      (Ligo.address * (Ligo.address * fa2_token_id)) * checker)
    : bool
  =
  assert_checker_invariants state;
