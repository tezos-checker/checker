...
let ctez_of_fraction_ceil (x_num : Ligo.int) (x_den : Ligo.int) : ctez =
  ...
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_CtezOfFractionCeilNegative : ctez)
  else
    Ligo.abs
      (cdiv_int_int (Ligo.mul_int_int x_num ctez_scaling_factor_int) x_den)

let ctez_of_fraction_floor (x_num : Ligo.int) (x_den : Ligo.int) : ctez =
  assert (Ligo.gt_int_int x_den (Ligo.int_from_literal "0"));
  if Ligo.lt_int_int x_num (Ligo.int_from_literal "0") then
    (Ligo.failwith internalError_CtezOfFractionFloorNegative : ctez)
  else
    Ligo.abs
      (fdiv_int_int (Ligo.mul_int_int x_num ctez_scaling_factor_int) x_den)
...
let ctez_to_ratio (amnt : ctez) : ratio =
  make_ratio (Ligo.int amnt) ctez_scaling_factor_int

let ratio_of_ctez (x : ctez) : ratio =
  { num = ctez_to_muctez_int x; den = ctez_scaling_factor_int }
