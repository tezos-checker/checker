...
let[@inline] tz_minting (p : parameters) : Ligo.nat =
  max_nat p.index p.protected_index

(** Compute the current liquidation index (in tez). To get tez/kit must multiply with q. *)
let[@inline] tz_liquidation (p : parameters) : Ligo.nat =
  min_nat p.index p.protected_index
...
let[@inline] compute_imbalance (outstanding : kit) (circulating : kit) : ratio =
  ...
  if
    Ligo.eq_nat_nat circulating (Ligo.nat_from_literal "0n")
    && Ligo.eq_nat_nat outstanding (Ligo.nat_from_literal "0n")
  then zero_ratio
  else if
    Ligo.eq_nat_nat circulating (Ligo.nat_from_literal "0n")
    && Ligo.ne_nat_nat outstanding (Ligo.nat_from_literal "0n")
  then make_ratio (neg_int num_il) den_il
  else
    ...
    if Ligo.geq_nat_nat circulating outstanding then
      make_ratio
        (min_int
           (Ligo.mul_int_int
              (Ligo.mul_int_int num_isf
                 (Ligo.sub_nat_nat circulating outstanding))
              den_il)
           (Ligo.mul_int_int num_il denominator))
      ...
      make_ratio
        (neg_int
           (min_int
              (Ligo.mul_int_int
                 (Ligo.mul_int_int num_isf
                    (Ligo.sub_nat_nat outstanding circulating))
                 den_il)
              (Ligo.mul_int_int num_il denominator)))
        (Ligo.mul_int_int den_il denominator) )
...
let compute_drift_derivative (target : fixedpoint) : fixedpoint =
  ...
  let mul_sub_den_tlb_num_tlb_sf =
    Ligo.mul_int_int
      (Ligo.sub_int_int den_tlb num_tlb)
      fixedpoint_scaling_factor
  in
  let mul_add_den_tlb_num_tlb_sf =
    Ligo.mul_int_int
      (Ligo.add_int_int den_tlb num_tlb)
      fixedpoint_scaling_factor
  in

  let mul_target_thb = Ligo.mul_int_int target den_thb in
  let mul_sub_den_thb_num_thb_sf =
    Ligo.mul_int_int
      (Ligo.sub_int_int den_thb num_thb)
      fixedpoint_scaling_factor
  in
  let mul_add_den_thb_num_thb_sf =
    Ligo.mul_int_int
      (Ligo.add_int_int den_thb num_thb)
      fixedpoint_scaling_factor
  in

  if
    Ligo.lt_int_int mul_sub_den_tlb_num_tlb_sf mul_target_tlb
    && Ligo.lt_int_int mul_target_tlb mul_add_den_tlb_num_tlb_sf
  then fixedpoint_zero (* no acceleration (0) *)
  else if
    Ligo.lt_int_int mul_sub_den_thb_num_thb_sf mul_target_thb
    && Ligo.leq_int_int mul_target_tlb mul_sub_den_tlb_num_tlb_sf
  then low_negative_acceleration
  else if
    Ligo.gt_int_int mul_add_den_thb_num_thb_sf mul_target_thb
    && Ligo.geq_int_int mul_target_tlb mul_add_den_tlb_num_tlb_sf
  then low_positive_acceleration
...
(** Calculate the current burrow fee index based on the last index and the
    ...
    ever-increasing. *)
let[@inline] compute_current_burrow_fee_index
    (last_burrow_fee_index : fixedpoint) (duration_in_seconds : Ligo.int)
    : fixedpoint
  =
  let { num; den } = burrow_fee_percentage in
  let denom = Ligo.mul_int_int den seconds_in_a_year in
  fixedpoint_of_raw
    (fdiv_int_int
       (Ligo.mul_int_int
          (fixedpoint_to_raw last_burrow_fee_index)
          (Ligo.add_int_int denom (Ligo.mul_int_int num duration_in_seconds)))
       denom)
...
*)
let[@inline] compute_current_protected_index (last_protected_index : Ligo.nat)
    (current_index : Ligo.nat) (duration_in_seconds : Ligo.int)
    : Ligo.nat
  =
  assert (Ligo.gt_nat_nat last_protected_index (Ligo.nat_from_literal "0n"));
  fraction_to_nat_floor
    (clamp_int
       (Ligo.mul_nat_int current_index protected_index_inverse_epsilon)
       (Ligo.mul_nat_int last_protected_index
          (Ligo.sub_int_int protected_index_inverse_epsilon duration_in_seconds))
       (Ligo.mul_nat_int last_protected_index
          (Ligo.add_int_int protected_index_inverse_epsilon duration_in_seconds)))
...
*)
let[@inline] compute_current_drift (last_drift : fixedpoint)
    (last_drift_derivative : fixedpoint) (current_drift_derivative : fixedpoint)
  ...
  fixedpoint_of_raw
    (fdiv_int_int
       (Ligo.add_int_int
          ...
          (Ligo.mul_int_int
             (fixedpoint_to_raw
                (fixedpoint_add last_drift_derivative current_drift_derivative))
             duration_in_seconds))
       (Ligo.int_from_literal "2"))
...
(** Calculate the current quantity based on the last quantity, the last drift,
    ...
    where [EXP(X) = X+1].
*)
let[@inline] compute_current_q (last_q : fixedpoint) (last_drift : fixedpoint)
    (last_drift_derivative : fixedpoint) (current_drift_derivative : fixedpoint)
    (duration_in_seconds : Ligo.int)
    : fixedpoint
  =
  ...
  fixedpoint_of_raw
    (fdiv_int_int
       (Ligo.mul_int_int (fixedpoint_to_raw last_q)
          (Ligo.add_int_int
             (Ligo.mul_int_int
                (Ligo.add_int_int
                   (Ligo.mul_int_int
...
*)
let[@inline] compute_current_target (current_q : fixedpoint)
    (current_index : Ligo.nat) (current_kit_in_tez : ratio)
    : fixedpoint
  =
  let { num; den } = current_kit_in_tez in
  fixedpoint_of_raw
    (fdiv_int_int
       (Ligo.mul_int_int den
          (Ligo.mul_int_nat (fixedpoint_to_raw current_q) current_index))
       (Ligo.mul_int_int (Ligo.int_from_literal "1_000_000") num))
...
let[@inline] compute_current_imbalance_index (last_outstanding_kit : kit)
    (last_circulating_kit : kit) (last_imbalance_index : fixedpoint)
    (duration_in_seconds : Ligo.int)
    : fixedpoint
  =
  let { num; den } =
    compute_imbalance last_outstanding_kit (* burrowed *) last_circulating_kit
    (* circulating *)
  in
  let denom = Ligo.mul_int_int den seconds_in_a_year in
  fixedpoint_of_raw
...
*)
let[@inline] compute_current_outstanding_with_fees (last_outstanding_kit : kit)
    (last_burrow_fee_index : fixedpoint) (current_burrow_fee_index : fixedpoint)
    : kit
  =
  kit_of_fraction_floor
    (Ligo.mul_nat_int
       (kit_to_mukit_nat last_outstanding_kit)
       (fixedpoint_to_raw current_burrow_fee_index))
    (Ligo.mul_int_int kit_scaling_factor_int
       (fixedpoint_to_raw last_burrow_fee_index))
...
*)
let[@inline] compute_current_outstanding_kit
    (current_outstanding_with_fees : kit) (last_imbalance_index : fixedpoint)
    (current_imbalance_index : fixedpoint)
    : kit
  =
  kit_of_fraction_floor
    (Ligo.mul_nat_int
       (kit_to_mukit_nat current_outstanding_with_fees)
       (fixedpoint_to_raw current_imbalance_index))
    (Ligo.mul_int_int kit_scaling_factor_int
       (fixedpoint_to_raw last_imbalance_index))
...
(** Update the checker's parameters, given (a) the current timestamp
    ...
    sub-contract. *)
let parameters_touch (current_index : Ligo.nat) (current_kit_in_tez : ratio)
    (parameters : parameters)
    : kit * parameters
  =
  let {
    q = parameters_q;
    index = _parameters_index;
    (* unused. Always set to the new one. *)
  ...
  let duration_in_seconds =
    Ligo.sub_timestamp_timestamp !Ligo.Tezos.now parameters_last_touched
  in
  assert (Ligo.geq_int_int duration_in_seconds (Ligo.int_from_literal "0"));

  (* NOTE: the protocol should ensure this I believe. *)

  (* Update the indices *)
  let current_burrow_fee_index =
    compute_current_burrow_fee_index parameters_burrow_fee_index
      duration_in_seconds
  in
  let current_imbalance_index =
    compute_current_imbalance_index parameters_outstanding_kit
      parameters_circulating_kit parameters_imbalance_index duration_in_seconds
  in

  (* Calculate all parameter updates and accrual to cfmm. *)
  let current_protected_index =
    compute_current_protected_index parameters_protected_index current_index
      duration_in_seconds
  in
  let current_drift_derivative = compute_drift_derivative parameters_target in
  let current_drift =
    compute_current_drift parameters_drift parameters_drift_derivative
      current_drift_derivative duration_in_seconds
  in
  let current_q =
    compute_current_q parameters_q parameters_drift parameters_drift_derivative
      current_drift_derivative duration_in_seconds
  in
  let current_target =
    compute_current_target current_q current_index current_kit_in_tez
  in
  let current_outstanding_with_fees =
    compute_current_outstanding_with_fees parameters_outstanding_kit
      parameters_burrow_fee_index current_burrow_fee_index
  in
  let accrual_to_cfmm =
    kit_sub current_outstanding_with_fees parameters_outstanding_kit
  in
  (* NOTE: can this be negative? *)
  let current_outstanding_kit =
    compute_current_outstanding_kit current_outstanding_with_fees
      parameters_imbalance_index current_imbalance_index
  in
  let current_circulating_kit =
    kit_add parameters_circulating_kit accrual_to_cfmm
  in
...
let[@inline] add_circulating_kit (parameters : parameters) (kit : kit)
    : parameters
  =
...
let[@inline] remove_circulating_kit (parameters : parameters) (kit : kit)
    : parameters
  =
...
let[@inline] add_outstanding_and_circulating_kit (parameters : parameters)
    (kit : kit)
    : parameters
  =
  {
...
let[@inline] remove_outstanding_and_circulating_kit (parameters : parameters)
    (kit : kit)
    : parameters
  =
