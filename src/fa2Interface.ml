...
type fa2_token_id = Ligo.nat [@@deriving show]
...
let ensure_valid_fa2_token (n : fa2_token_id) : unit =
  if n = kit_token_id || n = lqt_token_id then ()
  else failwith "FA2_TOKEN_UNDEFINED"

type fa2_state = {
  ledger : (fa2_token_id * Ligo.address, Ligo.nat) Ligo.big_map;
  operators :
    ( Ligo.address (* operator *) * Ligo.address (* owner *) * fa2_token_id,
      unit )
    Ligo.big_map;
}
...
let initial_fa2_state =
  {
    ledger =
      (Ligo.Big_map.empty : (fa2_token_id * Ligo.address, Ligo.nat) Ligo.big_map);
    operators =
      (Ligo.Big_map.empty : ( Ligo.address * Ligo.address * fa2_token_id,
                              unit )
                            Ligo.big_map);
  }
...
let set_fa2_ledger_value
    (ledger : (fa2_token_id * Ligo.address, Ligo.nat) Ligo.big_map)
    (key : fa2_token_id * Ligo.address) (value : Ligo.nat)
    : (fa2_token_id * Ligo.address, Ligo.nat) Ligo.big_map
  =
  if value = Ligo.nat_from_literal "0n" then Ligo.Big_map.remove key ledger
  else Ligo.Big_map.add key value ledger

let ledger_issue
    ((st, tok, addr, amnt) : fa2_state * fa2_token_id * Ligo.address * Ligo.nat)
    : fa2_state
  =
...
let[@inline] ledger_issue_then_withdraw
    ((st, tok, addr, amnt_to_issue, amnt_to_withdraw) :
      fa2_state * fa2_token_id * Ligo.address * Ligo.nat * Ligo.nat)
    : fa2_state
  =
  let ledger = st.ledger in
  let key = (tok, addr) in
...
let[@inline] fa2_is_operator
    ((st, operator, owner, token_id) :
      fa2_state * Ligo.address * Ligo.address * fa2_token_id)
  =
  owner = operator || Ligo.Big_map.mem (operator, owner, token_id) st.operators

let[@inline] fa2_get_balance
    ((st, owner, token_id) : fa2_state * Ligo.address * fa2_token_id)
    : Ligo.nat
  =
  let ledger = st.ledger in
...
let[@inline] fa2_all_tokens : Ligo.nat list = [ kit_token_id; lqt_token_id ]

let[@inline] fa2_run_balance_of
    ((st, xs) : fa2_state * fa2_balance_of_request list)
...
let[@inline] fa2_run_update_operators
    ((st, xs) : fa2_state * fa2_update_operator list)
    : fa2_state
  =
  Ligo.List.fold_left
    (fun ((st : fa2_state), (x : fa2_update_operator)) ->
      ...
      | Add_operator op ->
          ...
          if owner <> !Ligo.Tezos.sender then
            (failwith "FA2_NOT_OWNER" : fa2_state)
          else
            {
              st with
              operators =
                Ligo.Big_map.add (operator, owner, token_id) () st.operators;
            }
      | Remove_operator op ->
          let { owner; operator; token_id } = op in
          if owner <> !Ligo.Tezos.sender then
          ...
          else
            {
              st with
              operators =
                Ligo.Big_map.remove (operator, owner, token_id) st.operators;
            })
    st xs

let[@inline] fa2_run_transfer ((st, xs) : fa2_state * fa2_transfer list)
    : fa2_state
  =
  Ligo.List.fold_left
    (fun ((st, tx) : fa2_state * fa2_transfer) ->
      ...
      Ligo.List.fold_left
        (fun ((st, x) : fa2_state * fa2_transfer_destination) ->
          let { to_; token_id; amount = amnt } = x in
          if fa2_is_operator (st, !Ligo.Tezos.sender, from_, token_id) then
            let () = ensure_valid_fa2_token token_id in
            let st = ledger_withdraw (st, token_id, from_, amnt) in
            let st = ledger_issue (st, token_id, to_, amnt) in
            st
          else (failwith "FA2_NOT_OPERATOR" : fa2_state))
        st txs)
    st xs

let[@inline] ledger_issue_kit
    ((st, addr, amnt) : fa2_state * Ligo.address * kit)
    : fa2_state
  =
...
let[@inline] ledger_issue_then_withdraw_kit
    ((st, addr, amnt_to_issue, amnt_to_withdraw) :
      fa2_state * Ligo.address * kit * kit)
    : fa2_state
  =
  ledger_issue_then_withdraw
    ( st,
      kit_token_id,
      addr,
      kit_to_mukit_nat amnt_to_issue,
      kit_to_mukit_nat amnt_to_withdraw )

let[@inline] ledger_issue_lqt
    ((st, addr, amnt) : fa2_state * Ligo.address * lqt)
    : fa2_state
  =
...
let fa2_get_token_balance (st : fa2_state) (token_id : fa2_token_id) : Ligo.nat =
  ...
  |> List.fold_left
       (fun x y -> Ligo.add_nat_nat x y)
       (Ligo.nat_from_literal "0n")

let fa2_get_total_kit_balance (st : fa2_state) : kit =
  kit_of_mukit (fa2_get_token_balance st kit_token_id)

let fa2_get_total_lqt_balance (st : fa2_state) : lqt =
  lqt_of_denomination (fa2_get_token_balance st lqt_token_id)
