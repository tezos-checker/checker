...
let test_protected_index_pace =
  "test_protected_index_pace" >:: fun _ ->
    ...
    assert_equal ~printer:Tez.show (Tez.of_mutez (Z.of_int 1_030_420)) new_params.protected_index;
    ...
    assert_equal ~printer:Tez.show (Tez.of_mutez (Z.of_int 2_053_031)) new_params.protected_index;
    ...
    assert_equal ~printer:Tez.show (Tez.of_mutez (Z.of_int 970_407)) new_params.protected_index;
    ...
    assert_equal ~printer:Tez.show (Tez.of_mutez (Z.of_int 486_151)) new_params.protected_index
...
let test_minting_index_low_bounded =
  ...
  @@ QCheck.Test.make
    ...
    ~count:property_test_count
    (QCheck.map (fun x -> Tez.of_mutez (Z.of_int x)) QCheck.(0 -- 999_999))
  @@ fun index ->
  (* just the next block *)
  let new_tezos =
    { tezos with
  ...
  let _total_accrual_to_uniswap, new_params =
    Parameters.touch new_tezos index kit_in_tez params in
  (Parameters.tz_minting new_params >= Tez.of_mutez (Z.of_int 999_500)) (* 0.05% down, at "best" *)
...
let test_minting_index_high_unbounded =
  ...
  @@ QCheck.Test.make
    ...
    ~count:property_test_count
    (QCheck.map (fun x -> Tez.of_mutez (Z.of_int x)) QCheck.(1_000_001 -- max_int))
  @@ fun index ->
  (* just the next block *)
  let new_tezos =
    { tezos with
...
let test_liquidation_index_high_bounded =
  ...
  (Parameters.tz_liquidation new_params <= Tez.of_mutez (Z.of_int 1_000_500)) (* 0.05% up, at "best" *)
  ...
  "test_touch" >:: fun _ ->
    let initial_parameters : Parameters.t =
      { q = FixedPoint.of_hex_string "0.E666666666666666"; (* 0.9 *)
        index = Tez.of_mutez (Z.of_int 360_000);
        target = FixedPoint.of_hex_string "1.147AE147AE147AE1"; (* 1.08 *)
        protected_index = Tez.of_mutez (Z.of_int 350_000);
    ...
    let new_index = Tez.of_mutez (Z.of_int 340_000) in
    ...
    assert_equal
      { q = FixedPoint.of_hex_string "0.E6666895A3EC8BA5"; (* 0.90000013020828555983 *)
        index = Tez.of_mutez (Z.of_int 340_000);
        protected_index = Tez.of_mutez (Z.of_int 340_000);
