  ...
  "Auction tests" >::: [
    ("test starts descending auction" >::
     fun _ ->
       ...
       assert_equal (Some (Kit.of_mukit 2_000_000)) (Option.map (Auction.current_auction_minimum_bid start_time) auctions.current_auction) ~printer:show_some_kit;
       assert_equal (Some (Kit.of_mukit 2_000_000)) (Option.map (Auction.current_auction_minimum_bid start_time) auctions.current_auction) ~printer:show_some_kit;
       (* Price of descending auction should go down... *)
       let one_second_later = Timestamp.add_seconds start_time 1 in
       assert_equal (Some (Kit.of_mukit 1_998_000)) (Option.map (Auction.current_auction_minimum_bid one_second_later) auctions.current_auction) ~printer:show_some_kit;
       let two_seconds_later = Timestamp.add_seconds start_time 2 in
       assert_equal (Some (Kit.of_mukit 1_996_002)) (Option.map (Auction.current_auction_minimum_bid two_seconds_later) auctions.current_auction) ~printer:show_some_kit;
    );
